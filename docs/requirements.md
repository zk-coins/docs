...
Every zkCoins **wallet account** — an account whose seed a user holds through a wallet application, as opposed to one a node deployment operates for itself, such as a publisher's fee address — **has** an email-style NIP-05 name such as `alice@example.com`, publishes the signed payment object bound to it, and is therefore reachable and payable by that name alone. The obligation belongs to the wallet side of the system; node deployments that hold no wallet account, such as a publisher, carry no name. The name is **operational, not cryptographic**, and lives above the trustless core: it is resolved and hosted by the API layer or an external NIP-05 provider — never by the node kernel, which is addressed by key alone and must stay free of DNS and CA dependencies — is never derived from the seed, and enters no signature preimage and no value-bearing structure. Three consequences follow and are themselves requirements — an account can carry **several** names at once, any name can be **replaced** without touching keys, funds, or an established contact, and losing or changing a name can cost reachability under that name but can never cost custody, an identity, or a pinned counterparty. Lightning/LNURL and SMTP/email bridges are independent, optional operator services and are prerequisites for nothing.
