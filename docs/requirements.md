...
Every zkCoins **wallet account** — an account a user holds and is paid on — **has** an email-style NIP-05 name such as `alice@example.com`, publishes the signed payment object bound to it, and is therefore reachable and payable by that name alone. The obligation belongs to the wallet side of the system; node deployments that hold no wallet account, such as a publisher, carry no name. The name is **operational, not cryptographic**: it is served by a node or any name provider rather than by the wallet, is never derived from the seed, and enters no signature preimage and no value-bearing structure. Three consequences follow and are themselves requirements — an account can carry **several** names at once, any name can be **replaced** without touching keys, funds, or an established contact, and losing or changing a name can cost reachability under that name but can never cost custody, an identity, or a pinned counterparty. Lightning/LNURL and SMTP/email bridges are independent, optional operator services and are prerequisites for nothing.
