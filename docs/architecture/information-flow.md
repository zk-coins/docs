...
**What Bitcoin gives back (the index + integrity anchor).** The full ordered set of commitments and the global roots — reconstructable by any node. It lets you **authenticate every recovered bundle** (proof valid + anchored; the global double-spend check runs against the nullifier accumulator — normative in v1, [spec §3.7](/specification#37-the-nullifier-accumulator), implementation pending — see [Status](#status--caveats)). Because the on-chain commitment carries your signing public key, you can also **privately recognise your own commitments** — your seed derives the keys; outsiders cannot link the rotating keys, but you can — recovering the skeleton of your own activity.
