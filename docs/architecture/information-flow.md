...
The full grammar and resolution flow are specified in the [Access & Explorer §5.6](../specification/access-explorer) spec page. The canonical form is the local `zkcoins:` scheme (dispatched by a registered handler, never a network request); an HTTPS rendering — `https://explorer.zkcoins.app/tx#<bundle>/<view>` — **MUST** carry the `<bundle>`/`<view>` pair in the URL **fragment**, so the bearer secret never reaches a server, proxy, or `Referer` header, and the explorer decrypts client-side (see [Access & Explorer §5.6](../specification/access-explorer)).
