...
Independent of the service-layer `name@host` aliases above, the protocol itself encodes addresses and capabilities as **Bech32m** strings with distinct human-readable prefixes (HRPs), so the different objects can never be confused ([spec §1.7.7](/specification#177-bech32m-and-bitcoin-conventions)):
