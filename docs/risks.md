...
| Availability is a multi-holder guarantee, not cryptographic | **accepted v1 boundary** | recovery needs a reachable bootstrap node plus ≥1 live holder per plane (a seed-discoverable relay for the event + blob store for the blob, the sender's node, a self-hosted sync, or an operator backup); simultaneous total loss of every copy is unrecoverable, by design — the same rule as the seed |
