...
| D-15 | NIP-05 identity and contact discovery | Not part of either source. | Mandatory email-style NIP-05 identity maps to the standard Nostr `op_pubkey`; kind 0 carries the additive `zkcoins` payment block; first discovery is TOFU-pinned as an `nprofile`, after which known contacts use stored relay hints without DNS. | AMBER | None for v1 — standard NIP-05/NIP-17 naming and messaging layer with explicit first-use trust and DNS-resistant known-contact behavior (spec §4.3). | CURRENT — mandatory NIP-05 identity, standard kind-0 profile, and pinned Nostr contact discovery ([spec §4.3](/specification#43-addressing-for-delivery)). |
