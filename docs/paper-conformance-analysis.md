...
| D-15 | Naming layer and contact discovery | Not part of either source. | Email-style NIP-05 names sit **above** the protocol: optional, plural per account, never seed-derived, in no signature preimage, and replaceable without touching keys or funds — the node operates fully without one, and only end-user apps require them. Contact records are keyed by `op_pubkey`. First discovery through a name searches the union of recommended NIP-05 hints and configured profile/discovery/bootstrap relays, then TOFU-pins the **key** using only relays that returned valid relevant events. It retains an IP endpoint only after original-hostname-authenticated TLS and a successful relay WebSocket upgrade on the original path. Known contacts preserve that original scheme/host/port/path without DNS. | AMBER | None for v1 — the naming layer carries no value and no authority, so its failure modes are display and reachability only: an invalidated name drops its label while the pinned key, payment identity, and conversation survive. Residual DNS-outage boundary: at least one previously reached retained endpoint must still serve its authenticated WebSocket relay path; relay failure or signed rotation solely to unknown hostnames is not survivable without DNS. | CURRENT — optional naming layer over seed-derived identity, opt-in kind-0 payment profile, key-pinned contact discovery ([spec §4.3](/specification#43-addressing-for-delivery), [V.12](/specification#v12-kind-0-zkcoins-profile-and-nip-17-interoperability)); app-layer name requirements in the [Implementation Mandate](/implementation-mandate#app-layer-identity-and-contacts-normative). |
