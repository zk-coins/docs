...
- the paper’s publisher fee construction becomes sponsored publishing with no protocol fee, the selected-publisher fee coin being deferred ([spec §3.8.1](/specification#381-fee-coin-mechanism-deferred));
...
| D-09 | Publisher fees | First publisher to post can complete `payment_finalize_fee`. | v1 publishing is **sponsored**: no protocol fee, no fee coin, and every external hand-off is fee-less. The paid spender-picks-publisher design is retained but deferred ([spec §3.8.1](/specification#381-fee-coin-mechanism-deferred)). Under the deferred design the spender would include a fee coin under the same `ocr`. | AMBER | None for v1 — verdict **holds** in the [Risks](/risks) table (publisher censorship/delay row); no fee path exists to test. Historical cell superseded. | CURRENT, registered deviation — **sponsored publishing, no protocol fee**; both the spender-picks-publisher fee coin and the paper's first-to-publish-wins design are explicitly deferred ([spec §3.8](/specification#38-fees-and-economics)); incentive verdict **holds** — nothing is paid, so a censoring or lazy publisher costs a spender only a re-pick, and permissionless self-publish is the universal escape; recorded in the [Risks](/risks) verdict table under publisher censorship/delay (fee **pricing** risk is the separate D-20). |
...
Under the deferred fee mechanism the fee coin would be cryptographically atomic with the selected transition, which is good, and the fee asset may be illiquid or issuer-controlled, while Bitcoin fees must be paid in BTC. Publishers therefore bear exchange-rate, inventory and settlement risk. A market may converge on one publisher or a narrow accepted-asset list even without a protocol permission gate.
...
Poseidon vectors and both `circuit_digest(C)` / `circuit_digest(C_balance)` (mainnet, testnet, regtest) are pinned in V.4 from the reference implementation on open [`node#231`](https://github.com/zk-coins/node/pull/231) — not yet merged to `develop`. Intentionally unpinned residual cells remain: V.5/V.6 production signatures (verification-bound, not byte-pinned), the deployment-specific `block_anchor`, and harness-generated V.11 boundary-suite fixtures. Independent SDK reproduction of the hash- and derivation-level subset is still open.
...
The project’s own [Assurance Roadmap](/assurance) lists security definitions, paper reductions, and machine checking as the verification staircase; for v1 the release-gating artefacts are the in-spec soundness argument and the executable harness. There is no external-audit gate.
...
2. Residual vector work only: V.5/V.6 verification-bound signatures, deployment `block_anchor`, harness V.11 boundary fixtures; merge the generating node implementation ([`node#231`](https://github.com/zk-coins/node/pull/231)).
...
4. There is no external-audit gate; v1 assurance is the conformance harness and the in-spec soundness argument.
