...
| D-15 | NIP-05 identity and contact discovery | Not part of either source. | Mandatory email-style NIP-05 identity maps to the standard Nostr `op_pubkey`; kind 0 carries the additive `zkcoins` payment block. First discovery searches the union of recommended NIP-05 hints and configured profile/discovery/bootstrap relays, then TOFU-pins a standard `nprofile` using only relays that returned valid relevant events. It retains an IP endpoint only after original-hostname-authenticated TLS and a successful relay WebSocket upgrade on the original path. Known contacts preserve that original scheme/host/port/path without DNS. | AMBER | None for v1 — standard NIP-05/NIP-17 naming and messaging with explicit first-use trust and a DNS-outage boundary: at least one previously reached retained endpoint must still serve its authenticated WebSocket relay path; relay failure or signed rotation solely to unknown hostnames is not survivable without DNS. | CURRENT — mandatory NIP-05 identity, standard kind-0 profile, pinned Nostr contact discovery, and cold-start DNS-outage conformance ([spec §4.3](/specification#43-addressing-for-delivery), [V.12](/specification#v12-kind-0-zkcoins-profile-and-nip-17-interoperability)). |
