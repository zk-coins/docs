...
|---|---|---|
| `quoted` | the user requests a swap for a given invoice or amount | disclose the **amount, the fee/spread, and the exact refund amount**, issue the swap's `Invoice` ([spec §4.3](/specification#43-addressing-for-delivery)), and state the advertised caps, **before** the user commits |
| `funded` | the user's zkCoins transfer, paying the quote's `Invoice`, verifies (§4.2, §4.5) | attempt the Lightning payment for the quoted invoice |
| `settled` | the Lightning payment confirms | the swap is complete; nothing further is owed |
| `refunded` | the Lightning payment **fails definitively** | return the quote's **stated refund amount** to the user |
...
- The quote **MUST** include an operator-issued `Invoice` ([spec §4.3](/specification#43-addressing-for-delivery)) for the swap amount with a unique memo — the swap identifier — so verifying the user's transfer ties it to exactly this swap. A transfer that arrives after the quote's validity has expired matches no open swap, and the operator **MUST** return it.
...
The distinction between a **definitive** and a **transient** Lightning failure governs which state a swap ends in. A transient failure (a route that timed out, a temporarily-unavailable peer) keeps the swap in `funded` while the operator retries within the quote's validity; a **definitive** failure moves it to `refunded`. Exhaustion of the quote's validity without settlement **is itself a definitive failure**: a swap still unsettled when the validity expires moves to `refunded` under the refund rule above, so no swap can remain in `funded` indefinitely. A single funded swap resolves to exactly one terminal state — `settled` or `refunded` — so a user never both receives the Lightning payment and keeps the zkCoins.
