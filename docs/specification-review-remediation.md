...
|---|---|---|---|
| RB-P2-01 | `PARTIAL` | Supported Bitcoin Core versions, RPC/index requirements, archival/pruned profiles, network separation, snapshots, pagination, offline sync, and reorg handling are incomplete. | Publish a node dependency matrix and deterministic sync/recovery algorithm for Regtest, Signet, testnet, and mainnet. Test every supported Core/profile combination, pruned failure mode, offline interval, snapshot boundary, and reorg depth within the §3.9 finality bound (≤5-block canonical replay; a ≥6-block reorg surfaced as the accepted break, not recovered). |
