...
**Handle syntax.** Handle inputs are lowercased before validation and comparison, so `Alice@Example.com` normalises to `alice@example.com`. The canonical form — stored, displayed, resolved — is lowercase: the local part `<user>` is `a-z0-9-_.`, `<domain>` is a DNS hostname. The syntax is deliberately LUD-16-compatible (see *one handle for Lightning and zkCoins* below).
...
**Handle pinning.** On the **first** successful resolution the client **MUST** pin the mapping `<user>@<domain>` → `{address, op_pubkey, relays}` (trust-on-first-use). If a later resolution of the same handle yields a **different** `address`, the client **MUST** warn the user and **MUST NOT** proceed silently. The client **SHOULD** cross-check every subsequent resolution against the recipient's kind-30420 profile (`#d = <pinned address>`, [§7.3](#73-nostr-event-kinds-normative)) fetched from the **pinned** relay set — for an established handle, acceptance then requires the serving HTTPS domain and the pinned Nostr relay set to agree, and the domain alone can no longer silently re-map the handle. The first resolution remains trust-on-first-use.
