...
**Names are plural and replaceable (normative).** Any number of names, across any number of domains, **MAY** resolve to the same `op_pubkey`, and all are equally valid. A client **MUST NOT** treat one name as canonical, and **MUST NOT** require that a name appear in the account's kind-0 `nip05` field before accepting it for discovery. Adding or replacing a name changes no key, no address, and no pinned contact.

**Name loss (normative).** An unresolvable name **MUST NOT** invalidate the account's keys, `address`, ability to spend, retained conversations, or any pin a counterparty holds on it. The account is unreachable under that name until it resolves again or another name is published.
...
- **Reverse check fails, name already associated with a pinned `op_pubkey`.** The client **MUST** keep the contact, its conversation, and its pinned payment identity, **MUST** stop presenting that name, and **MUST NOT** delete, merge, or re-point the contact, nor treat messaging or payment as unavailable while retained transport serves. A name now mapping to a different key is a different contact and **MUST NOT** replace the pinned one.
...
*Name checklist* — applies **only** when the consumer reached the event by resolving a name: the reverse `nip05` match plus, for messaging, kind-10050 readiness ([§4.3](#43-addressing-for-delivery) *First discovery*). A consumer that arrived by `op_pubkey` — from an addressable event, an `nprofile`, an `Invoice`, or an existing contact — runs it not at all.
...
The event carries everything the three [§4.3](#43-addressing-for-delivery) checks need: a spender verifies it without a kind-0 event, without a name, and without DNS. `addr_sig` is BIP-340 under `pk0` over the profile-fixed `invoice_message` (`amount = 0`, all-zero `asset_id`, empty `memo`, `recipient` = `fee_address`, the event author as `op_pubkey`, and this event's `relays`). A wallet **MUST** verify, in order: (i) `H(pk0 ‖ nk_commit) == fee_address`; (ii) `addr_sig` under `pk0` over that message; (iii) the event signature under the author `op_pubkey`; plus the field-shape requirements above. Only then does it encrypt a fee `CoinProof` to this object's `ivpk`. Running a publisher therefore requires a key and a relay, never a domain, a name, or a CA ([§3.4](#34-the-publisher), [Requirement 7](/requirements)). The publisher's Bitcoin identity is just the reveal-transaction key ([§3.4](#34-the-publisher)) — there is **no** on-chain publisher protocol key.
...
**Name loss survivability (accept).** A fourth fixture starts from an established contact — name resolved, kind-0 and kind-10050 verified, `op_pubkey` and payment identity pinned, endpoints retained — then makes that name permanently unresolvable: the NIP-05 response 404s and DNS for the domain fails. The consumer **MUST** keep the contact, its conversation, its pinned payment identity, and its retained transport; **MUST** stop presenting that name; and **MUST** still complete NIP-17 in both directions and a payment against the pinned identity, with **zero** successful NIP-05 requests. It **MUST NOT** delete, merge, re-pin, or re-verify the contact.

**Multiple names, one key (accept).** A fifth fixture serves two names on two different domains, both mapping to the same `op_pubkey`, while the kind-0 carries only one of them in `nip05`. Discovery through **either** name **MUST** reach the same contact record, and the record **MUST** hold both names. A consumer **MUST NOT** create a second contact, and **MUST NOT** reject the name absent from `nip05` for payment or messaging.
