...
4. Require `|b| = L` for the call site's fixed expected length; a length mismatch **MUST** reject.

The two call sites and their fixed `(label, L)` pairs are:

| Field | Conversation key | `label` | Binary `b` | Expected `L` |
|---|---|---|---|---|
| `CoinProof.ciphertext` | `K_tx` | `"coin"` | `serialize(Coin)` ([§1.5](#15-core-data-structures)) | **112** |
| `out_ciphertext` (per `output_ref`) | `K_out` | `"K_tx"` | `K_tx` (32-byte note key) | **32** |
...
- **Outgoing recovery (`out_ciphertext`, normative).** For every outgoing coin the sender **MUST** derive `K_out = HKDF("zkCoins/v1/OutKey", ovk ‖ epk)` from its **own** `ovk` and produce `out_ciphertext = NIP44Binary(K_out, "K_tx", K_tx)` — the labelled binary envelope of the 32-byte note key under `K_out` as the conversation key. Each outgoing coin appears in the sender's `SelfDeliveryRecordV1` as an `output_ref { coin_id, blob_id, epk, out_ciphertext }` ([§4.2](#42-bundle-delivery)). A holder of the sender's `ovk` re-derives `K_out` from the stored `epk`, opens `out_ciphertext` under the decrypt rules above (`label = "K_tx"`, `L = 32`) to recover `K_tx`, and opens the outgoing coin's `ciphertext` under (`label = "coin"`, `L = 112`) — this is the concrete mechanism behind every "recover outgoing-coin plaintext" capability (§1.2, [§5.8](#58-address-view-full-history)). `ivk` alone therefore yields the incoming-only view; `ivk ‖ ovk` yields the full view.
...
3. The sender constructs a **delivery event**, an application-specific Nostr event whose plaintext payload is:
   ...
   DeliveryEvent.payload = {
     ...
     blob_locators,              // BlobLocatorSet {blob_id, holders:[base_url…]} — ordered
                                 //   holder hints; lives ONLY in this encrypted interior (§4.3)
...
SelfDeliveryRecordV1 =
  ...
  ‖ block_anchor ‖ occurred_at
  ‖ spent_or_folded_coin_ids[]
  ‖ output_refs[]                    // each { coin_id, blob_id, epk, out_ciphertext }
```
...
- **`block_anchor`** = `{ block_hash (32B), height (u32) }` as in [§3.5](#35-inscription-format). **`occurred_at`** is a `u64` Unix-seconds timestamp that is **NOT sender-asserted**: a conforming node **MUST** set it from the **verified first-occurrence inclusion** of this transition's own `(Pk, R)` in the nullifier-accumulator log ([§1.7.6](#176-nullifier-accumulator-append-only-merkle-log), [§3.6](#36-chain-scanning)) — the inclusion block's median time (or the node's chain-derived time for that height) — and **MUST** reject a recovered record whose claimed `occurred_at` is inconsistent with that first-occurrence inclusion.
...
The control event (step 3) and the acknowledgement are small and use **NIP-44 v2** directly. The **blob plaintext** — a `CoinProof` bundle or a `SelfDeliveryRecordV1` (typically ~100 KB) — exceeds NIP-44 v2's 65 535-byte plaintext limit, so it is encrypted with **zkCoins Bundle Encryption (ZBE)** — a chunked AEAD framing over **ChaCha20-Poly1305** (an IETF-standard AEAD; NIP-44 v2 uses the same ChaCha20 stream cipher but pairs it with HMAC-SHA-256 rather than Poly1305, so ZBE adds Poly1305 as its only new primitive — a standard, conservative choice). ZBE is its own on-wire format, **not** a sequence of NIP-44 v2 messages. ZBE applies to the value-bearing, recipient-encrypted off-chain blobs (`CoinProof` and `SelfDeliveryRecordV1`). (The nullifier accumulator is rebuilt from the on-chain nullifiers alone, [§3.6](#36-chain-scanning), so there is no public, consensus-bearing off-chain blob that would have to be stored in plaintext for every scanner to read.)
...
BootstrapManifestV1 = {
  ...
  manifest_sig                      // BIP-340 over bootstrap_message by a network bootstrap key
}
...
ReplicaReceiptV1 = {
  ...
  holder_op_pubkey,         // x-only op pubkey of the holding operator
  ...
  receipt_sig               // BIP-340(holder_op, receipt_message)
}
...
3. Decodes `serialize(BalanceAttestation)` per [§7.1](#71-serialization-conventions-normative) (reject wrong-width fields, length-prefix overflow, trailing bytes).
4. Checks `subject` equals `<address>`, `asset_id` equals `<asset_id>`, then verifies the attestation statement and host-side anchor checks of this section (proof under `C_balance`, canonical `nav_ceiling`, first-occurrence `completed` anchor).
...
‖ block_anchor = block_hash (32B) ‖ height (4B u32-be)
‖ occurred_at (8B u64-be)                                     // from first-occurrence inclusion, §4.2
...
Each **`output_ref`** is `coin_id (32B) ‖ blob_id (32B) ‖ epk (32B x-only) ‖ u16-be len(out_ciphertext) ‖ out_ciphertext` (`out_ciphertext` = UTF-8 bytes of the NIP-44 Base64 payload from `NIP44Binary(K_out, "K_tx", K_tx)`, [§1.3](#13-per-coin-keys-note-encryption--detection)). `M = 0` for a pure receive ([§2.3.3](#233-receive)); `N_spent = 0` for a pure mint that spends no prior coin. A trailing byte after the `M`th `output_ref`, a length prefix exceeding the remaining bytes, an unrecognised `magic`/`version`/`record_kind`, or a wrong-width fixed field makes the record **malformed** and **MUST** be rejected. This byte string is what ZBE encrypts for self-delivery content-addressing (`blob_id = H(ciphertext)`, [§4.2.1](#421-bundle-blob-encryption-zbe-normative)) and is the concrete framing behind every multi-device sync and emergency state rebuild of [§4.2](#42-bundle-delivery)/[§4.5](#45-recovery) and every "recover outgoing-coin plaintext" capability of [§1.3](#13-per-coin-keys-note-encryption--detection) and [§5.8](#58-address-view-full-history). The complete plaintext **MUST** remain inside the encrypted blob — the outer gift-wrap exposes only `zkdt`/`zkepk` ([§4.2](#42-bundle-delivery)).
...
| `PUT` | `/blossom/upload` | store a blob; the server computes `blob_id = H(body)` and returns it plus, when applicable, a `ReplicaReceiptV1` | authorization event (below) so only known peers fill storage |
...
The server **MUST** reject (`401`) an event whose signature, `kind`, `t` tag (mismatched method), or `x` tag (mismatching the body hash / target blob) fails; reject (`401`) an expired or future-dated event (`expiration` past, or `created_at` outside the window above — too far in the future beyond 60 s skew, or older than `replay_window`); reject (`403`) a PUT whose `op` key is not the paired account's or a configured replication peer's ([§4.6](#46-data-availability--replication-factor-k)); reject (`403`) a DELETE whose `op` key is not the blob's original uploader; and reject (`413`) a body over the advertised size limit. A successful PUT returns `200 { "blob_id": <hex32>, "receipt"?: ReplicaReceiptV1Json }` where `blob_id = H(body)` — the content address of [§4.2.1](#421-bundle-blob-encryption-zbe-normative). The optional `receipt` is a [§4.6](#46-data-availability--replication-factor-k) `ReplicaReceiptV1` and **MUST** be present only when the holder has durably committed **both** this blob **and** the delivery event identified by the request's `event_id` / `attempt_nonce` metadata (request headers or accompanying JSON fields on the upload, implementation-local framing; the signed receipt fields themselves are fixed by §4.6). Uploading bytes already present is idempotent and returns the same `blob_id` (and re-issues a receipt only after the durable dual-commit condition still holds). **`receipt` is a direct upload response to the uploader only** — it **MUST NOT** be published as a Nostr event, cleartext tag, or gossipable holder record ([§4.6](#46-data-availability--replication-factor-k) privacy). `GET /blossom/<blob_id>` returns the raw bytes and is unauthenticated (ciphertext is self-protecting, §4.2.1). A successful `DELETE` returns `200` with an empty body; a `DELETE` refused under the [§4.6](#46-data-availability--replication-factor-k) retention rules — the blob is still a required replica — returns `409` with `machine_code` `retention_hold`, and the blob **MUST** be retained.
...
| `GET` | `/v1/info` | `{ network, protocol_version: "v1", circuit_digests: { C, C_balance }, relay_url, blossom_url, max_blob_bytes, finality_confirmations: 6, activation_height: <u64>, max_tx_inputs: 8, max_tx_outputs: 8, max_rx_coins: 4, max_account_assets: 32, roles: [<closed role strings>], bootstrap: BootstrapManifestV1 }` — `network ∈ {mainnet, testnet, regtest}` is the sole network tag (v1 pins Bitcoin network 1:1 to this tag; there is **no** separate `bitcoin_network` field). `roles` is the closed advertisement set of [§6.4](#64-external-interfaces-abstract) / [§6.1](#61-components-and-responsibilities): each element ∈ `{core, local_prover, hosted_wallet_api, publisher, aliasing}`; unknown values **MUST** be ignored on read; a missing optional role means that role is **off**. `bootstrap` mirrors the per-network signed [§4.3](#43-addressing-for-delivery) manifest (seed relays, blob stores, operator IDs only; **no** account- or blob-specific fields) |
...
| `GET` | `/v1/chain/inscriptions?from_height=&limit=` | paginated list of zkCoins nullifier inscriptions. **Query (normative):** `from_height` optional, default `0` (inclusive lower bound on inscription height); `limit` optional, default `100`, valid range `1..1000` — `limit = 0` or `limit > 1000` → `400 bounds_exceeded`. **Response:** `{ inscriptions: [ { txid, height, count, format, nullifiers: [{ pubkey, r, state }], confirmation_state } ], next_height?: <u64> }` — the half-aggregated `(Pkⱼ, Rⱼ)` set of each inscription ([§3.5](#35-inscription-format)), whose signatures the node has verified against Bitcoin; each `nullifiers[i].state` is that member's §3.10 state (`completed` \| `pending` \| `failed`) — members of one aggregate MAY differ by first-occurrence (a later `Pk` collision is `failed` while earlier members stay `pending`/`completed`); `confirmation_state ∈ {pending, completed}` is **only** the reveal transaction's confirmation depth relative to the §3.9 6-confirmation floor (not a per-nullifier aggregate of §3.10) and **MUST NOT** carry `failed`. **Stable sort (normative):** ascending `(height, tx_index)` of the reveal transaction (then the §3.6 payload-member order inside one inscription). `next_height` is the exclusive cursor for the next page — the smallest height strictly greater than the last returned inscription's height when more results remain, else **absent** (a client requests the next page with `from_height = next_height`) |
...
- `fee_blob_locators` — the `BlobLocatorSet` for that blob, encrypted to the publisher `IVPK` (NIP-44 / labelled binary envelope, same privacy boundary as the [§4.2](#42-bundle-delivery) delivery interior — holder URLs never cleartext on this wire);
- `fee_epk` — a **fresh** per-hand-off ephemeral x-only public key so the publisher kernel re-derives `ss = ECDH(ivk, fee_epk)`, `K_tx = HKDF("zkCoins/v1/NoteKey", ss ‖ fee_epk)`, and **decrypts** the ZBE blob.
...
2. when the three fee fields are present: fetch the ZBE blob by `fee_blob_id` via `fee_blob_locators`, **decrypt** it with `ivk` and `fee_epk` ([§4.2.1](#421-bundle-blob-encryption-zbe-normative)), and open the fee `CoinProof` — fail → `invalid_fee_coinproof`;
...
|---|---|---|---|
| `GetInfo` | unary | network, `protocol_version`, `circuit_digests`, finality + bounds, `roles`, sync/`ready` (+ `ready_reason` when not ready) | `GET /v1/info`, `/health/ready` |
| `GetAccumulator` | unary | current `{ size, root, tip_block_hash, tip_height }` | `GET /v1/chain/accumulator` |
| `ListInscriptions` | server-stream | zkCoins nullifier inscriptions from a height (paginated; defaults `from_height=0`, `limit=100`) | `GET /v1/chain/inscriptions` |
...
message ListInscriptionsRequest {
  ...
  //   limit = 100. Valid limit ∈ 1..1000; 0 or >1000 → INVALID_ARGUMENT / HTTP 400 bounds_exceeded.
  // Proto3: optional so absence is distinguishable from zero; a caller that sets limit = 0 is rejected.
  // REST response carries next_height as an exclusive pagination cursor (§7.5); the kernel stream
  //   itself yields Inscription messages in stable sort order.
...
message RecordRef {
  ...
  uint32 record_kind = 2;                  // §4.2: 1=mint, 2=send, 3=receive; coin-only refs MAY use 0
  bytes blob_id = 3;                       // H(ciphertext), §4.2.1
  uint64 occurred_at = 4;                  // first-occurrence-derived; 0 if unknown
}
...
message RecordBlob { bytes canonical = 1; uint32 record_kind = 2; }  // §7.1 CoinProof or SelfDeliveryRecordV1 bytes
...
message PublishRequest {
  ...
  bytes fee_blob_locators = 8;              // NIP44Binary-encrypted BlobLocatorSet to publisher IVPK; empty iff fee_blob_id empty
}
...
- **DeliveryEvent** — Nostr delivery: rumor payload `{blob_id, blob_locators, ack_nonce}` (optional `record_kind` for self-delivery), NIP-44 encrypted to `IVPK` and NIP-59 gift-wrapped under an ephemeral key; the **outer** kind-1059 event carries **only** the per-coin scan tags `zkdt`/`zkepk` in cleartext. `blob_locators` is the encrypted interior `BlobLocatorSet`. The `ack_nonce` is a fresh sender-chosen 32-byte value the recipient echoes in the kind-1421 ACK content `{detect_tag, blob_id, ack_nonce, op_sig}`, binding the ACK to this delivery attempt. ([§4.2](#42-bundle-delivery), [§7.3](#73-nostr-event-kinds-normative))
...
- **`network_id`** — `Hc("Network", network_tag_bytes)`; public input of `C` and `C_balance` (4 Goldilocks field elements after `consumed_pubkey`); proof-level network binding only — does **not** bind the BIP-340 signature. ([§1.4](#14-identifiers-and-hashes), [§2.5](#25-circuit-dimensioning-normative))
...
- **`BlobLocatorSet`** — `{blob_id, holders:[base_url…]}` carried only inside encrypted delivery / self-delivery interiors — never as public holder gossip. ([§4.2](#42-bundle-delivery), [§4.3](#43-addressing-for-delivery))
...
| `prf_salt` | `SHA-256(UTF8("zkCoins/v1/PasskeyPRF"))` — the fixed WebAuthn `eval.first` salt of §1.2 | pin by single evaluation of the formula (SHA-256 of that exact ASCII; no other input) |
| `seed` | `HKDF-SHA256(IKM = prf_output, salt = 0x00×32, info = "zkCoins/v1/PasskeySeed", L = 32)` — i.e. `HKDF("zkCoins/v1/PasskeySeed", prf_output)` under the [§1.1](#11-cryptographic-primitives) mapping | pin by single evaluation of the formula over the pinned `prf_output` |
...
| V.8 synthetic signing fixture | MUST recompute & produce | MUST recompute & reproduce | **byte-equal** after recompute against fixture `m_state = "zkCoins/v1/StateUpdate/testnet"` (nonce rule deterministic; signature/aggregate rows are `<REGEN>` until filled) |
| V.11 nullifier-accumulator log vectors (`nflog_empty`, hand-listed `mth@n` / inclusion / consistency over the pinned sample-leaf sequence, **and** the **generated log-boundary suite** for every `k = 0…63`) | MUST produce | MUST reproduce | **byte-equal** (once the `<REGEN>` bytes are filled; both sides implement the [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log) Poseidon log). Structure / positions / boundary sizes / Accept\|Reject of the generated suite are normative **before** `<REGEN>` fill; after fill, every emitted node/root path is byte-equal. Feeds the D-05 release gate ([§1.7.8](#178-reference-instantiation-status-final-for-v1)) |
...
|---|---|---|
| `out_plain` | `UTF8("zkcoins-bin-v1:K_tx:" ‖ base64url_no_pad(K_tx))` — NIP-44 plaintext under conversation key `K_out` | pin by single evaluation over the pinned `K_tx` (ASCII; no Poseidon) |
...
Conformance vectors for the [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log) append-only Merkle log and the [§3.7](#37-the-nullifier-accumulator) inclusion / consistency proofs. All values are Poseidon-over-Goldilocks (`<REGEN>`, produced by the reference implementation — never hand-authored); the **construction structure** (split point `k` = largest power of two `< n`, the `MTH` / `PATH` / `SUBPROOF` recursions of [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log) / [§3.7](#37-the-nullifier-accumulator)) is fixed here. That structure was validated **during specification** by differential-testing two independent RFC-6962 reference constructions against each other (byte-structure identical across every boundary and negative control); the reference implementation **MUST** reproduce exactly this structure when it produces the Poseidon `<REGEN>` bytes below, which remain unproduced until then. The **generated log-boundary suite** (below) is the normative full-range freeze differential-test for every high size-bit of the in-circuit RFC-6962 gadgets (`H_MAX = 64`, [§2.5](#25-circuit-dimensioning-normative)); it **feeds the D-05 release gate** ([§1.7.8](#178-reference-instantiation-status-final-for-v1), [Paper-Deviation Analysis D-05](/paper-conformance-analysis)).

**Sample leaf sequence (normative, pinned) — hand-listed smoke set.** `mth@n`, `inclusion@(p,n)`, and `consistency@(m,n)` in the hand-listed positive table below are computed over a pinned sequence of `(Pkₚ, Rₚ)` leaves: reuse the V.8 fixture's `(Pk_j, R_j)` as the first two leaves, and for positions `p ≥ 2` define `Pkₚ = H("zkCoins/v1/test-vector/nflog/pk" ‖ u8(p))` and `Rₚ = H("zkCoins/v1/test-vector/nflog/r" ‖ u8(p))` (SHA-256, so the sample leaves are pinnable). The leaf hash is `Hc("NfLog/Leaf", p ‖ Pkₚ ‖ Rₚ)` (Poseidon, hence the roots stay `<REGEN>`). The reference implementation **MUST** use exactly this sample sequence when producing the hand-listed `<REGEN>` bytes. This hand-listed set covers only small `n ≤ 9` and does **not** exercise the high size-bits of the gadgets; full-range coverage is the generated suite below.

**Positive — hand-listed smoke set (formulas pinned; bytes `<REGEN>`).**
...
**Generated log-boundary suite (normative — every `k = 0…63`).** A conforming reference implementation **MUST** generate and differential-test this suite against an independent RFC-6962 reference; the SDK **MUST** reproduce the same Accept/Reject structure and, once filled, the same `<REGEN>` node/root bytes (V.7 parity matrix). V.11 pins **only** structure, positions, boundary sizes, and expected Accept/Reject outcomes — every concrete Poseidon node, path, and root byte is `<REGEN>` (produced by the reference implementation; never hand-authored).

- **Capacity filter.** A boundary size `n` is in scope **iff** `0 ≤ n ≤ 2^{H_MAX} − 1` with `H_MAX = 64` ([§2.5](#25-circuit-dimensioning-normative)) — i.e. `n ≤ 2⁶⁴ − 1`. For every integer `k ∈ {0,…,63}` the three critical sizes `n ∈ {2ᵏ−1, 2ᵏ, 2ᵏ+1}` that satisfy the filter are in scope (all three are in scope for every such `k`).
- **Boundary-suite leaf formula (normative).** For every position `p` used by the suite, `Pkₚ = H("zkCoins/v1/test-vector/nflog/boundary/pk" ‖ u64be(p))` and `Rₚ = H("zkCoins/v1/test-vector/nflog/boundary/r" ‖ u64be(p))` (SHA-256; `u64be(p)` = 8-byte big-endian; domain tags distinct from the hand-listed smoke sequence so the two sets cannot be confused). Leaf hash `Hc("NfLog/Leaf", p ‖ Pkₚ ‖ Rₚ)` with `p` absorbed as the 8-byte big-endian input of [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log).
- **No full materialisation (normative).** The suite **MUST** be checkable **without materialising all leaves** of a size-`n` log: only the **O(log n)** boundary nodes on each inclusion path or consistency witness are required. Lazy / on-demand leaf and subtree evaluation is permitted and expected; a harness that allocates `Θ(n)` leaves for large `k` is **not** required.
- **Per boundary size `n` — inclusion (Accept).** When `n ≥ 1`, emit an inclusion vector `inclusion@(p,n)` (RFC-6962 `PATH(p, D[0:n])` recomputing `mth@n`) at each of the following **representative positions** that are valid (`p < n`), deduplicated:
  - **first:** `p = 0`;
  - **last:** `p = n − 1`;
  - **power-of-two boundary:** `p = 2^{⌊log₂(n − 1)⌋}` when `n ≥ 2` (the largest power of two strictly less than `n` — the RFC-6962 top-run split size for a run of length `n`).
  When `n = 0`, no inclusion vector is emitted.
- **Per `k` — consistency between adjacent boundary sizes (Accept).** For each `k ∈ {0,…,63}`, emit consistency vectors `consistency@(m,n) = PROOF(m, D[0:n])` recomputing both `mth@m` and `mth@n` for every adjacent pair among the in-scope sizes of that `k`:
  - `(m, n) = (2ᵏ−1, 2ᵏ)` when both sizes are in scope;
  - `(m, n) = (2ᵏ, 2ᵏ+1)` when both sizes are in scope.
  (The `m = 0` case is the trivial empty-log prefix witness of [§3.7](#37-the-nullifier-accumulator).)
- **Per boundary size — roots (Accept, `<REGEN>`).** For every in-scope boundary size `n`, `mth@n = MTH(D[0:n])` and `nav_root@n = Hc("NfLog/Root", n ‖ mth@n)` are `<REGEN>` outputs of the suite (structure fixed; bytes from the reference implementation).
- **Boundary-suite negatives (normative — each MUST be rejected).** For **every** in-scope boundary size `n ≥ 2` (and, where a pair is required, every adjacent consistency pair above), each of the following mutations **MUST** be rejected by both the independent RFC-6962 reference and the in-circuit gadget:

| # | Case | Expected |
|---|---|---|
| NL-B1 | **faulty split** — a consistency or inclusion witness whose top-run (or an interior-run) split uses a **wrong pivot** `k' ≠` the largest power of two strictly less than the run length (false tree decomposition) | **reject** (recomputed `mth` / prefix fails) |
| NL-B2 | **faulty peak-/subtree-bagging** — a consistency witness that reorders, drops, duplicates, or wrongly summarises the subtree roots that bag into `mth_a` / `mth_b` (wrong order or wrong combination of the O(log n) peaks) | **reject** |

**Negative controls — general (normative — each MUST be rejected).**
...
**Acceptance (V.11).** Pass iff: (i) every hand-listed positive vector matches once `<REGEN>` is filled; (ii) the generated log-boundary suite covers every in-scope `(k, n)` / inclusion position / adjacent consistency pair above and Accepts the honest witnesses; (iii) every NL-1–NL-7 and every NL-B1–NL-B2 case Rejects; (iv) node and SDK are byte-equal on filled `<REGEN>` paths per the V.7 parity matrix. This acceptance is the executable form of the [§1.7.8](#178-reference-instantiation-status-final-for-v1) freeze differential-test and the D-05 release gate.
