...
**Name loss survivability (accept).** A fourth fixture starts from an established contact — name resolved, kind-0 and kind-10050 verified, `op_pubkey` and payment identity pinned, endpoints retained — then makes that name permanently unresolvable: the NIP-05 response 404s and DNS for the domain fails. The consumer **MUST** keep the contact, its conversation, its pinned payment identity, and its retained transport; **MUST** continue to present that last confirmed name, since no newer profile has superseded it and an unreachable provider is not evidence of a change (`M-16b`); and **MUST** still complete NIP-17 in both directions and a payment against the pinned identity, with **zero** successful NIP-05 requests. It **MUST NOT** delete, merge, re-pin, or re-verify the contact.
...
**Name-consent framing vector (structure fixed; byte values `<REGEN>`).** The semantic fixture above passes even when signer and verifier share the *same wrong* encoding, so the byte layout is fixed separately here, with its concrete values to be generated rather than invented. The inputs are the [V.2-ext](#v2-address-derivation-sha-256--bech32m--pinned) chain (`sk₀`, `pk0`, `op_pubkey`), `network = "regtest"`, and the normalized name `alice@example.com`.
...
| M-16b | a previously known name stops resolving, with **no** newer profile superseding it | **not** a security event — the client keeps that last confirmed name in force and continues presenting it ([§4.3](#43-addressing-for-delivery)); an unreachable name provider is not evidence that a name changed. Messaging, payment, and retained transport continue against the pinned identity, and the client **MUST NOT** unpin, duplicate, delete, or re-verify the contact |
