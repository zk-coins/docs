...
- **Anchors MMR** — `AnchorsMmr/Leaf`, `AnchorsMmr/Node`, `AnchorsMmr/Root` (the append-only accumulator over admitted batches, §1.7.10); `AnchorsView` (the hiding commitment to an account's anchors-MMR root that a proof exposes publicly, §1.7.10).
...
`1798'` is the chosen BIP-43 purpose index for zkCoins (hardened). All branch separations are **hardened**: the VIEW, `op`, and `nk` branches are hardened children of `A`, so a party holding them **cannot** derive the SPEND branch.
...
**`anchors_rand` (normative).** `anchors_rand` **MUST** be a per-transition value that is unique and unpredictable to outsiders. A conforming wallet derives it deterministically so it survives loss and can reconstruct any prior opening for the clause-1 carry-forward: `anchors_rand = HKDF("zkCoins/v1/AnchorsRand", op_secret ‖ u64-be(send_counter))`, keyed by a seed-derived operational secret and a strictly-increasing `send_counter`. It **MUST NOT** be reused across transitions and **MUST NOT** be derived from `anchors_root` alone — two transitions committing the same `(anchors_root, anchors_rand)` pair produce identical `anchors_commitment` values and become publicly linkable. Because the valid-root set is enumerable (one per `n`), the commitment's hiding rests entirely on this fresh 256-bit `anchors_rand` under Poseidon modelled as a random oracle ([§1.7.1](#171-poseidon-instance-and-digest-encoding)); the same domain-separation argument as `detect_tag` vs `K_tx` ([§1.3](#13-per-coin-keys-note-encryption--detection)) applies.

**`anchors_root` selection (normative).** A transition **MUST** set `anchors_root` to the node's current anchors-MMR root — the tip over all `completed` batches at proving time — subject to the `extends`/prefix constraints below (clauses 1 and 10). Choosing the live tip, rather than the minimal root the constraints would permit, is what keeps the opening uninformative: the tip is a **global** value shared by every account transacting at that time, correlated only with the wall-clock instant (which the batch `block_anchor` already discloses, [§3.5](#35-inscription-format)), so an opening holder — recipient or publisher — learns only *when* the transaction was proved, never the account's private receive count or history. A wallet that instead pinned an older or account-specific root would leak its own recency through the opening; conforming wallets **MUST NOT**.
...
**Keys it holds.** For accounts that delegate to it, the node holds the **operational bundle** `{ivk, ovk, op, nk}` (Foundations §1.2): `ivk` to detect and decrypt incoming coins, `ovk` to recover outgoing-coin plaintext, `op` to act as the account's Nostr identity and to sign view grants and acknowledgements, and `nk` to derive nullifiers when building proving witnesses ([§2.1 clause 4](#21-the-compliance-predicate)). For a *foreign* account it holds only an `op`-signed **view grant**, never the bundle directly.
...
- **Own node.** The wallet entrusts its node with the full operational bundle `{ivk, ovk, op, nk}` (Foundations §1.2) over an authenticated, encrypted channel. The node can then receive, decrypt, discover, prove, and serve on the account's behalf 24/7. None of the bundle can spend.
