...
w = {
  ...
  npk_rand,                   // fresh secret per proving attempt; blinds npk_commit (§2.1 clause 2, §7.5);
...
Node / prover:
  2. build the witness with empty inputs, asset_issuance = {asset_id, creator_pubkey = Pk₀,
     ...
     Hc("NavCommit", nav_root ‖ nav_rand) (§1.4); determine the five ProofData fields and surface them
...
Node / prover:
  ...
  5. set nav to the accumulator value (§3.7) at the **minimal covering** log size — the smallest
     size that includes every dependency position (its previous account state's nullifier position
     and each input/received coin's creating-transition nullifier position) and is ≤ size_final, the
     ≥6-confirmation-final prefix (§3.9). All dependencies MUST already be final; otherwise no valid
     size exists and the node waits (§2.3.3, §3.9). The minimal covering size leaks only a coarse
     global ordinal, not the account's receive history, and minimises reorg-order fragility — and
     derive nav_rand; compute nav_commitment = Hc("NavCommit", nav_root ‖ nav_rand) (§1.4); determine
     the five ProofData fields
...
**Finality bound (hard project directive).** zkCoins v1 fixes finality at **6 confirmations**: once a nullifier's inclusion block has 6 confirmations, its position in the accumulator is treated as **final** and is never revisited. Define `size_final` as the number of first-occurrence log entries ([§1.7.6](#176-nullifier-accumulator-append-only-merkle-log)) whose inclusion block has **at least 6 confirmations** — i.e. the log prefix through block height `tip_height − 5` (from the [§5.6](#56-shareable-confirmation-links) confirmation count `tip_height − height + 1`, so 6 confirmations ⇔ height `≤ tip_height − 5`). A conditional NAV a transition commits ([§2.1 clause 1](#21-the-compliance-predicate)) and a balance attestation ([§5.7](#57-balance-attestation-history-private)) **MUST** authenticate only positions `< size_final`. Because the accumulator is an **ordered** Merkle log (order-sensitive, unlike the retired order-independent SMT, [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log)), this confines every authenticated position to the never-reshuffled final region: a ≤5-block reorg touches only positions `≥ size_final`, and a ≥6-block reorg displacing a final position is the accepted break above. The order-sensitivity is a **liveness** cost only (a not-yet-final nav may need rebuilding after a reshuffle), never a double-spend lever. This is a deliberate, load-bearing project directive, not merely a UX default:
...
- **`ProofData`** — `{new_account_state_hash, output_coins_root, input_nullifiers_root, coin_history_root, nav_commitment, npk_commit}`; the proof's public inputs (five 32-byte digests, 160-byte `serialize`). ([§1.4](#14-identifiers-and-hashes), [§2.1 clause 9](#21-the-compliance-predicate))
...
| `H(ProofData@0)` | `SHA-256(serialize(ProofData@0))` = `SHA-256(ash@0 ‖ ocr@0 ‖ inr@0 ‖ coin_history_root@0 ‖ nav_commitment@0 ‖ npk_commit@0)`, `npk_commit@0 = H("zkCoins/v1/NpkCommit" ‖ Pk₁_sample ‖ npk_rand@0)` (canonical 192-byte `serialize(ProofData)`, [§1.4](#14-identifiers-and-hashes)) | derived from the five above |
