...
| **11 · Standard identity and messaging** | §1.2 (identity keys seed-derived), §4.1/§4.3 (names are a naming layer above the protocol — plural, disposable, in no preimage; contacts keyed by `op_pubkey`; nodes operate without any name), §7.3/V.12 (mandatory NIP-17/kind-10050 and external-client interoperability), Lightning/mail bridge pages (independently optional) |
...
**Two layers, kept apart (normative).** The **protocol layer** resolves a recipient from an `addr_sig`-carrying object — an `Invoice`, or the recipient's signed profile below — and from that object alone. An email-style **name** (`user@domain`, resolved as NIP-05) is a **naming layer above** it: served by a node or any name provider, never derived from the seed, present in no signature preimage and no value-bearing structure. A conforming node **MUST** be able to receive, pay, message, and recover for accounts that have **no** name at all, and **MUST NOT** require one to serve any interface family of [§6.4](#64-external-interfaces-abstract). Hosting names for users stays the optional `aliasing` role of the API layer ([§6.1](#61-components-and-responsibilities)); a sovereign node without it hands out the raw `Invoice`/profile, or the user fronts their own domain.
...
An account **MAY** carry one or more email-style **names** `<user>@<domain>`, such as `alice@example.com`, each resolved as a standard NIP-05 identity. A name identifies the account's existing Nostr `op_pubkey`; it is not a zkCoins identifier, is never derived from the seed, enters no signature preimage, and does not imply that an SMTP mailbox or Lightning bridge exists.

**Names are plural and disposable (normative).** Any number of names, across any number of domains, **MAY** resolve to the same `op_pubkey`, and all of them are equally valid. A client **MUST NOT** treat one name as canonical, **MUST NOT** require that a name appear in the account's kind-0 `nip05` field before accepting it for discovery, and **MUST NOT** make possession of a name a condition for payment, messaging, or recovery. Adding, replacing, or losing a name changes no key, no address, and no pinned contact. Which name (if any) an end-user application shows is an `app`-layer matter ([Implementation Mandate](/implementation-mandate#app-layer-identity-and-contacts-normative)).
...
**One name, independent optional bridges.** NIP-17 messaging is mandatory; names are optional at the protocol layer and mandatory at the `app` layer ([Implementation Mandate](/implementation-mandate#app-layer-identity-and-contacts-normative)). A name may independently support either, both, or neither optional bridge:
...
| **11 · Standard identity and messaging** | Identity keys are seed-derived (§1.2); standard NIP-17/kind-10050 messaging on that key, interoperable with ordinary Nostr clients (§4.1, §7.3). Email-style names are a naming layer above the protocol — optional, plural, replaceable, absent from every signature preimage — and a node operates fully without one (§4.3). Lightning and SMTP bridges remain independent and optional. |
...
**Nameless account (accept) — the name-independence control.** A fourth positive fixture has **no** NIP-05 response, **no** `nip05` field, and **no** DNS available at all. The counterparty is supplied only as an `nprofile` (and, for payment, an `addr_sig`-verified `Invoice`). The consumer **MUST** create the contact record keyed by `op_pubkey`, complete NIP-17 in both directions using the kind-10050 event fetched from the `nprofile` hints, and complete a payment from the `Invoice` — with **zero** DNS and **zero** NIP-05 requests asserted throughout. A build that cannot pass this fixture does not satisfy [Requirement 11](/requirements).
