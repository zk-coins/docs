...
```
    Alice                 Nostr relay     Publisher       Bitcoin       Bob
      ...
      ├──────────────────────▶│              │              │           │
      │                       │              │              │           │
      │ 3. hand nullifier (Pkᵢ,Rᵢ,sᵢ,R') + fee ZBE blob ref to publisher│
...
| **10 · Node portability** | §1.2/§4.5–§4.6 (value-bearing wallet state is seed-/chain-derived or content-addressed, replicated, fetchable, and independently verifiable; none is tied to one node), §6 (switch / multi-node and explicit non-value portability residuals) |
| **11 · Standard identity and messaging** | §1.2 (identity keys seed-derived), §4.1/§4.3 (every wallet account has a name and a signed kind-0 payment object; the name is operational, not cryptographic — node-served, in no preimage, plural, replaceable, survivable on loss; contacts keyed by `op_pubkey`), §7.3/V.12 (mandatory NIP-17/kind-10050 and external-client interoperability), Lightning/mail bridge pages (independently optional) |
...
- After a valid ACK the sender **MAY** drop its retained copy. The sender **MUST NOT** drop the copy before a valid ACK **and** a valid [§4.6](#46-data-availability--replication-factor-k) `ReplicaReceiptV1` set confirming the replication target `k` (including the indefinite-holder hand-off where the sender's node was the only §4.6/§4.8 holder).
...
- **`output_refs[]`** lists every outgoing coin of the transition (recipient outputs, change, fee coin) as `{ coin_id, blob_id, epk, out_ciphertext, blob_locators }` so an `ovk` holder recovers outgoing plaintext **and** knows which replicas hold the coin blob ([§1.3](#13-per-coin-keys-note-encryption--detection), [§7.1](#71-serialization-conventions-normative) `serialize(BlobLocatorSet)`); `M = 0` for a pure receive.
...
1. **Phase A — build + publish nullifier.** Prove the transition against a `proof_block_anchor` (the member's own proof-context tip — a strict ancestor under the [§3.5](#35-inscription-format) gap bound; **not** necessarily equal to the publisher's eventual on-chain `AggregateStateNullifierV3` header `block_anchor`), assemble the nullifier hand-off (`(Pk, R, s, R')` and any fee coin), and publish / self-publish the nullifier ([§3.4](#34-the-publisher), [§7.6](#76-publisher-interface-normative)). Outgoing coin blobs **MAY** already be replicated; the SDR is **not** yet final-serialised.
2. **Phase B — observe first-occurrence, then finalise SDR.** After the node's own scan confirms the first-occurrence inclusion of `(Pk, R)` ([§3.6](#36-chain-scanning)), set `inclusion_block` to that inclusion block's `{block_hash, height}`, set `occurred_at` to **`MTP(inclusion_block)`** (BIP-113), leave `proof_block_anchor` as the proof-context tip from Phase A (a **strict ancestor** of `inclusion_block` per [§3.5](#35-inscription-format) — **not** the inclusion block, and **not** re-derived from the publisher's inscription header), complete every `output_ref` / `self_blob_locators` holder set, **then** compute `serialize(SelfDeliveryRecordV1)`, ZBE-encrypt, content-address, replicate to `k` holders, and self-deliver under [§4.2](#42-bundle-delivery) / [§4.6](#46-data-availability--replication-factor-k).

Replacing a finalised SDR after self-delivery (rewriting `occurred_at` or any other field) would change `ciphertext` / `blob_id` / receipts and **MUST NOT** be done; a node that has not yet observed first-occurrence **MUST NOT** emit a final SDR.
...
The account's node **MUST** deliver this self-addressed record to its **own** advertised relay set under the identical delivery rules above — encrypted to the account's own `IVPK`, carrying its own `detect_tag`, ACK-tracked where applicable, and replicated to `k` independent holders under [§4.6](#46-data-availability--replication-factor-k) (including `ReplicaReceiptV1` where required). Self-delivery is not optional bookkeeping; it is what makes two situations work, and **without it neither does**:
...
- **Emergency recovery.** Steps 5–6 of §4.5 rebuild the spendable head by verifying and replaying recovered `SelfDeliveryRecordV1` objects in `send_counter` order; they are retrievable only because they were self-delivered and replicated here.
...
**Two layers, kept apart (normative).** Every **wallet account** — an account whose seed a user holds through a wallet application, as opposed to one a node deployment operates for itself, such as a publisher's `fee_address` ([§3.8](#38-fees-and-economics)) — **MUST** have an email-style **name** (`user@domain`, resolved as NIP-05) and is reachable and payable by it. The obligation binds the wallet side of the system, realised by the API layer and the end-user app ([§6.1](#61-components-and-responsibilities)); a node deployment that holds no wallet account, such as a pure publisher or a validating-only node, carries no name of its own. But the name is **operational, not cryptographic**, and it lives **entirely above the kernel**: it is served and resolved by the **API layer** or by any external NIP-05 provider, never by the `zkcoins-node` kernel and never by the wallet's key material. It is not derived from the seed and is present in no signature preimage and no value-bearing structure. The kernel is handed an already-resolved `op_pubkey` or `nprofile` and **MUST NOT** perform NIP-05 or DNS resolution itself ([§6.1](#61-components-and-responsibilities) *Kernel and API — two boundaries*). The **protocol layer** underneath therefore resolves a recipient from an `addr_sig`-carrying object — an `Invoice`, or the recipient's signed profile below — and from that object alone; the name is what leads a sender *to* that object, never what authorises it.

Two rules follow, and the rest of this section implements them: a name **MUST** be replaceable, and an account **MAY** carry several at once, without any of it touching a key, a balance, or an established contact. Hosting names for users stays the optional `aliasing` role of the API layer ([§6.1](#61-components-and-responsibilities)) — the account's name may equally come from a self-hosted domain or any third-party NIP-05 provider, and a node **MUST NOT** require that it issued the name itself in order to serve the account.

The `address` above is the correct destination but is **not** what an end user sees: an end-user app presents receive identities, payees, and contacts **only** as names and never as a raw `zk1…`/`0x…` or bare `lnurl1…` string. That is an application requirement on the `app` layer ([Implementation Mandate](/implementation-mandate#app-layer-identity-and-contacts-normative)), not a protocol one — the distinction this section maintains throughout.
...
A wallet account **MUST** publish its delivery data as an additive `zkcoins` object inside the standard replaceable Nostr **kind-0 user-metadata event** authored by its `op` key ([§7.3](#73-nostr-event-kinds-normative)). This is what makes the account payable by its name without the payer first obtaining an `Invoice` — the ordinary way to be paid:
...
**What publishing it discloses (normative).** A kind-0 event is public, unencrypted, and — like any Nostr profile — indexed and archived by ordinary clients and aggregators. The object therefore ties the profile's name and public key to `address`, `pk0`, `nk_commit`, and `ivpk` **permanently and publicly**, and that cannot be retracted once seen. Being payable by name is not separable from this disclosure: it is the same fact published for the payer's benefit. A client **MUST** state the consequence when the account is created, and **MUST** offer separate accounts as the way to keep activity unlinked ([§4.7](#47-metadata-and-privacy-tradeoffs)) — that separation, not withholding the object, is the privacy control this design provides.
...
BootstrapManifestV1 = {
  ...
  operator_ids:    [op_pubkey, …],  // configured operator trust-list entries (§4.6)
...
Every wallet account **MUST** carry at least one email-style **name** `<user>@<domain>`, such as `alice@example.com`, resolved as a standard NIP-05 identity. A name identifies the account's existing Nostr `op_pubkey`; it is not a zkCoins identifier, is never derived from the seed, and enters no signature preimage. Having a name does not imply that an SMTP mailbox or Lightning bridge exists.

**Names are plural and replaceable (normative).** Any number of names, across any number of domains, **MAY** resolve to the same `op_pubkey`, and all of them are equally valid. This is a **capability, not a resilience mechanism**: additional names do nothing for a payer who knows only the name that stopped resolving, because nothing tells that payer another name exists. What plural names buy is that an account can be reached under more than one label at once — a migration served alongside the old name, an operator-hosted name beside a self-hosted one — and that switching between them costs nothing. A client **MUST NOT** treat one name as canonical, and **MUST NOT** require that a name appear in the account's kind-0 `nip05` field before accepting it for discovery. Adding or replacing a name changes no key, no address, and no pinned contact.

**What a name cannot do (normative).** Because the name is operational rather than cryptographic, a name that becomes unresolvable — an expired domain, a failed provider, a DNS outage — **MUST NOT** invalidate the account's keys, its `address`, its ability to spend, its retained conversations, or any pin a counterparty holds on it. Such an account is **unreachable under that name** until it resolves again or another name is published; it is not a broken account. Which name an end-user application shows is an `app`-layer matter ([Implementation Mandate](/implementation-mandate#app-layer-identity-and-contacts-normative)).
...
The two outcomes are **separate and differently scoped**, following the [NIP-05](https://nips.nostr.com/5) rule that the public key — not the name — is the primary reference:

- **Reverse check fails on a name not yet associated with a known key.** The client has no binding beyond the DNS answer, so it **MUST NOT** adopt the name or contact the key on its authority. This resolution fails closed, with no SMTP or NIP-04 fallback.
- **Reverse check fails for a name already associated with a pinned `op_pubkey`.** The name has become invalid, the key has not. The client **MUST** keep the contact, its conversation, and its pinned payment identity, **MUST** stop presenting that name for the contact, and **MUST NOT** delete, merge, or re-point the contact or treat messaging or payment as unavailable while retained transport still serves. If the well-known response now maps the name to a *different* key, that different key is a different contact and **MUST NOT** replace the pinned one.
- **No valid kind-10050 event.** The identity is **not messaging-ready**: NIP-17 fails closed, with no SMTP or NIP-04 fallback. Payment resolution is unaffected (*Payment readiness and messaging readiness are separate*, above).
...
**Known-contact storage and DNS-resistant resolution.** The retained contact record is **keyed by `op_pubkey`** — the seed-derived, permanent value — and never by a name. Names are an attribute of that record, and a record **MAY** carry several. The record therefore splits along the kernel/API seam ([§6.1](#61-components-and-responsibilities)): the **kernel** holds everything addressed by key — the key itself, the `nprofile`, the DM relays, the retained transport endpoints, and the payment-identity pin — while the **name attributes** and their reverse-check state belong to the layer that resolved them, the API layer or the app. After a successful first discovery the client **MUST** retain, across those two layers, at minimum:

- `op_pubkey` — the record key;
- every normalized name currently known to resolve to it, each with the outcome of its last reverse check;
...
**Trust and portability.** NIP-05 first discovery is a name-to-key binding; what is pinned on first successful use is the **key**, not the name. The `addr_sig` separately binds the discovered Nostr key and zkCoins delivery fields to the payment address through the three checks above. A name appears in no value-bearing structure and in no signature preimage: funds remain on `address`, and loss of a domain can impair first discovery through that name but cannot spend or destroy funds, invalidate a key, or unpin a contact. Publishing further names under other domains does **not** insure against that: a payer holding only the dead name has no way to learn a live one, so the honest statement is that first discovery through a lost domain is lost until the holder communicates a new name out of band. Domain loss does not break a known contact while at least one retained, original-hostname-authenticated TLS endpoint still completes the relay WebSocket upgrade on its original path; it does not guarantee availability beyond that explicit boundary.
...
There are **two** off-chain object classes carrying value or spendable state — the `CoinProof` bundle (per coin, value-bearing) and the `SelfDeliveryRecordV1` (per transition, fully replayable account head, [§4.2](#42-bundle-delivery)) — both protected by the replication discipline below. The nullifier accumulator is **not** an off-chain object: it is a pure function of the on-chain nullifiers ([§3.6](#36-chain-scanning)–[§3.7](#37-the-nullifier-accumulator)), rebuilt by every node from Bitcoin alone with no data-availability assumption at all.

#### CoinProof bundles and self-delivery records

A `CoinProof` bundle is custody of a delivered coin; a `SelfDeliveryRecordV1` is custody of the account's spendable head. If every holder drops either before the recipient (or a recovering owner) fetches it, the coin or the account head becomes unrecoverable.

#### Replication factor `k` (normative)

- Before a delivery is considered **complete**, the relevant blob (the encrypted `CoinProof` bundle or `SelfDeliveryRecordV1` blob **and** its delivery event) **MUST** be replicated to at least `k` **independent** holders. **"Independent"** in v1 means **distinct configured operator IDs on the sender's operator trust list** (the list mirrored by the [§4.3](#43-addressing-for-delivery) bootstrap manifest's `operator_ids` and the sender's local configuration) — **not** a property cryptographically derivable from pubkeys alone. `k` copies under one operator ID do not count.
...
- The recommended replica set is: the recipient's own node, the sender's own node (retained until the §4.2/§4.6 drop conditions are met), and at least one additional holder from the recipient's advertised set / the trust list — yielding `k = 3` from parties that each have an incentive to retain.

#### `ReplicaReceiptV1` — durable-storage attestation (normative)

HEAD reachability alone is not a durability proof. A holder that has durably committed **both** the blob **and** the corresponding delivery event **MUST** issue a signed **`ReplicaReceiptV1`** — and **MUST NOT** issue one earlier:

```
ReplicaReceiptV1 = {
  blob_id,                  // H(ciphertext) of the stored blob (§4.2.1)
  event_id,                 // id of the kind-1059 delivery event that references blob_id
  holder_op_pubkey,         // x-only op pubkey of the *storing* holder node
                            //   (not the uploader / replication-peer that signed the
                            //   upload authorization event; see §7.4)
  canonical_base_url,       // base URL under which the holder serves /blossom
  stored_at,                // unix seconds of the durable commit
  retention_class,          // "indefinite" | "policy"  (ASCII)
  retention_until,          // unix seconds; 0 when retention_class == "indefinite"
  attempt_nonce,            // 32B; sender-chosen per replication attempt (binds the receipt)
  receipt_sig               // BIP-340 under holder_op_pubkey over receipt_message
}

receipt_message = H("zkCoins/v1/ReplicaReceipt" ‖ network_tag
                  ‖ blob_id ‖ event_id ‖ holder_op_pubkey ‖ canonical_base_url
                  ‖ u64-be(stored_at) ‖ retention_class ‖ u64-be(retention_until)
                  ‖ attempt_nonce)
```

Preimage framing: `network_tag` is the ASCII network tag of [§1.4](#14-identifiers-and-hashes) / [§2.2](#22-proof-types) (`zkCoins/v1/mainnet` etc.); `canonical_base_url` and `retention_class` are `u32-be length ‖ UTF-8 bytes`; fixed-width fields at their §1.7.3 widths; order exactly as written. The signature therefore binds **domain + network + every canonically serialised field**. A receipt is valid only if (i) `receipt_sig` verifies under `holder_op_pubkey`, (ii) `holder_op_pubkey` is on the sender's configured operator trust list, (iii) `attempt_nonce` matches the sender's attempt, and (iv) the holder is expected to serve `blob_id` at `canonical_base_url`.

**Counting and drop (normative).** The sender counts only **distinct trust-list operator IDs** among valid receipts. When the sender's own node was the **only** §4.6/§4.8 holder of the artefact so far, the sender **MUST** collect at least **two** valid receipts with `retention_class = "indefinite"` from **other** trust-list operators before the hand-off is complete. A sender **MUST NOT** drop its retained copy until **both** a valid recipient ACK ([§4.2](#42-bundle-delivery)) **and** a valid receipt set meeting the `k` / indefinite-hand-off rules above are in hand — so one indefinitely-retained replica survives the drop and the emergency-recovery path (§4.5) never depends on a generic relay's retention policy alone.

**Privacy (MUST).** `ReplicaReceiptV1` and the receipt set travel **only** as a **direct upload response** to the sender (e.g. the body of a successful `PUT /blossom/upload`, [§7.4](#74-blossom-blob-store-normative)) or inside a **NIP-44-encrypted** channel back to the sender. They **MUST NOT** appear as a public Nostr event, cleartext tag, `DeliveryEvent` outer/payload field visible to the relay without unwrapping, or any gossipable holder record — otherwise a passive observer correlates `event_id ↔ blob_id ↔ holder`.
...
- **A published `zkcoins` profile is public and permanent.** The kind-0 event carrying `nip05` and the `zkcoins` object ([§7.3](#73-nostr-event-kinds-normative)) is not gift-wrapped: it is public and unencrypted, and — like any Nostr kind-0 — is indexed and archived by ordinary Nostr clients and aggregators, so it cannot be retracted once seen. Publishing it therefore binds that name and public key to `address`, `pk0`, `nk_commit`, and `ivpk` in a searchable public record. This disclosure is inherent in being payable by a public name, and the object is mandatory for a wallet account ([§4.3](#43-addressing-for-delivery)) precisely because that is the ordinary way to be paid. The control the design offers is **separate accounts**: activity a holder wants unlinked belongs to an account with its own name and its own keys. A directly issued `Invoice` discloses the same fields to one payer instead of to everyone, and remains available for a payment that should not go through a public profile.
...
- **Persist before acting (MUST).** The durable write **MUST** precede every externally-visible effect — returning the §4.2 ACK, issuing a §4.6 `ReplicaReceiptV1`, crediting a coin, or serving the artefact to a peer. A node **MUST** order its work so that a crash at any point can never leave it having acted on data it did not first persist.
- **The ACK is a durability receipt (recipient side); `ReplicaReceiptV1` is the replication durability proof.** A node **MUST NOT** return the §4.2 acknowledgement until the artefact is committed to stable storage (fsync / write-ahead log), and **MUST NOT** issue a `ReplicaReceiptV1` until **both** the blob **and** the delivery event are so committed ([§4.6](#46-data-availability--replication-factor-k)). A sender that receives a valid ACK **and** a valid §4.6 receipt set may therefore drop its retained copy (§4.2) knowing the data survived a crash on the holding side — the ACK means *durably stored by the recipient*, the receipt set means *durably replicated under the trust list*.
- **No expiry for value-bearing data.** Unlike a generic Nostr relay's retention policy, a zkCoins node **MUST** retain value-bearing artefacts **indefinitely** (`retention_class = "indefinite"` on any receipt it issues as a §4.8-bound holder). A node **MAY** prune an artefact only when it is provably **superseded** *and* still covered by a valid §4.6 receipt set of at least `k` independent trust-list operators — e.g. an older `SelfDeliveryRecordV1` once the newer state is durably stored and receipt-backed — and even then conservatively.
...
| Standard NIP-17 messaging on kind-10050 DM relays, addressed by key, and known-contact pinning ([§4.3](#43-addressing-for-delivery), [§7.3](#73-nostr-event-kinds-normative)) — name resolution is **not** kernel work | **Core** | always on |
...
| Mail bridge — SMTP interop for NIP-05 identifiers ([extension](/mail-bridge)) | application features, not core | **off** |

A few standard **deployment profiles** follow:

- **Sovereign personal node** — core (standard NIP-17 messaging included, as for every node) plus own-account proving; no public API, no publisher, and no operator-hosted NIP-05 service. The account's mandatory name ([§4.3](#43-addressing-for-delivery)) therefore comes from a self-hosted domain or any third-party NIP-05 provider rather than from this node. This is the private default.
...
- **Names are API-layer, never kernel.** An account's email-style name ([§4.3](#43-addressing-for-delivery)) is resolved and, when hosted, issued by the **API layer**; where no API container runs, the end-user app does the resolving. The kernel **MUST NOT** perform NIP-05 lookups, DNS resolution, or `/.well-known` fetches, and **MUST NOT** require a name to serve an account: everything it is asked to do is addressed by `op_pubkey`, `nprofile`, or an `addr_sig`-carrying object. This keeps the trustless core free of DNS and CA dependencies, and is why a publisher — a pure kernel deployment — needs no domain ([§3.8](#38-fees-and-economics)).
...
**Core surface vs optional roles.** The families above are the node **core** surface — every node serves them, for the accounts it is responsible for. This includes contact pinning and standard NIP-17 messaging addressed by key. Names appear nowhere in it: resolving and hosting them are API-layer work ([§6.1](#61-components-and-responsibilities)), and a node deployment holding no wallet account has no name of its own at all. The optional node roles ([§6.1](#61-components-and-responsibilities)) layer **on top** of the same surface rather than adding new wire protocols: the **public wallet API** is `read.account` + `submit.tx` (with proving) offered for **hosted** accounts; the **publisher** consumes already-submitted transitions to half-aggregate and inscribe their nullifiers ([§3.4](#34-the-publisher)); operator-hosted NIP-05 names use `aliasing`. These roles are advertised through the closed `roles` set on `GET /v1/info` / kernel `Info` (`{core, local_prover, hosted_wallet_api, publisher, aliasing}`, [§7.5](#75-node-rest-api-normative)). The [Lightning bridge](/lightning-bridge) and [mail bridge](/mail-bridge) remain independent services outside both the core and that advertisement: each has its own path signal, and each signal is stated to the same depth: **Lightning receive** — a kind-0 `lud16` equal to the profile's normalized `nip05`, resolving to a working LUD-16/LNURL-pay endpoint at that name's domain; **Lightning send** — explicit local bridge configuration **and** a successful quote; **SMTP** — explicit operator-side enablement **and** an explicit user choice to send by email. No bridge role, endpoint, or event is added to the node protocol here. A client that has not confirmed a bridge through its own signal above **MUST** treat that bridge as absent, the same fail-closed rule applied to the closed `roles` set.
...
| **11 · Standard identity and messaging** | Identity keys are seed-derived (§1.2); standard NIP-17/kind-10050 messaging on that key, interoperable with ordinary Nostr clients (§4.1, §7.3). Every wallet account has an email-style name and the signed payment object bound to it, but the name is operational rather than cryptographic — node-served, absent from every signature preimage, plural, replaceable, and survivable on loss (§4.3). Lightning and SMTP bridges remain independent and optional. |
...
A node deployment exposes the **four externally-visible planes** above ([§6.1](#61-components-and-responsibilities)): a bitcoind-backed scanner/inscriber, a Nostr relay, a Blossom store, and the public REST API. The REST plane is served by the **API layer** on top of the internal **kernel RPC** (§7.8), or by the node directly in a single-process deployment — the kernel RPC is never public. A wallet needs only one node's base URL; from `/v1/info` it learns the network's bootstrap manifest (seed relays, blob stores, operator IDs, [§4.3](#43-addressing-for-delivery)) plus that node's own relay and Blossom URLs.

### 7.3 Nostr event kinds (normative)

zkCoins uses the account's `op` key as its ordinary Nostr identity ([§4.1](#41-roles-and-transport)). Standard NIP-17 direct messages and kind-10050 DM relay lists are **mandatory** for human messaging; publishing a kind-0 profile, and carrying a NIP-05 name at all, are **optional** and belong to the naming layer above the protocol ([§4.3](#43-addressing-for-delivery)). Every conforming client **MUST** be able to consume all four when a counterparty does use them. The separate zkCoins payment transport remains metadata-minimising: its outer kind-1059 delivery events reveal only the per-coin `zkdt`/`zkepk` scan tags of [§4.2 step 4](#42-bundle-delivery), and its inner kinds are visible only after the recipient unwraps.

| Kind | Name | Class | Purpose |
|---|---|---|---|
| `0` | Nostr user metadata | replaceable | standard profile; for a **wallet account** both the `nip05` name and the additive `zkcoins` object are mandatory, `lud16` only when the Lightning bridge is enabled. An ordinary Nostr account that is not a zkCoins wallet carries neither |
...
| `30421` | zkCoins publisher profile | addressable | the self-contained `{fee_address, fee_asset_id, fee, relays, pk0, nk_commit, ivpk, addr_sig}` of [§3.8](#38-fees-and-economics), `op`-signed; `d` tag = hex `op_pubkey` |
...
*Name checklist* — applies **only** when the consumer reached the event by resolving a name, and answers whether that name may be attached to the key: the reverse `nip05` match plus, for messaging, kind-10050 readiness ([§4.3](#43-addressing-for-delivery) *First discovery*). A consumer that arrived by `op_pubkey` — from an addressable event, an `nprofile`, an `Invoice`, or an existing contact — has no name to check and **MUST NOT** be required to invent one.
...
**Publisher profile (kind 30421).** A replaceable addressable event a publisher publishes so wallets can discover and rate it ([§3.8](#38-fees-and-economics) step 1). `content` = JSON `{fee_address, fee_asset_id, fee, relays, pk0, nk_commit, ivpk, addr_sig}` — **eight closed fields**, no others: `fee` a decimal string, `fee_address` Bech32m, `pk0`/`nk_commit`/`ivpk` lowercase hex of exactly 32 bytes, `addr_sig` lowercase hex of exactly 64 bytes, `relays` a non-empty JSON array of URL strings, keys hex per §7.1. The event is signed by the publisher's `op` key, its `d` tag is the hex `op_pubkey`, and the event `pubkey` **MUST** equal that key — so authenticating the event binds the advertised `fee_address` to the operator.

The event is **self-contained by design**: it carries everything the three [§4.3](#43-addressing-for-delivery) checks need, so a spender verifies it without a kind-0 event, without a name, and without DNS. `addr_sig` is BIP-340 under `pk0` over the profile-fixed `invoice_message` (`amount = 0`, all-zero `asset_id`, empty `memo`, `recipient` = `fee_address`, the event author as `op_pubkey`, and this event's `relays`). A wallet **MUST** verify, in order: (i) `H(pk0 ‖ nk_commit) == fee_address`; (ii) `addr_sig` under `pk0` over that message; (iii) the event signature under the author `op_pubkey`; plus the field-shape requirements above. Only then does it encrypt a fee `CoinProof` to this object's `ivpk`. This keeps publishing **permissionless** ([§3.4](#34-the-publisher), [Requirement 7](/requirements)): running a publisher requires a key and a relay, never a domain, a name, or a CA. The publisher's Bitcoin identity is just the reveal-transaction key ([§3.4](#34-the-publisher)) — there is **no** on-chain publisher protocol key.

**Operator endpoint (kind 30422) and bootstrap manifest (kind 30423).** Replaceable addressable events carrying the signed [§4.3](#43-addressing-for-delivery) `OperatorEndpointV1` / `BootstrapManifestV1` JSON (keys and signatures hex per §7.1). Both are **intentionally public** and carry **only** global, account-independent infrastructure. They **MUST NOT** list account addresses, `blob_id`s, per-blob holders, or recovery identifiers. **`ReplicaReceiptV1` is not a Nostr event kind** — receipts travel only as direct upload responses or encrypted sender channels ([§4.6](#46-data-availability--replication-factor-k)).

### 7.4 Blossom blob store (normative)

Bundle blobs — ZBE-encrypted `CoinProof` or `SelfDeliveryRecordV1` blobs ([§4.2.1](#421-bundle-blob-encryption-zbe-normative), [§7.1](#71-serialization-conventions-normative)) — are stored and fetched by SHA-256 content address using **Blossom** (BUD-01/02). A node MUST expose, under its base URL, the path prefix `/blossom`:

| Method | Path | Purpose | Auth |
|---|---|---|---|
| `GET` | `/blossom/<sha256>` | fetch a blob by its lowercase-hex SHA-256 (`= blob_id`) | none (ciphertext is already encrypted) |
| `HEAD` | `/blossom/<sha256>` | existence / size probe (supplementary; durability is attested by `ReplicaReceiptV1`, §4.6) | none |
| `PUT` / `POST` | `/blossom/upload` | store a blob; the server computes `blob_id = H(body)` and returns it plus, when applicable, a `ReplicaReceiptV1` | authorization event (below) so only known peers fill storage |
...
| `X-ZkCoins-Event-Id` | lowercase-hex **32-byte** Nostr event id of the kind-1059 delivery event that references this blob (`event_id` of [§4.6](#46-data-availability--replication-factor-k)) | **yes** when a `ReplicaReceiptV1` is requested / will be issued; **MAY** be omitted only for pure cache uploads that never receive a receipt |
| `X-ZkCoins-Attempt-Nonce` | lowercase-hex **32-byte** sender-chosen `attempt_nonce` ([§4.6](#46-data-availability--replication-factor-k)) | same presence rule as `X-ZkCoins-Event-Id` |
| `X-ZkCoins-Retention` | closed enum string `indefinite` \| `policy` (the desired `retention_class` of the receipt) | same presence rule as `X-ZkCoins-Event-Id` |

A server **MUST** reject (`400`) an upload that carries a partial set of the three `X-ZkCoins-*` binding headers, a non-hex or wrong-width hex value, or an `X-ZkCoins-Retention` outside the closed enum. Multipart or JSON request bodies on `/blossom/upload` are **not** a conforming v1 form and **MUST** be rejected (`415` or `400`).

**Authorization event (normative).** `PUT`/`POST /blossom/upload` and `DELETE /blossom/<blob_id>` carry the header `Authorization: Nostr <base64(event JSON)>` with a Nostr event of **kind `24242`** signed by the account's `op` key ([§1.2](#12-key-hierarchy)):

- `kind`: `24242`; `pubkey`: the `op` x-only key (hex).
- tags: `["t", "upload"]` for PUT/POST, `["t", "delete"]` for DELETE; `["x", <lowercase-hex SHA-256 of the exact request body bytes>]` (PUT/POST — this equals the resulting `blob_id`) or `["x", <blob_id hex>]` (DELETE); `["expiration", <unix seconds, decimal string>]`.
...
The server **MUST** reject (`401`) an event whose signature, `kind`, `t` tag (mismatched method), or `x` tag (mismatching the body hash / target blob) fails; reject (`401`) an expired or future-dated event (`expiration` past, or `created_at` outside the window above — too far in the future beyond 60 s skew, or older than `replay_window`); reject (`403`) a PUT/POST whose `op` key is not the paired account's or a configured replication peer's ([§4.6](#46-data-availability--replication-factor-k)); reject (`403`) a DELETE whose `op` key is not the blob's original uploader; and reject (`413`) a body over the advertised size limit. A successful PUT/POST returns `200` with JSON body:

```
{
  "blob_id": <lowercase-hex 32B>,           // = H(body)
  "receipt"?: ReplicaReceiptV1Json          // present only under the dual-commit rule below
}
```

where `blob_id = H(body)` — the content address of [§4.2.1](#421-bundle-blob-encryption-zbe-normative).

**`ReplicaReceiptV1Json` (normative, closed schema).** When present, `receipt` is exactly:

```
{
  "blob_id":            <lowercase-hex 32B>,
  "event_id":           <lowercase-hex 32B>,
  "holder_op_pubkey":   <lowercase-hex 32B x-only>,
  "canonical_base_url": <string>,
  "stored_at":          <decimal-string u64 unix seconds>,
  "retention_class":    "indefinite" | "policy",
  "retention_until":    <decimal-string u64 unix seconds>,   // "0" when retention_class == "indefinite"
  "attempt_nonce":      <lowercase-hex 32B>,
  "receipt_sig":        <lowercase-hex 64B BIP-340>
}
```

All binary fields are **canonical lowercase hex** with no `0x` prefix; all time fields are **decimal strings** of the `u64` (never JSON numbers). **`ReplicaReceiptV1Json` is a closed schema:** a decoder **MUST** reject any object that has **extra** fields beyond this exact key set **or** is **missing** any of the nine required keys. Non-canonical values **MUST** also reject: non-lowercase hex, wrong hex width (not exactly 32 decoded bytes for the 32B fields / 64 decoded bytes for `receipt_sig`), an unknown `retention_class` (must be exactly one of the closed enum `{ "indefinite", "policy" }`), or a non-canonical `canonical_base_url` / `u64` string as specified below.

**Canonical `u64` string form (normative, fail-closed).** The fields `stored_at` and `retention_until` **MUST** be JSON strings matching the exact regular expression **`0|[1-9][0-9]*`** (decimal digits only; **no** leading zeros except the single digit `0`, **no** `+` prefix, **no** whitespace, **no** fractional part, **no** exponent). A string that fails this pattern, or that decodes outside the `u64` range `[0, 2^64−1]`, **MUST** be rejected.

**Canonical `canonical_base_url` (normative, fail-closed).** `canonical_base_url` **MUST** be an absolute `http`/`https` URL in the holder's **canonical advertised Blossom-base form** and **MUST** be **byte-equal** to the Blossom base this holder authentically advertises (the same canonical form used for holder advertisement / trust-list matching): lowercase scheme and host, **no** default port (`:80` for `http`, `:443` for `https`), **no** trailing slash except for the root path `/`, **no** userinfo, **no** fragment. A receipt whose `canonical_base_url` is not byte-equal to that authenticated advertised base **MUST** be rejected (even if it is a semantically equivalent URL under looser URL equivalence).

The **upload authorization** (kind-`24242` event under `Authorization`) is checked **separately** under the **uploader / replication-peer** `op` key of that event — that key authorises the write, but is **not** the receipt's holder identity. The server **MUST** set `receipt.blob_id = H(body)`, `receipt.event_id` / `receipt.attempt_nonce` / `receipt.retention_class` from the request headers above, `receipt.holder_op_pubkey` to **this holder's own local operator `op` pubkey** (the key of the node that durably stores the blob — **not** the uploader's authorization-event `op`), `receipt.canonical_base_url` to this holder's advertised Blossom base in that same canonical form, and `receipt.retention_until` per the retention rules (`"0"` when `retention_class == "indefinite"`); it **MUST** produce `receipt_sig` as a BIP-340 signature **by exactly that same holder `op` key** over the canonical `receipt_message` (domain + network + every serialised field) per [§4.6](#46-data-availability--replication-factor-k). Only a receipt signed under a trust-list operator's own key can count toward that operator's independent replica. The optional `receipt` **MUST** be present only when the holder has durably committed **both** this blob **and** the delivery event identified by `X-ZkCoins-Event-Id` / `X-ZkCoins-Attempt-Nonce`; if the dual-commit condition does not hold, the response is `200 { "blob_id" }` **without** `receipt`. A client **MUST** reject a `receipt` that fails the closed-schema / canonical-value checks above (including the `u64` string form and byte-equal `canonical_base_url` rules), whose fields disagree with the request headers, whose `blob_id ≠ H(body)`, whose `holder_op_pubkey` is not on the sender's operator trust list, or whose `receipt_sig` fails under `holder_op_pubkey`. Uploading bytes already present is idempotent and returns the same `blob_id` (and re-issues a receipt only after the durable dual-commit condition still holds). **`receipt` is a direct upload response to the uploader only** — it **MUST NOT** be published as a Nostr event, cleartext tag, or gossipable holder record ([§4.6](#46-data-availability--replication-factor-k) privacy). `GET /blossom/<blob_id>` returns the raw bytes and is unauthenticated (ciphertext is self-protecting, §4.2.1). A successful `DELETE` returns `200` with an empty body; a `DELETE` refused under the [§4.6](#46-data-availability--replication-factor-k) retention rules — the blob is still a required replica — returns `409` with `machine_code` `retention_hold`, and the blob **MUST** be retained.

`GET /blossom/<sha256>` MUST return the exact bytes whose SHA-256 equals `<sha256>` or `404`; a client MUST verify `H(body) == <sha256>` on receipt (content-addressed self-check) and reject a mismatch. Replication (§4.6) is performed by `PUT`-ing the same blob to ≥ `k` independent trust-list operators' `/blossom/upload` and collecting their `ReplicaReceiptV1` set. Account-specific holder hints (`BlobLocatorSet`) live only in the **encrypted** delivery / self-delivery interior ([§4.2](#42-bundle-delivery), [§4.3](#43-addressing-for-delivery)) — never as a public directory on the Blossom plane.
...
| `GET` | `/v1/info` | `{ network, protocol_version: "v1", circuit_digests: { C, C_balance }, bootstrap_pubkey: <hex32 x-only>, relay_url, blossom_url, max_blob_bytes, finality_confirmations: 6, activation_height: <u64>, max_tx_inputs: 8, max_tx_outputs: 8, max_rx_coins: 4, max_account_assets: 32, roles: [<closed role strings>], bootstrap: BootstrapManifestV1 }` — `network ∈ {mainnet, testnet, regtest}` is the sole network tag (v1 pins Bitcoin network 1:1 to this tag; there is **no** separate `bitcoin_network` field). `bootstrap_pubkey` is the network-parameter pin of [§3.6](#36-chain-scanning) / [§4.3](#43-addressing-for-delivery) under which `bootstrap.manifest_sig` **MUST** verify. `roles` is the closed advertisement set of [§6.4](#64-external-interfaces-abstract) / [§6.1](#61-components-and-responsibilities): each element ∈ `{core, local_prover, hosted_wallet_api, publisher, aliasing}`; unknown values **MUST** be ignored on read; a missing optional role means that role is **off**. `bootstrap` mirrors the per-network signed [§4.3](#43-addressing-for-delivery) manifest (seed relays, blob stores, operator IDs only; **no** account- or blob-specific fields) |
...
| `POST` | `/v1/grants` | body = `{ subject: <zk-address>, grantee_pk: <hex32>, scope: { asset_ids: [<hex32>] \| "*", not_before?: <u64>, not_after?: <u64> }, expiry: <u64>, challenge: { nonce: <hex32> }, ownership_proof: OwnershipProofJson }` → `{ grant: <Bech32m zkgrant string> }` — the node signs the [§5.2](#52-view-grant) grant with the account's `op` key. **Auth (normative):** a fresh action-bound **OwnershipProof** by `sk₀` over `chal = H(domain ‖ nonce ‖ chan_bind ‖ subject ‖ expiry ‖ request_hash)` with `domain = "zkCoins/v1/IssueGrantChallenge"` and `request_hash = H("zkCoins/v1/IssueGrant" ‖ subject ‖ grantee_pk ‖ asset_ids ‖ not_before ‖ not_after ‖ expiry)` — `asset_ids` / time fields encoded exactly as in `grant_message` ([§5.2](#52-view-grant); omitted `not_before`/`not_after` encode as `0` / `2⁶³−1`). A `GrantProof` or any non-owner capability → `401 unauthorized` (**no-escalation**: a view-grant delegate **MUST NOT** mint further grants). Missing/invalid/wrong-domain OwnershipProof → `401 unauthorized`; unknown/consumed/expired `nonce` → `410 challenge_expired`. The node **MUST** hold the subject's operational bundle (to use `op`), but bundle possession **never** replaces this request authentication |
...
TransitionRequest = {
  ...
  fee_address      : <zk-address>,                   // optional; presence matrix with publisher_pubkey below
...
**Publisher / fee-address presence matrix (normative, closed).** Exactly one of the following three cases holds; any other combination is **malformed** (`400 malformed_request`):
...
| **(b) Publisher + fee** | **present** | **present** | external publisher hand-off with fee: `fee_address` **MUST** equal the `recipient` of **exactly one** `output_template` — the publisher-fee coin ([§3.8](#38-fees-and-economics)); zero or more than one matching template is malformed |
| **(c) Fee-less hand-off** | **present** | **absent** | external publisher hand-off **without** a fee coin — admissible when that publisher's policy accepts fee-less hand-offs ([§7.6](#76-publisher-interface-normative); fee policy is not consensus, [§3.8](#38-fees-and-economics)) |
...
**Verification order (normative).** The publisher **MUST**, before accepting a well-formed body:
...
|---|---|---|---|
| `GetInfo` | unary | network, `protocol_version`, `circuit_digests`, `bootstrap_pubkey`, finality + bounds, `roles`, sync/`ready` (+ `ready_reason` when not ready) | `GET /v1/info`, `/health/ready` |
...
message Info {
  ...
  repeated string roles = 19;              // closed set (§7.5 /v1/info): each ∈
                                           //   {"core","local_prover","hosted_wallet_api","publisher","aliasing"};
                                           //   unknown values ignored on read; missing optional role = role off
...
message TransitionRequest {
  ...
  string fee_address = 7;                  // set iff case (b); empty for (a) and fee-less (c) (§7.5 matrix)
...
message PublishRequest {
  ...
  bytes fee_blob_id = 5;                    // 32B; empty ⇒ fee-less (§7.6). Was fee_coinproof (plaintext) — retired
...
| `FAILED_PRECONDITION` | wrong phase, stale S2C, retention hold, dependency not final, idempotency conflict, invalid signature at `/sign` | `wrong_phase`, `stale_message`, `invalid_signature`, `retention_hold`, `dependency_not_final`, `idempotency_conflict` |
...
- **`ReplicaReceiptV1`** — signed durability attestation `{blob_id, event_id, holder_op_pubkey, canonical_base_url, stored_at, retention_class, retention_until, attempt_nonce}` issued only after durable commit of blob **and** delivery event; counted by distinct trust-list operator IDs; private to the sender (direct upload response or encrypted channel — never a public Nostr event). ([§4.6](#46-data-availability--replication-factor-k), [§7.4](#74-blossom-blob-store-normative))
...
- **wallet account** — an account whose seed is held by an end user through a wallet application; the accounts the app and API layers exist to serve. Every wallet account has a mandatory NIP-05 **name** and a kind-0 `zkcoins` payment object ([§4.3](#43-addressing-for-delivery), [Requirement 11](/requirements)). Contrast an account a **node deployment** operates for its own working purposes — a publisher's `fee_address` ([§3.8](#38-fees-and-economics)) is the standing example: it receives coins, but no user holds it through a wallet, so it carries no name obligation and is discovered through its signed kind-30421 profile instead ([§7.3](#73-nostr-event-kinds-normative)).
...
| V.12 profile + NIP-17 wire vectors (incl. the name-loss and publisher-profile controls) | MUST produce/accept every positive fixture and reject every negative | MUST reproduce the profile checks and the wire construction within its scope | three-check profile rule + independent-seal construction; the live client matrix is an `app`-layer release check, not a wire vector |
...
| N-20 | Kernel returns `UNAUTHENTICATED` with `ErrorInfo.reason = "session_expired"` and `ErrorInfo.metadata.http_status = "410"` (e.g. on `GetCoinProof` / `GetAccountState` / `SubscribeReceipts`) | independent API layer **MUST** emit HTTP **410** with `{ "error": "session_expired", … }` — **never** 401 ([§7.8](#78-kernel-rpc--the-internal-interface-normative) error contract; 410-Sonderfall) |
...
| `coin_bytes` | `serialize(Coin)` for a fixture coin (112 B: `identifier ‖ recipient ‖ amount ‖ asset_id`) | `identifier` / `asset_id` Poseidon-dependent → `<REGEN>` (V.4); `recipient` / `amount` as chosen by the harness once `address` is regenerated |
| `coin_plain` | `UTF8("zkcoins-bin-v1:coin:" ‖ base64url_no_pad(coin_bytes))` — NIP-44 plaintext under conversation key `K_tx` | `<REGEN>` where `coin_bytes` is Poseidon-dependent |
...
**Name loss survivability (accept) — the operational-not-cryptographic control.** A fourth fixture starts from an established contact: name resolved, kind-0 and kind-10050 verified, `op_pubkey` and payment identity pinned, endpoints retained. The fixture then makes that name permanently unresolvable — the NIP-05 response 404s and DNS for the domain fails. The consumer **MUST** keep the contact, its conversation, its pinned payment identity, and its retained transport; **MUST** stop presenting the dead name; and **MUST** still complete NIP-17 in both directions and a payment against the pinned identity, asserting **zero** successful NIP-05 requests throughout. It **MUST NOT** delete, merge, re-pin, or re-verify the contact. A build that cannot pass this fixture treats the name as cryptographic and does not satisfy [Requirement 11](/requirements).

**Multiple names, one key (accept).** A fifth positive fixture serves two names on two different domains, both mapping to the same `op_pubkey`, while the account's kind-0 carries only one of them in `nip05`. Discovery through **either** name **MUST** reach the same single contact record, and the record **MUST** hold both names. A consumer **MUST NOT** create a second contact, and **MUST NOT** reject the name absent from `nip05` for payment or for messaging.

**Publisher profile (accept + reject).** A sixth fixture is a kind-30421 event with the eight closed content fields of [§7.3](#73-nostr-event-kinds-normative), `H(pk0 ‖ nk_commit) == fee_address`, valid `addr_sig` under `pk0` over the profile-fixed `invoice_message`, and a valid event signature whose author equals the `d` tag. A spender **MUST** accept it and encrypt a fee `CoinProof` to its `ivpk` **without** fetching any kind-0 event, resolving any name, or making any DNS request. Mutating any of the three checks, or omitting any of the eight fields, **MUST** reject the profile as unusable for a fee-bearing hand-off.
...
| M-16 | a verified kind-0 under an already-pinned `op_pubkey` carries a different `nip05` value, or a previously known name stops resolving | **not** a security event — record the name change, stop presenting an invalidated name, and leave the pinned key, payment identity, conversation, and retained transport untouched; **MUST NOT** unpin, duplicate, delete, or re-verify the contact ([§4.3](#43-addressing-for-delivery) *Name changes*) |
