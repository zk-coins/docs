...
- the last valid, three-check-verified kind-0 `zkcoins` object, when present — pinned alongside `op_pubkey`, not merely cached (*Payment-identity pinning* below).
...
**Payment negatives (P-01–P-07 each MUST disable payment for the profile; P-08–P-09 MUST block the silent change or use of a pinned payment identity).**
...
**Acceptance:** accept the positive profile; reject P-01–P-09 with the named scope; pass every live test in both directions with Primal, Amethyst, Damus, and 0xchat. A release fails V.12 if baseline one-to-one messaging fails with any client.
