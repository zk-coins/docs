...
3. The sender's app decodes the LNURL, calls the endpoint, and receives the recipient's `addr_sig`-signed `Invoice` (or profile tuple). It then **MUST** run the same three-check verification of §4.3 — `H(pk0 ‖ nk_commit) == recipient`, `addr_sig` under `pk0`, `sig` under `op_pubkey` — before encrypting anything.
