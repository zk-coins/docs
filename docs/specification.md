...
Node / prover:
  2. build the witness with empty inputs, asset_issuance = {asset_id, creator_pubkey = Pk₀,
     ...
     first mint on a fresh network (or, for a follow-up mint on an existing lineage, to `size_final` — the shared ≥6-confirmation-final prefix ([§2.3.2 step 5](#232-send), [§3.9](#39-finality-and-reorg-handling)) — which contains the transition's (final) dependencies); nav_commitment =
...
Node / prover:
  ...
  5. set nav to **`size_final`** — the ≥6-confirmation-final prefix (§3.9), the **shared** canonical
     ...
     the node **waits** for it to finalize (§2.3.3, §3.9). A wallet **MAY** instead **pipeline
     proving** — build and prove early against a still-pending dependency by choosing the minimal
     size that covers it — but **MUST NOT** hand the resulting nullifier to a publisher or broadcast
     it until that size `≤ size_final` (every covered position final); pipelining reveals an
     account-specific size (a privacy give-back) and is the wallet's explicit latency choice — and
...
2. **Nothing final is reversed by a tolerated reorg.** A receiver **MUST NOT** credit a coin until its creating nullifier is final (§3.10 `completed`). Because a ≤5-block reorg touches only **non-final** nullifiers, it cannot reverse a credited (final) coin or a final account state; a wallet simply re-publishes any orphaned not-yet-final transition after canonical replay. No final value is lost within the tolerated window. (A wallet MAY **build and prove** a transition against a still-`pending` dependency to pipeline proving latency (§6.3), but **MUST NOT broadcast** its nullifier until every covered position is final: publishing a nullifier whose `nav` covers a not-yet-final position risks a tolerated ≤5-block reorg reordering that position, which would strand the nullifier's `Pkᵢ` as the permanent first-occurrence winner while its `nav` is forever non-canonical — bricking the account. Publication is therefore finality-gated; only proving is pipelined.)
...
| `GET` | `/health/ready` | `{ ready, bitcoin_tip_height, nav_root, size, scanner_lag }` where `nav_root = Hc("NfLog/Root", size ‖ mth)` (§3.7) — the byte-unique committed accumulator value, not an ambiguous bare root; `503` until synced |
...
TransitionRequest = {
  ...
  nav_size?        : <u64>                           // optional; omitted ⇒ size_final. If set > size_final the node builds a PIPELINED proof and the wallet MUST delay broadcast until size ≤ size_final (§2.3.2 step 5)
}
...
message TransitionRequest {
  ...
  uint64 nav_size = 12;                    // 0 ⇒ size_final; >size_final ⇒ pipelined, broadcast-gated (§2.3.2 step 5)
}
...
| `inclusion@(p,n)` for `p < n`, `n ∈ {2ᵏ−1, 2ᵏ, 2ᵏ+1}` | the RFC-6962 audit path `PATH(p, D[0:n])` recomputing `mth@n`; the reference implementation MUST emit an inclusion vector for every `p ∈ {0, 1, k−1, k, n−1}` (clamped to `p < n`) at each listed `n`, so the set is a closed enumeration |
...
| NL-7 | a `nav` presented for **crediting** that authenticates a position `≥ size_final` (a not-yet-final entry) | **deferred** — not creditable until that position is ≥6-confirmation-final ([§3.9](#39-finality-and-reorg-handling)); canonical but not yet final, not a permanent rejection |
