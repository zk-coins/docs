...
**One name, independent optional bridges.** A name and NIP-17 messaging are both mandatory; what stays optional is each overlay. A name may independently support any combination of the following, or none of them:
...
- Marmot/MLS group chat: the API advertises `group_chat` ([Group chat](/group-chat)).

Disabling either bridge or `group_chat` **MUST NOT** disable NIP-05 identity, NIP-17 messaging, native zkCoins delivery, or the other overlays. A receive QR may encode the shared identifier; it never needs to expose a raw `zk1…` address or bare `lnurl1…` value.
...
| `group_chat` | Optional Marmot/MLS group overlay ([Group chat](/group-chat)); independent of NIP-17 and of both bridges | **off** |
...
- **Marmot group state is residual operational state.** When `group_chat` is on, MLS leaf secrets, HPKE init keys, epoch exporter secrets, and `group_event_key` material live only on the node that holds the operational bundle. They are **not** seed- or chain-derived. Switching nodes or recovering from seed + Bitcoin **does not** reconstruct group membership or history. Losing that state loses groups, never coins. [Requirement 12](/requirements#12-data-permanence) does not apply to Marmot application messages. A deployment that wants group continuity across a node switch **MUST** transfer the MLS client credential store as local operational state. The MLS group id **MUST NOT** appear on the public REST surface ([Group chat](/group-chat)).
...
**Group chat (gated on `group_chat` + `wallet`; [Group chat](/group-chat)).** Crypto stays node-side. Auth is a still-valid **ownership** pull session (`Authorization: Bearer <token>`); a GrantProof session **MUST** be rejected. No SPEND. A request when API `group_chat` is off, or when the kernel `group_chat` part is off, **MUST** be answered `404 feature_disabled`. `group_id` is a node-local stable id; the MLS group id **MUST NOT** appear on this surface.
...
- **MLS** — Messaging Layer Security, RFC 9420. The group crypto adopted via Marmot; leaf, HPKE, and epoch secrets are CSPRNG-generated node-held operational state and are never derived from `op` or the seed. ([Group chat](/group-chat))
...
| G-03 | deriving MLS secrets from `op` / seed / `op_secret` | reject the construction; secrets MUST be CSPRNG |
...
| G-09 | enabling API `group_chat` without the kernel `group_chat` part | every `/v1/groups*` request answers `404 feature_disabled` |
