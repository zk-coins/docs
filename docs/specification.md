...
- **A published `zkcoins` profile is public and permanent.** The kind-0 event carrying `nip05` and the `zkcoins` object ([§7.3](#73-nostr-event-kinds-normative)) is not gift-wrapped: it is public and unencrypted, and — like any Nostr kind-0 — is indexed and archived by ordinary Nostr clients and aggregators, so it cannot be retracted once seen. Publishing it therefore binds that name and public key to `address`, `pk0`, `nk_commit`, and `ivpk` in a searchable public record. This is what being payable by a public name consists of ([§4.3](#43-addressing-for-delivery)). A directly issued `Invoice` carries the same fields to one payer instead of to everyone.
