...
| `GET` | `/v1/chain/accumulator` | `{ size, root, tip_block_hash, tip_height }` — the current `NAV(tip)` as log pair `(size, mth)` with `root = nav_root = Hc("NfLog/Root", size ‖ mth)` (§3.7) ([§3.7](#37-the-nullifier-accumulator); NAV(tip) is `(size, mth)` now) |
...
| `inclusion@(p,n)` for `p < n`, `n ∈ {2ᵏ−1, 2ᵏ, 2ᵏ+1}` | the RFC-6962 audit path `PATH(p, D[0:n])` recomputing `mth@n`; the reference implementation MUST emit an inclusion vector at `p ∈ {0, 1, s−1, s, n−1}` (deduplicated, clamped to `0 ≤ p < n`), where `s` = the split point of `n` = the largest power of two `< n` ([§1.7.6](#176-nullifier-accumulator-append-only-merkle-log)), so the set is a closed enumeration |
...
| NL-7 | a `nav` presented for **crediting** that authenticates a position `≥ size_final` (a not-yet-final entry) | credit **rejected** — the position is `≥ size_final` (not yet final), so it MUST NOT be credited; it becomes creditable only once final (§3.9). (Deferral is the rejection of crediting, not of the proof.) |
