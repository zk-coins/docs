...
| M-16 | a verified kind-0 under an already-pinned `op_pubkey` carries a different `nip05` value, or a previously known name stops resolving | **not** a security event — record the name change **only** once the event's `name_sig` verifies under the pinned `pk0` over that new name (a profile failing that check keeps the stored name in force, [§4.3](#43-addressing-for-delivery) *Name changes*), stop presenting an invalidated name, and leave the pinned key, payment identity, conversation, and retained transport untouched; **MUST NOT** unpin, duplicate, delete, or re-verify the contact ([§4.3](#43-addressing-for-delivery) *Name changes*) |
