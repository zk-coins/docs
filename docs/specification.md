...
10. **Received-coin admission (receive path).** For every `received_coins[j]` (empty for a transition that receives nothing), the circuit **MUST** check, using `received_auth[j]`:
    ...
    b. **Coin binding.** `received_coins[j].identifier` is a member of `creating_proof.ProofData.output_coins_root` via `inclusion_proof` ([§1.7.5](#175-poseidon-merkle-tree-used-for-ocr-and-inr)), and `received_coins[j].recipient == prev_account_state.owner`;
