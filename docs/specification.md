...
| **11 · Standard identity and messaging** | §1.2 (identity keys seed-derived), §4.1/§4.3 (the app and API layers give every account they serve one name and a signed kind-0 payment object; the holder attests the name with `sk₀` via `name_sig`; the name is replaceable and survivable on loss; contacts keyed by `op_pubkey`), §7.3/V.12 (mandatory NIP-17/kind-10050 and external-client interoperability), Lightning/mail bridge pages and the [group-chat](/group-chat) overlay (independently optional) |
| **12 · Data Permanence** | §4.8 (never delete received data — store everything indefinitely and unconditionally; no drop, no expiry, no pruning, no supersession clean-up, except Marmot MLS application messages when `group_chat` is on — [Group chat](/group-chat)); §4.2/§4.5 (sender and every holder retain their copy forever; recovery therefore needs no central backup custodian); §7.4 (blob store is append-only — no `DELETE`) |
...
| `op` | the node | act as the standard Nostr identity, send/read NIP-17 messages, sign profiles, relay lists, view grants & acknowledgements; when `group_chat` is on, sign Marmot account-identity proofs and kind-30443 KeyPackage events | spend, decrypt others' coins, derive MLS secrets |
...
Human messages and zkCoins coin delivery are separate protocols on the same Nostr identity. One-to-one human messages **MUST** use NIP-17 kinds `14`, `13`, `1059`, and `10050`. When the API advertises `group_chat`, groups use Marmot/MLS kinds `444`, `445`, and `30443` ([Group chat](/group-chat)); they are not NIP-17 rooms. zkCoins delivery and acknowledgement rumors remain kinds `1420` and `1421` and **MUST NOT** carry chat content or group plaintext ([§7.3](#73-nostr-event-kinds-normative)).
...
- **Group-chat metadata (when `group_chat` is on).** Kind `445` exposes `nostr_group_id` (32 random bytes, not member-derived), a fresh ephemeral event pubkey, timing, and volume; an optional NIP-40 `expiration` further reveals that the event is an application message. Welcomes are NIP-59 gift wraps addressed to the invitee's `op_pubkey` and look like DMs to the invitee's inbox relays. KeyPackages (kind `30443`) are public and authored by `op_pubkey`. The MLS group id **MUST NOT** appear on the public REST surface ([Group chat](/group-chat)).
...
| **zkCoins API** (+ own PostgreSQL) | the sole public REST surface; features `wallet`, `explorer`, `publisher`, the two bridges, and `group_chat`, each switched on by the operator | `zk-coins/api` | present whenever a wallet or explorer is served; owns a **non-value-bearing** database |
...
| Marmot/MLS group-chat client — credentials, epoch state, kinds 444/445/30443 ([Group chat](/group-chat)) | optional | **off** — not a `/v1/info` feature; the API feature is separate. Enabling API `group_chat` while this kernel part is off **MUST** answer `404 feature_disabled` |
...
| `group_chat` | Optional Marmot/MLS group overlay ([Group chat](/group-chat)); independent of NIP-17 and of both bridges. The API **MUST NOT** advertise it unless `wallet` is also advertised | **off** |
...
flowchart TB
  ...
  subgraph apilayer["API layer — own repo/container · the only public surface"]
    api["public REST /v1/<br/>features: wallet · explorer<br/>publisher · bridges · group_chat"]
...
| `10050` | NIP-17 DM relay list | replaceable | recipient's preferred relays for NIP-17 messages; reused unchanged as the Marmot Welcome inbox when `group_chat` is on |
| `10002` | NIP-65 relay list | replaceable | write-capable relay set for KeyPackage fetch; required when `group_chat` is on; otherwise unused by this protocol |
| `444` | Marmot Welcome rumor | (rumor — unsigned, inside the seal) | MLS Welcome; **inert** unless `group_chat` is on ([Group chat](/group-chat)) |
| `445` | Marmot group message | regular | encrypted MLS group message; **inert** unless `group_chat` is on |
| `30443` | Marmot KeyPackage | addressable | public KeyPackage slot, authored by `op_pubkey`; **inert** unless `group_chat` is on |
...
message Info {
  ...
  repeated string kernel_parts = 19;       // which kernel parts this kernel runs: each ∈
                                           //   {"scanner","prover","publisher","group_chat"}.
                                           //   `group_chat` is off by default and is not a §7.5 /v1/info
                                           //   feature — the API feature is separate. Enabling API
                                           //   `group_chat` while this kernel part is off MUST answer
                                           //   `404 feature_disabled`. NOT the §7.5 /v1/info `features`
                                           //   array — that is API-layer configuration the API owns
                                           //   and constructs itself, and the kernel cannot know it.
...
- **`group_chat`** — optional API-layer feature, off by default, advertising the Marmot/MLS group overlay. Independent of NIP-17, of both bridges, and of payment conformance. ([Group chat](/group-chat), [§6.1](#61-components-and-responsibilities))
...
| V.13 group-chat / Marmot overlay | **only if** the layer advertises `group_chat`: MUST emit/accept the pinned Marmot wire and reject G-01–G-10 | MUST NOT implement MLS; calls `/v1/groups*` | White Noise interop is an `app`-layer release check when the feature is on; a release without `group_chat` skips V.13 |
...
This group tests the optional Marmot/MLS overlay ([Group chat](/group-chat)). It applies **only** when the layer advertises `group_chat` in `GET /v1/info`. A release that does not advertise `group_chat` **skips V.13 and still ships**. V.13 **MUST NOT** gate V.12, A-to-Z, or payment conformance.
