...
Receiver / node:
  ...
  2. RE-VERIFY THE FULL RECURSIVE PROOF: C.verify(proof) under the canonical verifier data.
     ...
     in-circuit prefix chain (clause 1, clause 10(c)) forces every dependency nullifier of every
     transition in the coin's past to be contained in this top-level nav, so a single canonical
     check validates all of them at once. A reorg that orphaned any dependency makes nav
     non-canonical and the transition a no-op (§3.9).
...
Because `Pkᵢ` is a fresh rotating key unlinkable to the account without the account's secrets ([Foundations §1.2](#12-key-hierarchy)), the published nullifiers reveal that *some* account transacted without revealing which account, coin, or amount ([Requirement 2](/requirements)). The whole-lineage receive predicate ([§2.1 clause 10](#21-the-compliance-predicate), [§2.3.3](#233-receive)) requires **every** nullifier-carrying transition in a coin's lineage to be anchored this way — which, because every spend publishes its nullifier on Bitcoin, is what makes an unanchored self-spend hop impossible.
