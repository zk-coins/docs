...
**Name changes (normative).** Pinning covers `op_pubkey` and the payment identity above. A verified kind-0 event under the pinned `op_pubkey` whose `nip05` differs from the stored name **MAY** be adopted silently once its `name_sig` verifies under the pinned `pk0`: the client replaces the stored name, leaving the pinned keys, the payment identity, the conversation, and retained transport as they are. A profile whose `name_sig` fails keeps the stored name in force. A name that now maps to a **different** public key belongs to a different contact, per the reverse-check outcome above.
