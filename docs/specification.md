...
| `GET` | `/v1/chain/accumulator` | `{ size, root, tip_block_hash, tip_height }` — the current `NAV(tip)` as log pair `(size, mth)` with `root = mth` ([§3.7](#37-the-nullifier-accumulator); NAV(tip) is `(size, mth)` now) |
...
message AccumulatorTip { bytes root = 1; bytes tip_block_hash = 2; uint64 tip_height = 3; uint64 size = 4; }
...
| `nav_commitment@0` | `Hc("NavCommit", nav_root(nav_empty) ‖ nav_rand_sample@0)` (a first-transition mint commits the empty conditional NAV) | `<REGEN>` |
