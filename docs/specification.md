...
Kinds `444`, `445`, `30443`, and `10002` are **inert** unless API `group_chat` is on. A node that does not advertise the feature **MUST** ignore them for group-chat processing. Wire shapes, custody, REST, and negative controls live on [Group chat](/group-chat). The one-to-one NIP-17 profile below is unchanged.
