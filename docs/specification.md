...
w = {
  ...
  received_auth[] = {         // per received coin, provenance evidence (clause 10)
    ...
    creating_nav_membership_path,                    // 256-sibling membership path of (Pk_create → leaf
                                                     //   R_create) in w.nav — the clause 10(d) check
  ...
  prev_state_nullifier = {    // the PREVIOUS transition's on-chain account-state nullifier (absent for InitialProof):
    ...
    nav_membership_path       //   256-sibling membership path of (Pk_prev → leaf R_prev) in w.nav
                              //   (the clause-1(i) check, same gadget as clause 10(d))
