...
**Network/chain separation (normative).** The verifier data of `C` is parameterised by a fixed network tag (`zkCoins/v1/mainnet`, `zkCoins/v1/testnet`, `zkCoins/v1/regtest`), so a proof valid against one network's verifier data is unsatisfiable against another's. This is the **closed** set of network tags; `/v1/info.network` and `kernel.v1 Info.network` advertise the short names `mainnet` | `testnet` | `regtest`, which map 1:1 to these tags. A conforming verifier MUST refuse a proof whose verifier data does not match the network it is operating on ([§1.7.9](#179-proof-system-parameters-normative)).
...
Receiver / node:
  ...
  2. RE-VERIFY THE FULL RECURSIVE PROOF: C.verify(proof) under the canonical verifier data.
     ...
     it non-canonical is the accepted break boundary (§3.9), not a no-op.
  ...
  4. anchoring: verify the creating transition's on-chain nullifier (Pk_create, R_create) from the
     ...
     creating transition has no on-chain Pk_create to be a first occurrence. The receiver's own node runs Path A (maintain the accumulator locally by scanning the marker inscriptions, §3.6) — that is the normal, self-contained case ([Requirement 4](/requirements)). Where the classification instead rests on Path B (a delegated SMT-path lookup, §3.7), a single foreign node's answer **MUST NOT** be the sole basis for crediting: the verifier **MUST** either obtain agreeing paths from multiple independent nodes (the [§6.3](#63-node-portability-and-multi-node-operation) fan-out; any discrepancy is **fail-closed** — treat the nullifier as not anchored until resolved against Path A) or fall back to Path A. (A mint coin is NOT a special case: the
...
| `MAX_NAV_DEPTH` | **48** | maximum **admission count** of the in-circuit conditional-NAV prefix/membership gadgets ([§2.1 clause 1, clause 10](#21-the-compliance-predicate)); `2⁴⁸` nullifier admissions is far beyond any realistic lifetime. **Orthogonal** to the nullifier-accumulator SMT's **256-bit key** length ([§1.7.6](#176-nullifier-accumulator-sparse-merkle-tree), [§3.7 *Prefix*](#37-the-nullifier-accumulator)): the SMT is keyed by the full 256-bit `Pkᵢ` (256-sibling membership paths) whatever `MAX_NAV_DEPTH` is; `MAX_NAV_DEPTH` bounds the prefix-chain admission logic, not the key length |
...
**`C_balance` shape (normative).** The balance-attestation circuit `C_balance` ([§2.2](#22-proof-types), [§5.7](#57-balance-attestation-history-private)) is dimensioned and pinned by the same discipline as `C`: its public inputs are exactly the §5.7 public values in declaration order — `subject` (32B), `asset_id` (32B), `balance` (`u128`, §2.6 limb encoding), `nav_ceiling` (32B), `anchor.txid` (32B, internal order), `anchor.block_hash` (32B, internal order), `anchor.height` (`u64`), `anchor.Pk_anchor` (32B), `anchor.R_anchor` (32B) — encoded per [§1.7.2](#172-field-encoding-e-of-hc-inputs); one verifying key per network tag; `circuit_digest(C_balance)` is a function of this layout and is pinned in [V.4](#v4-poseidon-derived-values--regen-table). Any change to this layout is a version bump ([§1.7.8](#178-reference-instantiation-status-final-for-v1)).
...
5. **Fold by first occurrence (first-spend-wins).** In that order, for each nullifier, insert the key `Pkⱼ` into the global nullifier accumulator ([Foundations §1.6](#16-trees-one-global-structure-one-per-account-structure)) with leaf value `Rⱼ` (its sign-to-contract commitment) **if `Pkⱼ` is not already present**. If `Pkⱼ` **is** already present, this nullifier is a **fork or double-spend attempt** — a second state-advancing transition on the same account state — and the scanner MUST treat it as **invalid**: its `Rⱼ` is not stored, and any transition or output that opens against it is treated as never anchored. The **first** on-chain occurrence of `Pkⱼ`, in this canonical order, is the one and only valid transition on that account state.
...
**Prefix — leaf-preserving monotone extension (normative).** The relation `prefix(a, b)` used by the conditional NAV ([§2.1 clause 1, clause 10c](#21-the-compliance-predicate), [§5.7](#57-balance-attestation-history-private)) holds **iff `b` is a monotone, leaf-preserving extension of `a`** over this 256-bit-keyed accumulator SMT: **every** `(key Pkⱼ ↦ leaf Rⱼ)` present in `a` is present in `b` with the **identical** leaf value `Rⱼ`, and `b` **MAY** contain additional keys — but a prefix extension **MUST NOT** alter, overwrite, or drop any leaf already in `a`. A leaf's value is therefore **invariant** along any prefix chain: membership-with-leaf `Pkⱼ ↦ Rⱼ` proven against `a` implies the **same** `Pkⱼ ↦ Rⱼ` in `b`, and — composed up a lineage's prefix chain to the top transition's `w.nav`, which a receiver checks **canonical** against its own `NAV(tip)` scan ([§2.3.3 step 2](#233-receive)) — implies the same leaf in the canonical accumulator. This leaf-preservation is **load-bearing** for the same-key fork closure at recursion-**internal** hops ([§2.1 clause 1](#21-the-compliance-predicate)): an internal hop's `w.nav` is a witness, **not** itself checked canonical, so its membership/leaf checks (clause 1 (i)/(ii)) are trustworthy **only** because `prefix()` lifts each leaf **unchanged** to the canonical top. Were `prefix()` to enforce mere key-**presence**, a malicious prover could witness a nested `w.nav` carrying a forged leaf `Pkᵢ ↦ R_loser` at a deep hop and pass (i)/(ii) against that forgery; leaf-preservation forbids it, because a forged `Pkᵢ ↦ R_loser` cannot be a leaf-preserving sub-map of a canonical accumulator whose first-occurrence leaf at `Pkᵢ` is `R_winner ≠ R_loser` — so the prefix, hence the proof, is unsatisfiable. A conforming implementation **MAY** realise `prefix()` with any gadget that decides exactly this leaf-preserving-extension relation; the *relation* is fixed, the *gadget* is not ([§2.6](#26-in-circuit-non-native-cryptography-normative)). **`MAX_NAV_DEPTH` vs SMT key length (orthogonal).** The accumulator's **key** length is **256 bits** (keyed by the full `Pkᵢ`, [§1.7.6](#176-nullifier-accumulator-sparse-merkle-tree)), so a membership/non-membership authentication is a **256-sibling** path regardless of `MAX_NAV_DEPTH`; `MAX_NAV_DEPTH = 48` ([§2.5](#25-circuit-dimensioning-normative)) is a **different, orthogonal** bound — it limits the conditional-NAV prefix/membership gadget's **admission count** (`2⁴⁸` admitted nullifiers, far beyond any realistic lifetime), **not** the 256-bit key length. The two never trade off: a prefix step is a leaf-preserving extension of the same 256-bit-keyed SMT and does not shorten the key.
...
- **Path B — delegate the lookup.** Hold nothing but ask any Path-A node for a self-verifying SMT path of `Pkᵢ` against the accumulator root the answering node reports. The path is 256 siblings, self-verifying under the §1.7.6 SMT construction; the answering node may serve either an **inclusion path** (proving `Pkᵢ` present with leaf `Rᵢ`) or a **non-inclusion path** (proving `Pkᵢ` empty). Delegation has a sharp edge a membership check lacks: a dishonest node can falsely answer *absent* for an already-spent state and so trick a receiver into accepting a double-spend. Because there is no on-chain root to check the path against (nullifiers are the on-chain data, not a root), for any **crediting** decision ([§2.3.3 step 4](#233-receive), [§3.10](#310-transaction-states) `completed`), a delegating verifier **MUST** query several independent nodes and require agreement — correctness holds as long as ≥1 is honest ([System Architecture §6.3](#63-node-portability-and-multi-node-operation)); a discrepancy is fail-closed (not anchored until resolved against Path A) — or fall back to Path A; a single foreign Path-B answer **MUST NOT** be the sole basis for crediting. For non-crediting display a single answer **MAY** be shown, marked unverified. A node MAY serve a **checkpoint accumulator root** to help a Path-A client cross-check its own scan; because that root is a deterministic function of the on-chain nullifiers, anyone who reconstructs the set recomputes and rejects a wrong one, so it carries no authority and the protocol inscribes none.
...
**Finality bound (hard project directive).** zkCoins v1 fixes finality at **6 confirmations**: once a nullifier's inclusion block has 6 confirmations, its position in the accumulator is treated as **final** and is never revisited. This is a deliberate, load-bearing project directive, not merely a UX default:
...
| `GET` | `/v1/chain/nullifier/<pubkey>` | a self-verifying SMT path for the account-state key `<pubkey> = Pkᵢ` against the current accumulator root (the Path-B service of [§3.7](#37-the-nullifier-accumulator)): `{ root, tip_height, present: bool, leaf?: <Rᵢ hex, when present>, siblings: [hex; 256] }`; `siblings` are ordered **bottom-to-top** (element `i` is the level-`i` sibling, [§1.7.6](#176-nullifier-accumulator-sparse-merkle-tree)), left/right per bit `i` of the key under the §1.7.6 bit rule, and the response **MUST** include `tip_block_hash` (32B, internal order) alongside `tip_height` |
...
message NullifierPath {
  ...
  bytes tip_block_hash = 6;                // 32B, internal order (§1.7.7)
}
...
- **`issuance_version`** — `u8` schema version under which an asset is created (`1` in this spec); bound into `asset_id` so coins minted under different versions are distinct. ([§1.4](#14-identifiers-and-hashes), [§6.5](#65-issuance--token-standards))
...
- **Path B (verifier path)** — a light-client verifier that holds no accumulator and asks any Path-A node for a self-verifying SMT path of `Pkᵢ` (present with leaf `Rᵢ`, or absent). Because there is no on-chain root to check the path against, for any **crediting** decision, agreeing answers from multiple independent nodes ([§6.3](#63-node-portability-and-multi-node-operation) fan-out) or fall back to Path A are **REQUIRED** — a single foreign Path-B answer **MUST NOT** be the sole basis for crediting, and a discrepancy is fail-closed ([§3.7](#37-the-nullifier-accumulator), [§2.3.3 step 4](#233-receive)); a single answer MAY back non-crediting display, marked unverified. ([§3.7](#37-the-nullifier-accumulator))
...
signature    (64B): <REGEN — BIP-340(skᵢ, m_state) with S2C tweak t = H(bytes(R') ‖ H(ProofData))>,
...
3. Substitute the regenerated values into V.3 (`asset_id`, `coin_history_root@0` — never `E'₂₅₆`) and V.4's `H(ProofData@0)` (which the V.5 signature commits via sign-to-contract); `ash@0` is built from that substituted V.3 byte string and therefore uses `coin_history_root@0`, never the empty-tree constant `E'₂₅₆`.
...
| V.4 Poseidon values (`E₂₅₆`, `nk_commit`, `asset_id`, `ash`, `coin.identifier`, `nf`, roots, …) | MUST produce | MUST reproduce | **byte-equal** (SDK implements the §1.7.1 Poseidon primitive) |
