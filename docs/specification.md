...
- **Payment-identity pins are node-local and not seed-derivable.** The [§4.3](#43-addressing-for-delivery) payment-identity pin is node-local and is **not** seed-derivable. A node switch or an emergency recovery under [§4.5](#45-recovery) leaves the new or rebuilt component with no pins. A client that has lost its pins **MUST NOT** silently re-pin a previously known contact as if it were a first contact; the full obligation is stated under [§4.3](#43-addressing-for-delivery) *Payment-identity pinning*.
