...
- **Anchors MMR** — `AnchorsMmr/Leaf`, `AnchorsMmr/Node`, `AnchorsMmr/Root` (the append-only accumulator over admitted batches, §1.7.10).
...
| **ProofData** (public inputs) | `{ new_account_state_hash, output_coins_root, input_nullifiers_root, coin_history_root, anchors_root }` — there is **no** live accumulator root here: a spender's per-account proof attests local soundness; global double-spend is enforced by the publisher's `AggregateBatchProof` ([§2.2](#22-proof-types)) when it inserts the batch's nullifiers into the on-chain-committed accumulator. The fifth field, **`anchors_root`** (`anr`), is the account's **anchors-MMR root** ([§1.7.10](#1710-anchors-mmr-normative)): it commits to the set of admitted batches against which the account's receive checks ([§2.1 clause 10](#21-the-compliance-predicate)) were performed, is carried forward **monotonically** by every transition (clause 1), and **MUST** be checked against the verifier's own chain scan whenever a per-account proof is verified directly ([§2.3.3](#233-receive) step 2). **Canonical serialization** `serialize(ProofData) := new_account_state_hash ‖ output_coins_root ‖ input_nullifiers_root ‖ coin_history_root ‖ anchors_root` (the five 32-byte digests in that exact order — **160 bytes**), and **`H(ProofData) := SHA-256(serialize(ProofData))`** is the single normative definition the **spender's** sign-to-contract tweak uses everywhere ([§2.1 clause 9](#21-the-compliance-predicate), [§2.2 clause 2](#22-proof-types), [§3.3](#33-off-chain-signature-handling), [§5.7](#57-balance-attestation-history-private), V.4). (The **publisher's** separate `BatchInscription` S2C binds `H_agg` over the proof bytes, not `H(ProofData)`, [§3.2](#32-batchinscription-signing-bip-340--sign-to-contract).) This digest order (`ash ‖ ocr ‖ inr ‖ chr ‖ anr`) is deliberately **not** the `SpendRecord` `message = inr ‖ ocr` order — they are distinct objects | hashes/roots only |
...
- **Root ("bagging"):** for `n` leaves with peaks `P₁ … P_p` (left-to-right, tallest first), `anchors_root = Hc("AnchorsMmr/Root", n ‖ P₁ ‖ … ‖ P_p)` with `n` a `u64` (wide-numeric encoding, [§1.7.2](#172-field-encoding-e-of-hc-inputs)). The **empty root** (`n = 0`) is `anr_empty = Hc("AnchorsMmr/Root", 0)` — the initial `anchors_root` of every account ([§2.2](#22-proof-types) canonical empty account).
...
- **Extension proof (`extends(anr, anr')`):** attests that the MMR with root `anr'` (at `n'` leaves) is an **append-only extension** of the MMR with root `anr` (at `n ≤ n'` leaves) — the standard MMR consistency proof: the old peaks are recomputed from witnessed subtrees of the new forest, and both roots are recomputed via the bagging rule. `extends(anr, anr)` holds trivially (empty witness). This is the relation clause 1 of the compliance predicate uses to carry `anchors_root` forward monotonically — an account can only ever **extend** its anchors view, never replace it, so a fabricated (non-admitted) anchor leaf poisons every later `anchors_root` of that lineage and is caught the next time any verifier checks the exposed root against its own scan ([§2.3.3 step 2](#233-receive)).

A node **MUST** maintain the anchors MMR alongside the nullifier accumulator, index the historical `anchors_root` value after every append, and answer two queries for its wallets: a membership path for a given admitted `member_root`, and the validity of a presented `anchors_root` (i.e. whether it equals the MMR root after some prefix of admissions — a lookup in the per-append root index). Reorg handling follows [§3.7](#37-the-nullifier-accumulator): because only `completed` batches are appended, a reorg deeper than the [§3.9](#39-finality) bound is a protocol-failure event, not an MMR rewind.

**Trust note (normative).** Nothing on Bitcoin commits the anchors MMR directly, so an `anchors_root`-validity answer is **not** self-verifying the way a [§3.7](#37-the-nullifier-accumulator) Path-B nullifier path is: it is exactly as trustworthy as the node that derived it from the chain and the bundles. A Path-A verifier (any full node, including every wallet's own node — the [§6.2](#62-wallet--node) default) performs the check trustlessly; a pure light client inherits the [§6.3](#63-node-portability-and-multi-node-operation) multi-node/honest-node spectrum for this one check. A future version **MAY** commit the anchors root inside the `BatchInscription` to close this gap for light clients.
...
w = {
  ...
  received_auth[] = {         // per received coin, provenance evidence (clause 10)
    ...
    anchor_path                                      // anchors-MMR membership path of that member_root (§1.7.10; absent for a mint)
  },
  anchors_extension,          // MMR extension witness prev.anchors_root ⇝ new anchors_root (§1.7.10; empty when unchanged)
...
1. **Recursive verification (PCD) and anchors carry-forward.** Either this is an `InitialProof` and `w.prev_proof` is absent and `w.prev_account_state` is the canonical empty account for `owner = H(Pk₀)` and `ProofData.anchors_root` satisfies `extends(anr_empty, anchors_root)` ([§1.7.10](#1710-anchors-mmr-normative)); **or** `w.prev_proof` verifies under the circuit's own verifier data (cyclic recursion), its public output `new_account_state_hash` equals the `ash` of `w.prev_account_state`, its `coin_history_root` equals the coin-history root over which clause 2 proves inclusion, **and** `ProofData.anchors_root` satisfies `extends(prev_proof.anchors_root, anchors_root)` via the witnessed `anchors_extension` — the account's anchors view is carried forward **monotonically** and can never be replaced or rewound, so a fabricated anchor leaf poisons every later `anchors_root` of the lineage (caught by the [§2.3.3 step 2](#233-receive) check). The verifier data **MUST** be fixed and identical in prover and verifier; a proof verified against any other verifier data is invalid.
...
9. **Public-input binding.** All five `ProofData` fields — `new_account_state_hash`, `output_coins_root`, `input_nullifiers_root`, `coin_history_root`, `anchors_root` — **MUST** be the in-circuit-computed values above and are the proof's public inputs. Nothing else is public: amounts, asset ids, recipients, keys, and counts remain in the witness (zero-knowledge).
...
Node / prover:
  ...
  6. run C as an AccountUpdateProof: recursive verify of prev_proof + anchors carry-forward
...
CoinProof produced (per recipient coin, delivered off-chain):
  { coin, proof = π, inclusion_proof (membership in ocr), creating_prev_ash (= the ash of
    ...
    known only after publisher hand-off; MAY follow the initial delivery), epk, ciphertext,
    detect_tag }
...
Receiver / node:
  ...
  2. RE-VERIFY THE FULL RECURSIVE PROOF: C.verify(proof) under the canonical verifier data.
     ...
     ADDITIONALLY: proof.ProofData.anchors_root MUST be a valid historical anchors-MMR root
     per the receiver's OWN scan (§1.7.10). This one check is what makes the provenance's
     receive history trustworthy: every receive folded anywhere into the coin's past bound
     its creating batch into the (monotonically carried-forward) anchors_root, so a single
     lookup against the verifier's own chain view validates all of them at once.
...
On all of 2–6 passing:
  7. RUN THE RECEIVE TRANSITION: execute C as an AccountUpdateProof whose received_coins[]
     contains the verified coin (clause 10) — the node fetches the witness material from the
     anchoring batch (creating SpendRecord + member path, via anchor_hint or the §5.5
     anchoring trail) and its own anchors MMR (§1.7.10), extends anchors_root as needed
     (clause 1), credits balances (clause 7), and admits the coin to the coin-history SMT
     (clause 8). Several verified receipts MAY be folded into one transition (bounded by
     MAX_RX_COINS, §2.5), and a receive MAY be combined with a send in a single transition.
     The wallet signs the transition signature exactly as for a send (§7.5 proving handshake);
     no SpendRecord is handed to any publisher (nothing to anchor — a pure receive spends
     nothing and its nullifier list is empty).
...
MUST retain the incoming CoinProof bundle per §4.8 and MAY return an encrypted acknowledgement
so the sender can drop its copy (Transport & Recovery).
```
...
| Received-coin admission (10) + anchors carry-forward (1) | **No fabricated receipts** — a coin enters an account's provable holdings only with its creating proof verified in-circuit and (for non-mints) its creating `SpendRecord` bound, via the anchors MMR, to an admitted `completed` batch; because `anchors_root` can only be extended, never reset, a colluding chain of holders cannot smuggle an un-anchored spend into any downstream lineage — the single `anchors_root` check of §2.3.3 step 2 exposes it | 3 |
...
The control event (step 3) and the acknowledgement are small and use **NIP-44 v2** directly. The **`CoinProof` bundle blob** (typically ~100 KB) exceeds NIP-44 v2's 65 535-byte plaintext limit, so it is encrypted with **zkCoins Bundle Encryption (ZBE)** — a chunked AEAD framing over **ChaCha20-Poly1305** (the same AEAD NIP-44 v2 is built on, so no new cryptographic assumption). ZBE is its own on-wire format, **not** a sequence of NIP-44 v2 messages. ZBE applies **only** to the value-bearing, recipient-encrypted `CoinProof` bundles: a **`BatchBundle`** is a public, self-authenticating object that every scanner must read, so it is stored as a **plaintext** blob, content-addressed by `blob_id = H(serialize(BatchBundle))` ([§1.4](#14-identifiers-and-hashes), [§7.4](#74-blossom-blob-store-normative)) — encrypting it would serve no one.
...
BalanceAttestation:
  public inputs (revealed):
    { subject : address,
      asset_id,
      balance : B,
      anchors_root,                                  // the attested state's anchors-MMR root
                                                     //   (§1.7.10); verifier checks it against
                                                     //   its own scan
  ...
  statement (domain tag "zkCoins/v1/BalanceProof"):
    ...
    7. pi.ProofData.anchors_root == anchors_root    (exposes the attested state's anchors view)
```

The verifier checks the proof, that `anchors_root` is a valid historical anchors-MMR root per its **own** scan ([§1.7.10](#1710-anchors-mmr-normative) — this validates every receive folded into the attested state), and that the `BatchInscription` at `anchor` (`txid`) — which contains the `bundle_locator` whose `BatchBundle` carries `SpendRecordⱼ` at position `j` — is in state `completed` ([On-chain §3.10](#310-transaction-states)) at `{block_hash, height}`. No node, relay, or explorer is trusted. Because the anchor is a `SpendRecord`, the attestation binds the balance **as of the account's most recent anchored spend**; balance changes made by later receive transitions (which produce no `SpendRecord`) are attested only once the account's next anchored spend binds the newer state.
...
**What it cannot do.** A node **MUST NOT** be able to spend, forge, or double-spend: it never holds any SPEND-branch key (the rotating `skᵢ`), and value integrity is enforced by proof soundness and the nullifier accumulator, not by the node's honesty. `nk` (held for the node's own accounts only) enables nullifier derivation and therefore linkage of *that account's own* spends — a privacy consideration internal to the operator, never spend authority. A node **MAY** lie or withhold data, but it cannot make the wallet accept an unverifiable answer (§6.3).
...
| `GetAnchors` | unary | current anchors-MMR root + size; validity lookup for a presented root ([§1.7.10](#1710-anchors-mmr-normative)) | `GET /v1/chain/anchors`, `GET /v1/chain/anchors/<root>` |
...
- **`anchors_root` (`anr`)** — the fifth `ProofData` field: the account's anchors-MMR root, committing to the admitted batches its receive checks were performed against; carried forward monotonically (clause 1) and validated against the verifier's own scan on every direct proof verification. ([§1.7.10](#1710-anchors-mmr-normative), [§2.1](#21-the-compliance-predicate), [§2.3.3](#233-receive))
...
- **`ProofData`** — `{new_account_state_hash, output_coins_root, input_nullifiers_root, coin_history_root, anchors_root}`; the proof's public inputs. ([§1.4](#14-identifiers-and-hashes), [§2.1 clause 9](#21-the-compliance-predicate))
...
- **`serialize(ProofData)` / `H(ProofData)`** — canonical `new_account_state_hash ‖ output_coins_root ‖ input_nullifiers_root ‖ coin_history_root ‖ anchors_root` (160 bytes); `H(ProofData) = SHA-256(serialize(ProofData))` is the spender's sign-to-contract tweak digest. ([§1.4](#14-identifiers-and-hashes), [§3.3](#33-off-chain-signature-handling))
...
| `H(ProofData@0)` | `SHA-256(serialize(ProofData@0))` = `SHA-256(ash@0 ‖ ocr@0 ‖ inr@0 ‖ coin_history_root@0 ‖ anr_empty)` (canonical 160-byte `serialize(ProofData)`, [§1.4](#14-identifiers-and-hashes)) | derived from the five above |
