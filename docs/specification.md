...
| N-31 | Recovery full scan under a same-second **tie flood**: more than `L` kind-`1059` events share one identical `created_at = t`, and a target `SelfDeliveryRecordV1` delivery sits among them beyond the first page of `limit = L` | Recovery **MUST** still discover and process that target (no skip) — [§4.5](#45-recovery) step 3 gapless scan (limit-free drain at `since = t, until = t` before `until = t − 1`) |
