...
**Readiness enforcement (normative).** A kernel **MUST** return `UNAVAILABLE` / `network_not_ready` / `503` from every **consensus-dependent** procedure — the ones carrying that triple below — whenever it is not ready to serve consensus-dependent reads and credits (a `/health/ready` `reason ∈ {syncing, scanner_lag, deep_reorg, circuit_mismatch, dependency_unavailable}`, [§7.5 *Multi-network projection*](#75-node-rest-api-normative)). This makes the readiness gate **kernel-authoritative**: an API-layer pre-dispatch check is an optional optimisation, but the kernel's per-RPC enforcement is what closes the ready→not-ready race for a stale-state read or a credit-affecting mutation. The API maps this triple to the REST `503 network_not_ready` body; the `404 network_unavailable` case is **API-only** (an unserved `{network}` never reaches a kernel) and is therefore **not** a kernel `ErrorInfo.reason`.
...
|---|---|---|---|---|---|---|---|---|
| `GetInfo` | — | — | — | — | — | — | `network_not_ready`/`503` (when used for `/health/ready`) | `internal_error`/`500` |
