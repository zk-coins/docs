...
zkCoins lets you send value on Bitcoin without anyone seeing the amount, the asset, who paid, or who received. Bitcoin stores only **opaque markers** that spends happened — *not* the coin's contents, which travel privately between sender and receiver as a small encrypted bundle. **Double-spend protection** is the chain's job: every spent coin reveals a one-time random-looking *nullifier* that can enter the Bitcoin-anchored global nullifier set only once — a publisher anchors a whole batch of spends with a single constant-size inscription, and any second spend of the same coin is rejected. Your **seed phrase** derives every key, your **wallet** is the only thing that can spend, **any node** can serve you, and you check every figure against Bitcoin yourself.
:::
...
```
    Alice                 Nostr relay     Publisher       Bitcoin       Bob
      ...
      │                       │              check nf non-membership    │
      │                       │              of accumulator at tip      │
      │                       │              │              │           │
      │                       │           10. receive transition folds  │
      │                       │               the coin in (trustless)   │
      │                       │              │              │           │
      │11. encrypted ACK · A may drop her copy once k replicas confirmed│
      ◀──────────────────────────────────────────────────────────────────
      │                       │              │              │           │
```
...
| **2 · Private** | §1.3 (per-coin encryption), §1.4 (opaque `BatchInscription` carries only roots, a publisher key, a locator hash, a block anchor, and a signature — no individual nullifiers, amounts, parties, or per-spender keys on chain), §2 (ZK proof hides amounts/parties/graph) |
...
- **Per-transition commitments** — `NextPubkeyCommit` (hiding commitment to the rotated spend key, §1.4/§2.1 clause 2), `OutIntentCommit` (hiding commitment to the validated output intent, §1.4/§2.1 clause 2). Both are `H` (SHA-256) tags, wallet-computable with no Poseidon.
...
- **Anchors MMR** — `AnchorsMmr/Leaf`, `AnchorsMmr/Node`, `AnchorsMmr/Root` (the append-only accumulator over admitted batches, §1.7.10); `AnchorsView` (the hiding commitment to an account's anchors-MMR root that a proof exposes publicly, §1.7.10); `AnchorsRand` (the HKDF context for the per-transition `anchors_rand` derivation, §1.7.10).
...
- **Batching and transport** — `BatchBundle` (the `bundle_locator` on-chain binding value, [§1.4](#14-identifiers-and-hashes)), `BatchMember`, `BatchMember/Node` (the batch `member_root` hash tree, [§2.5](#25-circuit-dimensioning-normative)), `BlobKey`, `Blob` (ZBE blob encryption key derivation and per-chunk AAD, [§4.2.1](#421-bundle-blob-encryption-zbe-normative)).

The fixed strings `zkCoins/v1/genesis` (an `Hc` *input* constant, [§1.4](#14-identifiers-and-hashes)), the network tags `zkCoins/v1/mainnet` / `zkCoins/v1/testnet` (verifier-data parameters, [§2.2 clause 5](#22-proof-types)), and the test-vector labels (V.1) reuse the version prefix for namespacing but are **not** domain-separation contexts — they never select an `Hc`/`HKDF`/`H` domain and are therefore not listed above.
...
seed  (256-bit; BIP-39 mnemonic, or Passkey PRF → HKDF)
  └─ BIP-32 ─▶ m  (master)
        └─ m / 1798' / account'                              = A   (per-account root; 1798' = zkCoins purpose)
              ...
              └─ A / 4'        = op_secret     (anchors-rand secret; keys the anchors_rand HKDF, §1.7.10)
```

`1798'` is the chosen BIP-43 purpose index for zkCoins (hardened). All branch separations are **hardened**: the VIEW, `op`, `nk`, and `op_secret` branches are hardened children of `A`, so a party holding them **cannot** derive the SPEND branch. `op_secret` (`A/4'`) is a dedicated 256-bit secret — separate from `op` so the anchors-randomness derivation never shares key material with the Nostr signature — that keys the deterministic `anchors_rand` HKDF (§1.7.10); like `nk` it is part of the operational bundle the wallet entrusts to its own node.
...
| **SpendRecord message** | `message = inr ‖ ocr ‖ next_pubkey_commit` — binds the spent nullifier set (`inr`), the produced output coins (`ocr`), **and** a **hiding commitment** `next_pubkey_commit` to the account's rotated spend authority `next_pubkey = Pkᵢ₊₁` (x-only, the new `current_pubkey`, §1.5, §2.1 clause 7). The third segment is **not** the raw key: signing `next_pubkey_commit` (and checking its opening in-circuit, §2.1 clause 2) makes the account's single transition signature authorise the key rotation just as directly as signing the raw key would — so a node cannot rotate `current_pubkey` to a key it controls on any transition, including an unbatched pure receive that never reaches the publisher's `C_batch` (§2.1 clause 2) — **while** the value published in the (public) `SpendRecord`/`BatchBundle` is hiding, so the rotation edge `next_pubkey(i) → current_pubkey(i+1)` can no longer chain an account's consecutive records ([§2.3.2](#232-send)) | 96 bytes |
| **next_pubkey_commit** | `next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)` — SHA-256 (the doc-`H`, §1.1; wallet-native, **no** Poseidon) hiding commitment to the account's rotated spend key `next_pubkey = Pkᵢ₊₁` (x-only), forming the third `message` segment above. `npk_rand` is a **fresh secret 32-byte blind the wallet draws per transition** — the analogue of `anchors_rand` ([§1.7.10](#1710-anchors-mmr-normative)) for this commitment. The blind is **load-bearing for privacy**: without it the commitment would be `H(next_pubkey)` and a `BatchBundle` scanner, who reads the next record's `public_key = Pkᵢ₊₁` in the clear, could recompute it and re-link the records; the secret per-transition `npk_rand` is exactly what breaks that join. `npk_rand` is a **circuit witness only** (§2.1 clause 2 opens the commitment against it) and travels wallet→node in the `/sign` body ([§7.5](#75-node-rest-api-normative)); it is **never** placed in a `SpendRecord`, `BatchBundle`, `CoinProof`, or handed to a publisher. Unlike `anchors_rand` it need not be reconstructible: no later transition re-opens `next_pubkey_commit` (the rotated key is carried forward as the committed `AccountState.current_pubkey`, §2.1 clause 7), so a fresh `npk_rand` may be discarded after signing | 32-byte digest (blind: 32-byte secret) |
| **out_intent_commit** (`oic`) | `out_intent_commit = H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)` — SHA-256 (the doc-`H`, §1.1; wallet-native, **no** Poseidon) hiding commitment to the transition's **validated output intent**: the exact set of output coins the wallet authorises. `serialize(output_templates)` is the canonical byte concatenation of the transition's output `CoinTemplate`s in the **§2.1 clause-5 output order** (the caller's recipient/payee coins first, then one change coin per asset in ascending `asset_id`, then the publisher-fee coin last), each template encoded as `recipient (32B) ‖ asset_id (32B) ‖ amount (16B, u128 big-endian)` — **80 bytes per template**, the same `recipient ‖ asset_id ‖ amount` field order as the `coin.identifier` preimage ([§1.7.3](#173-fixed-widths)) — over exactly the active output slots ([§2.5](#25-circuit-dimensioning-normative)); this is the **same** tuple list, in the same order, that clause 5 builds the output coins from and thus commits into `output_coins_root` (`ocr`). `oic_rand` is a **fresh secret 32-byte blind the wallet draws per transition** — the analogue of `npk_rand` for this commitment. The blind is **load-bearing for privacy**: without it the commitment would be `H(serialize(output_templates))` and any party holding one output's plaintext (a payee, or a `CoinProof` recipient) could confirm or recover the other recipients/amounts by re-hashing candidate template lists — exactly the linkage the [`next_pubkey_commit`](#14-identifiers-and-hashes) lesson forbids; the secret per-transition `oic_rand` breaks that join. `oic_rand` is a **circuit witness only** ([§2.1 clause 2](#21-the-compliance-predicate) opens the commitment against it) and travels wallet→node in the `/sign` body ([§7.5](#75-node-rest-api-normative)); it is **never** placed in a `SpendRecord`, `BatchBundle`, `CoinProof`, or handed to a publisher. `out_intent_commit` is the **sixth `ProofData` field** (below), a hiding public input — so, unlike `next_pubkey_commit` (which rides in the 96-byte `message`), it does **not** widen the published `SpendRecord`, whose `message` stays `inr ‖ ocr ‖ next_pubkey_commit` (96 bytes) and whose byte arithmetic (`193 + 32·\|nf\|`) is unchanged. Because `out_intent_commit` sits in `ProofData`, the account's transition signature — whose sign-to-contract nonce binds `H(ProofData)` ([§2.1 clause 9](#21-the-compliance-predicate), [§3.3](#33-off-chain-signature-handling)) — binds it exactly as it binds `ocr` **wherever that S2C tweak is re-verified**: the publisher's `C_batch` for every anchored spend ([§2.2 clause 2](#22-proof-types)) and the balance-attestation circuit ([§5.7](#57-balance-attestation-history-private)). A witness-building node therefore cannot redirect **a spend's** outputs to a recipient the wallet did not authorise ([§2.1 clause 2](#21-the-compliance-predicate), [§7.5](#75-node-rest-api-normative)); the one path an S2C re-check cannot reach — an **unbatched mint** — is instead closed by the no-input rule of [§2.1 clause 5](#21-the-compliance-predicate) (every output of a `k = 0` transition MUST pay `owner`), so **no path is uncovered**. Like a fresh `npk_rand`, `oic_rand` need not be reconstructible: no later transition re-opens `out_intent_commit`, so it may be discarded after signing | 32-byte digest (blind: 32-byte secret) |
| **SpendRecord** | `{ public_key: Pkᵢ (32B x-only), signature: BIP-340(skᵢ, message) (64B), message: inr ‖ ocr ‖ next_pubkey_commit (96B), k: u8 (1B, the count of input nullifiers), nullifiers: [nf]ⱼ (32B each — the coins spent in this transition; exactly `k` entries) }` — an **off-chain** object: a spender produces one per transition and hands it to a publisher. The publisher aggregates many `SpendRecord`s into one `BatchBundle` ([§4.6](#46-data-availability--replication-factor-k)), builds the `AggregateBatchProof` over them, and anchors the batch with one constant-size `BatchInscription` on Bitcoin ([§3.1](#31-the-on-chain-object)). A mint, which spends no coin, has `k = 0` and need not be batched at all — its receiver verifies it directly against its `CoinProof` bundle | 193 + 32·\|nf\| bytes off-chain |
...
| **ProofData** (public inputs) | `{ new_account_state_hash, output_coins_root, input_nullifiers_root, coin_history_root, anchors_commitment, out_intent_commit }` — there is **no** live accumulator root here: a spender's per-account proof attests local soundness; global double-spend is enforced by the publisher's `AggregateBatchProof` ([§2.2](#22-proof-types)) when it inserts the batch's nullifiers into the on-chain-committed accumulator. The fifth field, **`anchors_commitment`** (`anc`), is the account's **hiding commitment to its anchors-MMR root** `anchors_root` ([§1.7.10](#1710-anchors-mmr-normative)): the underlying root commits to the set of admitted batches against which the account's receive checks ([§2.1 clause 10](#21-the-compliance-predicate)) were performed and is carried forward **monotonically** by every transition (clause 1); the commitment hides it from chain-only observers, publishers, and `BatchBundle` scanners, and is opened only to a coin's recipient (via the `CoinProof` bundle) or a disclosure verifier, who then checks `anchors_root` against their own chain scan ([§2.3.3](#233-receive) step 2). The sixth field, **`out_intent_commit`** (`oic`, the row above), is the account's **hiding commitment to its validated output intent** — the exact output-`CoinTemplate` list clause 5 builds `ocr` from — computed by the wallet itself in SHA-256 (**no** Poseidon); [§2.1 clause 2](#21-the-compliance-predicate) opens it against those templates, so the wallet's single transition signature (which binds `H(ProofData)`, clause 9) authorises **which coins the outputs pay**, closing output-redirection by a witness-building node on the anchored-spend path where the S2C is re-verified ([§2.2 clause 2](#22-proof-types)); the one path the S2C re-check misses — an unbatched mint — is closed in-circuit by the no-input⟹`owner` rule of [§2.1 clause 5](#21-the-compliance-predicate). It is a **hiding** public input (fresh secret `oic_rand`), so it exposes nothing to a chain observer, publisher, or `BatchBundle` scanner — like `anchors_commitment`. **Canonical serialization** `serialize(ProofData) := new_account_state_hash ‖ output_coins_root ‖ input_nullifiers_root ‖ coin_history_root ‖ anchors_commitment ‖ out_intent_commit` (the six 32-byte digests in that exact order — **192 bytes**), and **`H(ProofData) := SHA-256(serialize(ProofData))`** is the single normative definition the **spender's** sign-to-contract tweak uses everywhere ([§2.1 clause 9](#21-the-compliance-predicate), [§2.2 clause 2](#22-proof-types), [§3.3](#33-off-chain-signature-handling), [§5.7](#57-balance-attestation-history-private), V.4). (The **publisher's** separate `BatchInscription` S2C binds `H_agg` over the proof bytes, not `H(ProofData)`, [§3.2](#32-batchinscription-signing-bip-340--sign-to-contract).) This digest order (`ash ‖ ocr ‖ inr ‖ chr ‖ anc ‖ oic`) is deliberately **not** the `SpendRecord` `message = inr ‖ ocr ‖ next_pubkey_commit` order — they are distinct objects | hashes/roots only |
| **BatchInscription** | `{ publisher_pubkey: Pkₚ, prev_root, new_root, bundle_locator, block_anchor, signature }` — the **only** object written to Bitcoin; 231 bytes per batch, constant in batch size; commits the publisher's accumulator state transition `prev_root → new_root` and binds the off-chain bundle by `bundle_locator = Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root)` ([§3.1](#31-the-on-chain-object), [§3.5](#35-inscription-format)) | 231 bytes per batch |
| **BatchBundle** | `{ prev_root, new_root, spend_records: [SpendRecord], aggregate_proof: AggregateBatchProof, nullifiers: [nf] (derived view) }` — off-chain, content-addressed by `blob_id`, `k = 3` replicated by the same DA discipline as `CoinProof` bundles ([§4.6](#46-data-availability--replication-factor-k)). Carries every member `SpendRecord` plus the recursive proof that attests the whole batch. The top-level `nullifiers` field is a **derived view** equal to the canonical multi-set concatenation of every member `SpendRecord`'s `nullifiers` in canonical bundle order — provided as a convenience for scanners that want the flat `nf` list without re-parsing every record. **Wire serialisation** `serialize(BatchBundle)` (what Blossom stores as a **plaintext** blob — a `BatchBundle` is public and self-authenticating, so ZBE applies only to `CoinProof` bundles, [§4.2.1](#421-bundle-blob-encryption-zbe-normative)/[§7.1](#71-serialization-conventions-normative)) is `prev_root ‖ new_root ‖ u32-be(m) ‖ SpendRecord₁ ‖ … ‖ SpendRecord_m ‖ <length-prefixed aggregate_proof>`. The **`bundle_locator`** is `Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root)`, where **`member_root`** is a binary Poseidon hash tree over the ordered members, defined exactly as the recursive aggregator builds it ([§2.5](#25-circuit-dimensioning-normative), [§2.2 clause 6](#22-proof-types)): the active-member leaf value is `dⱼ = Hc("BatchMember", serialize(SpendRecordⱼ))`, an inactive (padding) leaf is `Hc("BatchMember", ∅)` (`∅` = the empty byte string), and an internal node is `Hc("BatchMember/Node", left, right)`; the canonical shape is the perfect binary tree over `2^⌈log₂ max(m,1)⌉` leaves with the `m` active members left-aligned and the remainder padded. The distinct `BatchMember` (leaf) and `BatchMember/Node` (internal) tags rule out leaf/node second-preimage collisions. The locator preimage is **fixed-size** (three digests + a `u32`), so it is computed bottom-up by the aggregator rather than by hashing every record at once; `member_root` binds both the exact member set **and** their order. The derived `nullifiers` field and the `aggregate_proof` are **excluded** from the locator preimage (the former is redundant under §2.2 clauses 3–4; the latter because a proof cannot commit to its own bytes, so it is bound separately by the publisher's sign-to-contract tweak, [§3.2](#32-batchinscription-signing-bip-340--sign-to-contract)). A scanner recomputes `member_root` from the wire `spend_records` and confirms `bundle_locator` matches the inscription ([§3.6 step 6](#36-chain-scanning)) | grows with member count + recursive-proof size (typically ~100 KB) |
...
CoinProof    = {                            // the value-bearing off-chain bundle (bearer)
  ...
  anchor_hint?,                              // OPTIONAL bundle_locator of the batch that anchors
                                             // the creating SpendRecord (speeds §2.3.3 step 4 and
                                             // the clause-10 witness fetch; absent for a mint)
  anchors_opening = { anchors_root,          // the creating proof's anchors-MMR root and its
                      anchors_rand },        // commitment randomness (§1.7.10); lets the recipient
                                             // open creating_proof.anchors_commitment and check
                                             // extends(anchors_root, own anchors_root) in clause 10
...
| **Anchors MMR** | derived (identical on every honest node) | the `member_root` of every admitted batch, in canonical admission order (append-only; supports membership + extension proofs, [§1.7.10](#1710-anchors-mmr-normative)) | a pure function of the admitted `BatchInscription` chain plus the content-addressed bundles — no new on-chain field and no new trust assumption |

There is **exactly one** consensus-bearing global structure — the nullifier accumulator — and Bitcoin is the only ordering surface the protocol relies on. (The **anchors MMR** is not a second consensus structure: it is a deterministic, locally-derived index over the same admitted-batch sequence — every honest node computes the identical MMR from the chain and the bundles, exactly as it computes the identical accumulator. Its role is to let a receive transition prove, in-circuit and in constant size, that a creating `SpendRecord` belongs to an **admitted** batch, [§2.1 clause 10](#21-the-compliance-predicate).) zkCoins defines **no** global, account-keyed commitment tree: an account's latest state is carried by its own constant-size recursive proof ([Proofs §2.2](#22-proof-types)), never by a global per-account on-chain index. This is deliberate. A global structure keyed by a stable account identifier would have to be **either** rebuildable from publicly verifiable data **or** privacy-preserving — never both. The protocol keeps privacy ([Requirement 2](/requirements)) and rebuildability ([Requirement 10](/requirements)) at once by removing that structure entirely and anchoring double-spend protection in the **nullifier accumulator** alone.
...
**`‖` inside `Hc` call sites (normative).** Wherever this document writes `Hc(tag, x₁ ‖ x₂ ‖ … ‖ xₙ)`, the `‖` separates the **input list** of the §1.7.1 signature: each `‖`-separated argument is **one** input, individually encoded by the rules above and the widths of [§1.7.3](#173-fixed-widths) — the arguments **MUST NOT** be byte-concatenated into a single byte-string input first. `Hc(tag, a ‖ b)` and `Hc(tag, a, b)` denote the same invocation. For example, `Hc("Nullifier", nk ‖ coin.identifier)` absorbs `nk` as a byte-string input (length prefix + 5 chunks) followed by `coin.identifier` as a 4-limb digest input. Byte concatenation applies only outside `Hc` input lists — e.g. in `serialize(…)` layouts, `H(…)` and HKDF preimages (`ss ‖ epk`; the `next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)` preimage of [§1.4](#14-identifiers-and-hashes), whose ASCII tag and two 32-byte inputs are absorbed as raw bytes; and the `out_intent_commit = H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)` preimage of [§1.4](#14-identifiers-and-hashes), whose ASCII tag, `serialize(output_templates)` (each output template a raw `recipient(32) ‖ asset_id(32) ‖ amount(16)` = 80-byte record, in clause-5 order), and 32-byte blind are byte-concatenated raw), Bech32m payloads (`ivk ‖ ovk`), and `message = inr ‖ ocr ‖ next_pubkey_commit`. In the `bundle_locator` preimage `Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root)` ([§1.4](#14-identifiers-and-hashes)), `u32-be(m)` pins the member count `m` as a **4-byte big-endian byte string**, absorbed via the byte-string rule (one length element + one chunk element) — **not** as a small-numeric element.
...
**Operations.** A transition that spends `input_coins[j]` proves in-circuit that `coin.identifier = input_coins[j].identifier` has leaf state `1` against the prior `coin_history_root` (clause 2(b)); the same transition flips that leaf from `1` to `2` (spent) and admits each newly received or created coin by a **non-membership→insert**: it proves the key's leaf state is `0` against the prior root and only then flips it to `1` (received-unspent). Both directions are proven old-state transitions — `1 → 2` for a spend, `0 → 1` for an admission (clause 8) — so no coin can be admitted (and credited) twice along an account's lineage. `coin_history_root` after the transition is the recomputed root over these updates and is the value bound into the new `AccountState` (clause 8, §1.7.4). The distinct `CoinHist/Leaf` and `CoinHist/Node` tags — and the per-level domain separation in `H'_node` — make these constants distinct from the nullifier accumulator's `E_i` even though the SMT skeleton is the same.

#### 1.7.7 Bech32m and Bitcoin conventions

- Addresses, view grants, and bearer view capabilities use Bech32m with distinct HRPs so they are never confused: `zk` (address, 32-byte payload), `zkgrant` (view grant, full `ViewGrant` byte serialization), `zkview` (per-coin view capability, 32-byte payload), `zkavk` (bearer account view key, 64-byte `ivk ‖ ovk` payload, or 32-byte `ivk`-only payload — the incoming-only variant; see [Access & Explorer §5.8](#58-address-view-full-history)), `zkbid` (confirmation-link blob locator, 32-byte `blob_id = H(ciphertext)` — distinct from a `BatchBundle` `bundle_locator`; see [Access & Explorer §5.6](#56-shareable-confirmation-links)). A node/explorer **MUST** reject a value presented under the wrong HRP.
...
**Canonical proof serialization (normative).** Wherever the protocol hashes or content-addresses a proof — `H_agg = H(serialize(AggregateBatchProof))` ([§3.2](#32-batchinscription-signing-bip-340--sign-to-contract)) and any `bundle_locator` preimage that embeds proof bytes — `serialize(·)` of a Plonky2 proof is its **native `ProofWithPublicInputs::to_bytes()`** encoding (the Plonky2 1.1.0 canonical byte layout: public inputs as 8-byte-LE field elements followed by the proof body). It is **not** a serde/`bincode` encoding (an implementation MAY use `bincode` or any other format for its own at-rest storage, but that form is never hashed or content-addressed). Because production proofs are zero-knowledge (randomised), `to_bytes()` differs run-to-run; this is harmless — the publisher fixes one concrete proof, publishes those exact bytes in the `BatchBundle`, and every verifier reads back the same bytes and recomputes the same `H_agg`. The `bundle_locator` preimage (`prev_root ‖ new_root ‖ u32-be(m) ‖ member_root`, [§1.4](#14-identifiers-and-hashes)) carries **no proof bytes at all**; the proof is bound separately by the S2C tweak over `H_agg`, so the non-determinism of ZK proof bytes never enters the on-chain `bundle_locator` binding. (The proof bytes do appear in the *wire* `serialize(BatchBundle)` that Blossom stores; that byte string is content-addressed by its Blossom `blob_id = H(serialize(BatchBundle))` — a per-batch fetch key the publisher fixes once, never an on-chain binding — and it never enters the on-chain `bundle_locator`, whose preimage is only `member_root`, derived from the records.)

#### 1.7.10 Anchors MMR (normative)

The **anchors MMR** is an **append-only Merkle Mountain Range** over the admitted batches, used by the receive path ([§2.1 clause 10](#21-the-compliance-predicate)) to prove — in-circuit, in constant size — that a creating `SpendRecord` belongs to an **admitted** batch. It is **derived, not consensus-bearing** ([§1.6](#16-trees-one-global-structure-one-per-account-structure)): every node computes it deterministically while scanning ([§3.6](#36-chain-scanning)), with **no** new on-chain field.

- **Leaf `j`** (0-based): `Lⱼ = Hc("AnchorsMmr/Leaf", member_rootⱼ)`, where `member_rootⱼ` is the [§1.4](#14-identifiers-and-hashes) member root of the `j`-th batch whose `BatchInscription` reached state **`completed`** ([§3.10](#310-transaction-states)), in canonical Bitcoin order ([§3.6 step 4](#36-chain-scanning): block height, then reveal-tx index). Only `completed` batches are appended, so under the [§3.9](#39-finality) reorg assumption the leaf sequence is stable.
- **Internal node:** `Hc("AnchorsMmr/Node", left, right)`. The MMR is the standard forest of perfect binary trees ("peaks"): appending a leaf merges equal-height peaks pairwise.
- **Root ("bagging"):** for `n` leaves with peaks `P₁ … P_p` (left-to-right, tallest first), `anchors_root = Hc("AnchorsMmr/Root", n ‖ P₁ ‖ … ‖ P_p)` with `n` a `u64` (wide-numeric encoding, [§1.7.2](#172-field-encoding-e-of-hc-inputs)). The **empty root** (`n = 0`) is `anr_empty = Hc("AnchorsMmr/Root", 0)` — the base of every account's anchors lineage ([§2.1 clause 1](#21-the-compliance-predicate), InitialProof path: `extends(anr_empty, anchors_root)`).
- **Membership proof:** the sibling path from `Lⱼ` to its peak, plus the remaining peaks and `n` — verified by recomputing `anchors_root`. Cost: one Poseidon path of depth `≤ ⌈log₂ n⌉` plus the bagging hash.
- **Extension proof (`extends(anr, anr')`):** attests that the MMR with root `anr'` (at `n'` leaves) is an **append-only extension** of the MMR with root `anr` (at `n ≤ n'` leaves) — the standard MMR consistency proof: the old peaks are recomputed from witnessed subtrees of the new forest, and both roots are recomputed via the bagging rule. `extends(anr, anr)` holds trivially (empty witness). Extension is transitive: `extends(a, b) ∧ extends(b, c) ⇒ extends(a, c)`.

**Anchors commitment (what a proof exposes, normative).** A proof **MUST NOT** expose `anchors_root` in the clear: the set of valid roots is tiny (one per admission-prefix length `n`) and every node indexes it, so a public root would decode to `n` — the account's exact receive-recency — and correlate two proofs that share it ([Requirement 2](/requirements)). The fifth `ProofData` field is therefore a **hiding commitment**

```
anchors_commitment = Hc("AnchorsView", anchors_root ‖ anchors_rand)
```

The pair `{anchors_root, anchors_rand}` is the **anchors opening**. It is disclosed only to parties who must check the root against their own scan — carried in the `CoinProof` bundle to a coin's recipient (§1.5) and, for a fee coin, to its publisher; a chain-only observer and a `BatchBundle` scanner never see it and learn **nothing** from the commitment.

**`anchors_rand` (normative).** `anchors_rand` **MUST** be a per-transition value that is unique and unpredictable to outsiders. It is derived **deterministically** so any prover holding the operational bundle — the wallet or its own node ([§6.2](#62-wallet--node)), which builds the witness ([§7.5](#75-node-rest-api-normative)) — reproduces it identically, and so a fresh node can rebuild any prior opening for the clause-1 carry-forward (node portability, [Requirement 10](/requirements)): `anchors_rand = HKDF("zkCoins/v1/AnchorsRand", op_secret ‖ u64-be(send_counter))`, keyed by `op_secret` (the hardened `A/4'` secret, [§1.2](#12-key-hierarchy); part of the operational bundle) and the strictly-increasing `send_counter`. The `(op_secret, send_counter)` input is unique per transition and never reused, so `anchors_rand` never repeats; it **MUST NOT** be derived from `anchors_root` — two transitions committing the same `(anchors_root, anchors_rand)` pair would produce identical `anchors_commitment` values and become publicly linkable. Because the valid-root set is enumerable (one per `n`), the commitment's hiding rests entirely on `anchors_rand`'s 256-bit entropy under Poseidon modelled as a random oracle ([§1.7.1](#171-poseidon-instance-and-digest-encoding)); the same domain-separation argument as `detect_tag` vs `K_tx` ([§1.3](#13-per-coin-keys-note-encryption--detection)) applies. (The [test vectors](#test-vectors-conformance-harness) pin a fixed illustrative `anchors_rand` value rather than exercising this HKDF chain.)

**`anchors_root` selection (normative, wallet obligation).** A transition **MUST** set `anchors_root` to the node's current anchors-MMR root — the tip over all `completed` batches at proving time — subject to the `extends`/prefix constraints below (clauses 1 and 10). Choosing the live tip, rather than the minimal root the constraints would permit, is what keeps the opening uninformative: the tip is a **global** value shared by every account transacting at that time, correlated only with the wall-clock instant (which the batch `block_anchor` already discloses, [§3.5](#35-inscription-format)), so an opening holder — recipient or publisher — learns only *when* the transaction was proved, never the account's private receive count or history. **This is a wallet obligation, not a circuit- or verifier-enforced rule:** clauses 1 and 10 enforce only `extends` (relative monotonicity), and the receiver's scan ([§2.3.3 step 2](#233-receive)) accepts any valid *historical* root, so a non-conforming wallet **could** pin an older or account-specific root and still produce an accepted proof — it would thereby leak **its own** recency to that transaction's opening holders. This is self-inflicted (never a soundness break and never another account's privacy), and tip-freshness is inherently not in-circuit provable (the tip is a time-varying global value); conforming wallets **MUST** tip-pick.

**How the predicate uses it (summary; full rules in [§2.1](#21-the-compliance-predicate)).** Two `extends` relations bind the commitment into an unforgeable, transitive chain:

- **Carry-forward (clause 1).** In-circuit, `extends(prev_proof.anchors_root, anchors_root)` — the account can only extend its own anchors view, never rewind or replace it. The prover reconstructs its previous opening from the deterministic `anchors_rand` derivation above (keyed by the prior `send_counter`), so it witnesses `prev_proof.anchors_root` and re-derives `prev_proof.anchors_commitment` without persisting extra state.
- **Cross-account binding (clause 10).** For every received coin, in-circuit `extends(creating_proof.anchors_root, anchors_root)` — the creating account's anchors view **MUST** be a prefix of the receiver's own. The receiver opens the creating proof's commitment with the opening the sender placed in the `CoinProof` bundle, then checks the extension. A fabricated (non-admitted) anchor leaf makes the creating account's root a **non-prefix** of any honest root, so it can never be a prefix of the receiver's scanned-valid root — the forgery cannot be laundered across the hop, and the first honest verifier catches it. This is what makes the anchoring guarantee **transitive** rather than one-hop.

Only the top-level `anchors_root` of a directly-verified proof is checked against a scan ([§2.3.3 step 2](#233-receive)); every earlier root in the lineage is bound to it by the in-circuit `extends` chain, so one scan lookup validates the whole receive history.

A node **MUST** maintain the anchors MMR alongside the nullifier accumulator, index the historical `anchors_root` value after every append, and answer, for its wallets: a membership path for a given admitted `member_root` (the [§7.5](#75-node-rest-api-normative) `/v1/chain/anchors/member/<member_root>` service), and the validity of a presented `anchors_root` (whether it equals the MMR root after some prefix of admissions — a lookup in the per-append root index). Reorg handling follows [§3.7](#37-the-nullifier-accumulator): because only `completed` batches are appended, a reorg deeper than the [§3.9](#39-finality) bound is a protocol-failure event, not an MMR rewind.

**Trust note (normative).** Nothing on Bitcoin commits the anchors MMR directly, so an `anchors_root`-validity answer is **not** self-verifying the way a [§3.7](#37-the-nullifier-accumulator) Path-B nullifier path is: it is exactly as trustworthy as the node that derived it from the chain and the bundles. A Path-A verifier (any full node, including every wallet's own node — the [§6.2](#62-wallet--node) default) performs the check trustlessly against its own index and reveals nothing; a pure light client that must ask a foreign node inherits both the [§6.3](#63-node-portability-and-multi-node-operation) honest-node spectrum **and** a metadata disclosure (the queried root reveals the subject's recency to that node), so it **SHOULD** sync the root index in bulk rather than query single roots. A future version **MAY** commit the anchors root inside the `BatchInscription` to close both gaps for light clients.
...
w = {
  ...
  received_auth[] = {         // per received coin, provenance evidence (clause 10)
    ...
    member_path,                                     // Merkle path of Hc("BatchMember", creating_spend_record) to its batch's member_root (absent for a mint)
    anchor_path                                      // anchors-MMR membership path of that member_root against this transition's anchors_root (§1.7.10; absent for a mint)
  },
  anchors_root,               // this transition's anchors-MMR root (opened value behind ProofData.anchors_commitment)
  anchors_rand,               // fresh 256-bit commitment randomness (§1.7.10)
  prev_anchors_opening,       // {prev anchors_root, prev anchors_rand} opening prev_proof.anchors_commitment (absent for InitialProof)
  anchors_extension,          // MMR extension witness extends(prev anchors_root, anchors_root) (§1.7.10; empty when unchanged / InitialProof from anr_empty)
  nk,                         // nullifier key (Foundations §1.2; held by the wallet and its own node — operational bundle)
  next_pubkey   = Pkᵢ₊₁,      // rotated spend pubkey for the new state (committed as next_pubkey_commit, §1.4)
  npk_rand,                   // fresh 256-bit blind for next_pubkey_commit (wallet-chosen per transition; §1.4, opened by clause 2)
  oic_rand,                   // fresh 256-bit blind for out_intent_commit (wallet-chosen per transition; §1.4, opened by clause 2
                              //   against output_templates, which build ocr in clause 5)
...
1. **Recursive verification (PCD) and anchors carry-forward.** First, the fifth public input binds the witnessed anchors root and randomness: `ProofData.anchors_commitment` **MUST** equal `Hc("AnchorsView", w.anchors_root ‖ w.anchors_rand)` ([§1.7.10](#1710-anchors-mmr-normative)). Then either this is an `InitialProof` and `w.prev_proof` is absent and `w.prev_account_state` is the canonical empty account for the account's `owner` ([§2.2](#22-proof-types)), where the InitialProof **MUST** check `owner == H(txn_pubkey ‖ prev_account_state.nk_commit)` (i.e. `owner = H(Pk₀ ‖ nk_commit)`, §1.4) **and** `prev_account_state.nk_commit == Hc("NkCommit", nk)` (clause 4) — so the genesis address commits to **both** the initial spend key `Pk₀` (= `txn_pubkey` at the first transition) and the nullifier key `nk`; two distinct `nk` yield two distinct addresses, so a coin sent to an address has exactly **one** valid nullifier and genesis equivocation is impossible — and `w.anchors_root` satisfies `extends(anr_empty, w.anchors_root)` (`w.anchors_extension`); **or** `w.prev_proof` verifies under the circuit's own verifier data (cyclic recursion), its public output `new_account_state_hash` equals the `ash` of `w.prev_account_state`, its `coin_history_root` equals the coin-history root over which clause 2 proves inclusion, its `anchors_commitment` equals `Hc("AnchorsView", w.prev_anchors_opening.anchors_root ‖ w.prev_anchors_opening.anchors_rand)` (opening the previous commitment), **and** `w.anchors_root` satisfies `extends(prev.anchors_root, w.anchors_root)` via `w.anchors_extension` — the account's anchors view is carried forward **monotonically** and can never be replaced or rewound, so a fabricated anchor leaf poisons every later `anchors_root` of the lineage (caught by the [§2.3.3 step 2](#233-receive) check). The verifier data **MUST** be fixed and identical in prover and verifier; a proof verified against any other verifier data is invalid.
   - **No global lineage anchor.** An account's latest state is attested **entirely** by its own constant-size recursive proof; the protocol defines no global, account-keyed commitment tree to bind to ([Foundations §1.6](#16-trees-one-global-structure-one-per-account-structure)). Anchoring to Bitcoin comes instead via the **publisher batching path**: a spend takes effect only once its `SpendRecord` is included in a `BatchBundle` whose `BatchInscription` is admitted on-chain (§2.3.3, [On-chain §3.6](#36-chain-scanning)), and equivocation between two forks of one account is caught because both forks **provably reuse the same input-coin `nf`** — the `nk` that derives `nf` is committed to the account by the `nk_commit` field of `prev_account_state` (clause 4), so two forks of one `prev_account_state` are forced to the identical `nk`, hence the identical `nf` for any shared coin, and cannot equivocate — and that `nf` can enter the global nullifier accumulator only once: the publisher's `AggregateBatchProof` ([§2.2](#22-proof-types)) attests `new_root` as the deterministic SMT-insertion result, so an `nf` already in the accumulator at `prev_root` would make the proof unsatisfiable.

2. **Input authenticity.** The whole transition is authorised by the account's **single transition signature** — there is no per-coin key and no per-coin signature ([Foundations §1.2](#12-key-hierarchy)). The circuit **MUST** check that `txn_sig` is a valid **BIP-340** signature (see [Foundations §1.1](#11-cryptographic-primitives)) over `message = inr ‖ ocr ‖ next_pubkey_commit` (the `SpendRecord` message of [Foundations §1.4](#14-identifiers-and-hashes): `input_nullifiers_root` from clause 4 ‖ `output_coins_root` from clause 6 ‖ `next_pubkey_commit`, the **hiding commitment** to the account's rotated spend key) by `txn_pubkey = Pkᵢ`, and that `Pkᵢ` is `prev_account_state.current_pubkey`. The circuit **MUST** additionally check the commitment **opening** `next_pubkey_commit == H("zkCoins/v1/NextPubkeyCommit" ‖ w.next_pubkey ‖ w.npk_rand)` ([Foundations §1.4](#14-identifiers-and-hashes)), where `w.next_pubkey = Pkᵢ₊₁` (x-only) is the **same** value clause 7 folds into `new_account_state` as the rotated `current_pubkey` (so the committed key and the new state agree by construction) and `w.npk_rand` is a fresh per-transition witness blind. Because SHA-256 is collision-resistant, a signature over `next_pubkey_commit` plus this opening bind the rotated key `Pkᵢ₊₁` **exactly as tightly** as a signature over the raw key would — a party without `skᵢ` can neither produce a signature over a different commitment nor open the given commitment to any key other than `Pkᵢ₊₁` — while the value published in the (public) `SpendRecord` is now hiding, not `Pkᵢ₊₁` in the clear ([§2.3.2](#232-send)). Committing `next_pubkey` into the signed `message` this way is what binds the account's **future spend authority** to its single custody signature on **every** transition. Without it, `next_pubkey` — a free witness (`w.next_pubkey`) that **no other clause constrains** (`owner` and `nk_commit` are pinned unchanged by clause 7, `balances` are fixed by clauses 3/10, `send_counter` is incremented by exactly one, `coin_history_root` is fixed by clause 8, and every `received_coins[]` credit is bound by clause 10) — would enter the new state authorised by no executed check on a pure **receive**: a receive produces no `SpendRecord` for any publisher ([§2.3.3 step 7](#233-receive)) and so never passes through the publisher's `C_batch` sign-to-contract check ([§2.2 clause 2](#22-proof-types)), the only place `H(ProofData)` — hence `ash`, hence `next_pubkey` — was otherwise bound. A node holding the operational bundle `{ivk, ovk, op, nk, op_secret}` but **not** the spend key could then rotate `current_pubkey` to a key it controls behind an innocuous receive `message` (whose `inr` and `ocr` are the fixed empty-list roots) and afterwards spend the account, breaking the [§6.6](#66-threat-model-and-trust-configurations) custody guarantee — already from the first transition, since even the InitialProof runs this signature check over a `message` that now carries `next_pubkey_commit`. Because `next_pubkey` is the **only** freely-chosen new-state field, committing it into the signed `message` is authorisation-complete: the current key `Pkᵢ` signs the hiding commitment `next_pubkey_commit` to the next key `Pkᵢ₊₁`, and clause 2's opening check ties that commitment to the very `w.next_pubkey` clause 7 folds into the new state, so the operational bundle alone still cannot spend.

The circuit **MUST** additionally check the **output-intent opening** `ProofData.out_intent_commit == H("zkCoins/v1/OutIntentCommit" ‖ serialize(w.output_templates) ‖ w.oic_rand)` ([Foundations §1.4](#14-identifiers-and-hashes)), where `w.output_templates` is the **same** ordered template list clause 5 constructs the output coins — hence `output_coins_root` (`ocr`, clause 6) — from; `serialize(output_templates)` is its canonical [§1.4](#14-identifiers-and-hashes) byte layout (each template `recipient(32) ‖ asset_id(32) ‖ amount(16)`, in the clause-5 output order, over the active output slots — the in-circuit, `MAX_TX_OUTPUTS`-padded absorption follows the inactive-slot discipline of `serialize(AccountState)`, [§1.7.4](#174-serializeaccountstate)/[§2.5](#25-circuit-dimensioning-normative), so the in-circuit `out_intent_commit` equals the wallet's out-of-circuit SHA-256 bit-for-bit); and `w.oic_rand` is a fresh per-transition witness blind. Because `out_intent_commit` is the **sixth `ProofData` field**, it is covered by `H(ProofData) = SHA-256(serialize(ProofData))` ([§1.4](#14-identifiers-and-hashes)), which the account's transition signature binds through its sign-to-contract nonce (clause 9, [§3.3](#33-off-chain-signature-handling)) — so the single custody signature authorises **which coins the outputs pay** exactly as it authorises `ocr` itself, and it does so **without** widening the 96-byte `message`, leaving the published `SpendRecord` byte-for-byte unchanged.

This opening is what lets a **thin wallet bind an output intent it cannot otherwise verify.** `ocr` already commits every output's `recipient`/`amount` (clause 5, [§1.4](#14-identifiers-and-hashes)) and is already signed (the middle `message` segment) — but `ocr` is a **Poseidon** root the wallet **cannot recompute** ([§7.5](#75-node-rest-api-normative): no Poseidon in the wallet), so the wallet cannot tell whether a node-supplied `ocr` roots its intended payees or an attacker's. Without a wallet-native binding, a witness-building node (a hosted "public wallet API" account, [§6.1](#61-components-and-responsibilities)/[§6.6](#66-threat-model-and-trust-configurations), or a compromised own node) could set `w.output_templates = {recipient = attacker}`, build the matching `ocr'`, hand the wallet `inr ‖ ocr'` to blind-sign, and emit a valid proof + `SpendRecord` that pays the **attacker** — stealing the coins even though the spend key never left the wallet, breaking [Requirement 3](/requirements) ("a public node … cannot steal") and the [§6.6](#66-threat-model-and-trust-configurations) custody guarantee. `out_intent_commit` closes this: the wallet computes it **itself**, in SHA-256, over the plaintext output templates it **validated** in the proving handshake ([§7.5](#75-node-rest-api-normative): the payee templates equal its own intent, the fee coin goes to the quoted `fee_address`/`fee_asset_id` and its `amount` **equals** the quoted `fee` exactly, every change coin has `recipient == owner`, and **no** template has a non-`owner` recipient other than an intended payee or that single fee coin — so the node can neither over-charge the fee nor smuggle in an extra non-`owner` output), and reconstructs `H(ProofData)` over that self-computed `out_intent_commit` before signing. Any substitution of `w.output_templates` changes **both** `ocr` (clause 5/6) and `out_intent_commit` (this opening); the wallet's signature — which the node cannot forge — commits, via `H(ProofData)`, to the `out_intent_commit` the wallet formed over its validated intent, so the substituted templates cannot simultaneously open this commitment **and** match the signed `H(ProofData)`, and the proof is unsatisfiable.

Only the **output** side is a theft lever, so only it needs this binding: `inr` (the node's input selection) may stay node-determined and wallet-blind, because per-asset conservation (clause 3, now the **exact equality** `In(a) + Mint(a) == Out(a)`) *forces* any over- or mis-selection of inputs back to the account as a **change** coin to `owner` — the equality forbids dropping the surplus (a burn is unsatisfiable) and the [§7.5](#75-node-rest-api-normative) handshake's exact-fee / no-unexpected-non-`owner`-output checks forbid rerouting it to the publisher or a stranger, so value flows into the outputs (`ocr`), never away — whereas redirecting an **output** hands value to an attacker-chosen recipient. This clause-2 **opening** (`out_intent_commit ↔ w.output_templates ↔ ocr`) runs in **every** execution of `C`; the remaining link — that the `ProofData.out_intent_commit` the opening uses is the value the **wallet** signed, not one the node substituted — is supplied by the **sign-to-contract** binding of `H(ProofData)` (clause 9), which is re-verified wherever the transition signature is checked against its nonce: the publisher's `C_batch` ([§2.2 clause 2](#22-proof-types)) for **every anchored spend** — so **all sends** (the [§6.6](#66-threat-model-and-trust-configurations) theft vector, since a spend always produces a batched `SpendRecord`) are fully covered — and the balance-attestation circuit ([§5.7](#57-balance-attestation-history-private)). The only transitions that can go **unbatched** — a mint and a pure receive — are exactly those with **no** input coin (`k = 0`), and those are closed by the **no-input rule of clause 5**: every active output of a `k = 0` transition MUST pay `prev_account_state.owner`, enforced in-circuit independent of any S2C path. So an unbatched mint's coin cannot be redirected either (a pure receive has no outputs at all). Every transition's outputs are therefore bound to recipients the wallet authorised — `k ≥ 1` via `out_intent_commit`/S2C, `k = 0` via clause 5's owner rule — with **no uncovered path**. Then, for every `input_coins[j]`:
...
3. **Per-asset balance conservation.** Let `In(a) = Σ { input_coins[j].amount : input_coins[j].asset_id = a }` and `Out(a) = Σ { output_templates[k].amount : output_templates[k].asset_id = a }`, plus `Mint(a)` from any `asset_issuance` for asset `a` (zero otherwise). For **every** `asset_id` `a` appearing in inputs or outputs: `In(a) + Mint(a) == Out(a)` — an **exact equality**, not an inequality. Each `amount` is range-checked to `[0, 2^128 − 1]` (§1.7.3); an amount outside range invalidates the proof. Because each per-asset sum ranges over at most `MAX_TX_INPUTS` inputs or `MAX_TX_OUTPUTS` outputs plus an optional `Mint` (§2.5), `In(a)`, `Out(a)`, and `Mint(a)` are accumulated in-circuit as **exact non-negative integers** in a fixed width of at least `128 + ⌈log₂ max(MAX_TX_INPUTS + 1, MAX_TX_OUTPUTS)⌉` bits (132 bits at the v1 bounds) — wide enough that no term or partial sum can overflow — and the equality is an **exact wide-integer comparison**, never a modular one over the Goldilocks field (`p ≈ 2^64`, §1.1). Range-checking each `amount` alone bounds each term but **not** their sum: a field- or `u128`-wrapping sum would let outputs whose integer total exceeds the inputs pass conservation whenever their wrapped total does not, minting spendable value from nothing. This wide-integer accumulation and comparison are non-native to Goldilocks and are realised by the multi-limb balance gadgets of [§2.6](#26-in-circuit-non-native-cryptography-normative). **Equality conserves value in both directions:** outputs can neither **exceed** inputs plus an explicit, predicate-checked `Mint(a)` (no inflation) **nor fall short** of them (no burn / value destruction). Every unit an account spends as input therefore **MUST** reappear in the transition's outputs; the surplus `In(a) + Mint(a) − (payee and fee outputs for a)` cannot be dropped — the node **MUST** re-output it as a **change coin to `owner`** ([§2.3.2](#232-send) step 4), whose `amount` is itself range-checked to `[0, 2^128 − 1]` (§1.7.3) — so an input excess is forced back to the account rather than being burnable or divertible. (The earlier `≥` form permitted value destruction — an input surplus matched by no output — which this equality removes.) Conservation is taken over `input_coins` versus `output_templates` **only**: `received_coins[]` are **not** outputs and never enter these sums — clause 10 credits each received `amount` to `balances`/coin-history separately (clause 7, clause 8) — so a **pure receive** (no inputs, no outputs) satisfies `0 == 0`, and a combined receive-plus-send still balances `input_coins` against `output_templates` alone. Together with the `owner`-binding of the [§7.5](#75-node-rest-api-normative) proving handshake (exact fee, no unexpected non-`owner` output) and the no-input rule of clause 5, this equality is what makes the [clause 2](#21-the-compliance-predicate) promise — *value flows into the outputs, never away* — a circuit-enforced fact, not a convention. When `asset_issuance` is present, the v1 mint clauses of [Architecture §6.5](#65-issuance--versioned-schemas-v1-minimal) **MUST** all hold — these are the normative content of the v1 mint circuit, and they hook §6.5 into the predicate enumerated here. In summary:
...
4. **Nullifier derivation.** The circuit **MUST** first bind the witnessed `nk` to the account: `Hc("NkCommit", nk)` **MUST** equal `prev_account_state.nk_commit` ([§1.5](#15-core-data-structures), [§1.2](#12-key-hierarchy)); a witness whose `nk` does not open the committed `nk_commit` invalidates the proof. Only then, for every `input_coins[j]`, compute `nf_j = Hc("Nullifier", nk ‖ input_coins[j].identifier)` ([Foundations §1.4](#14-identifiers-and-hashes)) in-circuit from that bound `nk`. This binding closes fork double-spend: because `nk_commit` is carried forward unchanged from genesis (clause 1, clause 7), two forks of the same `prev_account_state` share the identical `nk_commit`, are forced to the identical `nk`, and therefore derive the **identical** `nf` for any shared input coin — so both forks' spends of one coin collide on a single global-accumulator entry (which admits each `nf` at most once, clause 1, [§2.3.3 step 5](#233-receive), [§6.5](#65-issuance--versioned-schemas-v1-minimal)) and cannot both be admitted. All `nf_j` within one transition **MUST** be pairwise distinct, and they form the leaves whose root is `ProofData.input_nullifiers_root`. These `nf_j` are carried into the spender's `SpendRecord` (off-chain, [§1.5](#15-core-data-structures)); the per-spender `message = inr ‖ ocr ‖ next_pubkey_commit` signature plus the sign-to-contract tweak bind the **root** `input_nullifiers_root`, and the opening of that root to the record's actual `nullifiers` list is enforced by the publisher's `AggregateBatchProof`, which **MUST** check `NullifiersRoot(SpendRecord.nullifiers) == input_nullifiers_root` and `k == |nullifiers|` ([§2.2 clause 3](#22-proof-types)) — so the `nf` list actually inserted into the accumulator is exactly the opening of the committed `inr` and cannot drop or swap a nullifier. The per-account proof makes **no** in-circuit claim of global non-membership. Global double-spend protection is enforced by the **publisher's** `AggregateBatchProof` ([§2.2](#22-proof-types)) when the batch is inscribed: it attests every member `nf` is correctly derived and that the new accumulator root is the SMT-insertion of every batch member into the previous root ([On-chain §3.7](#37-the-nullifier-accumulator)). A receiver checks non-membership against the live on-chain accumulator (§2.3.3 step 5) via Path A or Path B ([§3.7](#37-the-nullifier-accumulator)). Within the account, clause 2(b) together with the coin-history update (clause 8) prevent the account from spending the same coin twice along its own lineage.

5. **Output coin construction.** For each `output_templates[k]`, the new `coin.identifier` is computed as `Hc("Coin", prev_account_state_hash ‖ output_templates[k].recipient ‖ output_templates[k].asset_id ‖ output_templates[k].amount ‖ coin_index_k)` ([Foundations §1.4](#14-identifiers-and-hashes)), with `coin_index_k` assigned monotonically within the transition — so each output coin's `recipient` and `amount` are committed into `output_coins_root` (clause 6) and cannot be restated by a later holder. **Canonical output order (normative):** `coin_index` is assigned `0, 1, 2, …` over the outputs in this exact order — (i) the recipient coins in the caller's `output_templates[]` order, then (ii) the per-asset change coins in **ascending `asset_id`** order ([§2.3.2](#232-send) step 4), then (iii) the publisher-fee coin ([§3.8](#38-fees-and-economics)) last, if present. This fixes a single `ocr` for a given logical transaction so test vectors are reproducible and a wallet's own `ocr` is deterministic. This is also the exact order `serialize(w.output_templates)` uses for the clause-2 `out_intent_commit` opening ([§1.4](#14-identifiers-and-hashes)), so the template list the wallet's signature binds (via `out_intent_commit`) and the list this clause builds `ocr` from are provably one and the same — a witness-building node cannot pass this clause with one template list while having committed another to the wallet. Using the **prior** state's `ash` here keeps the identifier non-circular with respect to `new_account_state_hash` (which itself folds in the post-transition `coin_history_root` covering these very output coins). The resulting `Coin` objects (`{identifier, recipient, amount, asset_id}`) are the transition's outputs.

   **No-input transitions pay only the owner (normative).** If the transition spends **no** input coin — its input set is empty, i.e. `k = 0`, equivalently `input_nullifiers_root == L_⊥` (the empty-list `NullifiersRoot`, [§1.7.5](#175-poseidon-merkle-tree-used-for-ocr-and-inr)) — then **every active output coin MUST have `recipient == prev_account_state.owner`.** This is a pure per-output-slot field equality (**no** Poseidon, **no** EC, **no** signature check) that adds no witness and no published bytes. It closes output redirection on the **only** path the [`out_intent_commit`](#14-identifiers-and-hashes) sign-to-contract binding (clause 2) does not reach. A transition that spends inputs (`k ≥ 1`) always produces a `SpendRecord` and can be admitted only through a publisher batch, whose `C_batch` re-verifies the spender's S2C tweak ([§2.2 clause 2](#22-proof-types)) and thereby binds `out_intent_commit` — so **every** value-moving spend is already covered there. The transitions that can go **unbatched** are exactly the `k = 0` ones — a **mint** ([§2.3.1](#231-mint--issuance)) and a **pure receive** — and their `out_intent_commit` is never S2C-re-checked; without this clause a witness-building node could point an **unbatched mint's** coin at itself. This clause forbids that in-circuit, independent of any batching, and breaks no legitimate flow: a mint's issued coin is created to `recipient = owner` by construction ([§2.3.1](#231-mint--issuance), [§6.5](#65-issuance--versioned-schemas-v1-minimal)); a pure receive builds no output coins at all (its `output_templates` is empty, so the rule holds vacuously); and any distribution to a third party spends inputs (`k ≥ 1`), is therefore batched, and is covered by `out_intent_commit`. (A `k = 0` transition also has no input value — `In(a) = 0`, clause 3 — so its only outputs are the creator-bound `Mint(a)` coins, which §6.5 already assigns to `owner`.)
...
7. **New account state.** `new_account_state` is `prev_account_state` with: `balances` updated per clause 3 (debit spent inputs, credit change and any issuance) **and credited with every `received_coins[]` amount admitted by clause 10**, `current_pubkey = next_pubkey = Pkᵢ₊₁` (the same `w.next_pubkey` clause 2 checks opens `next_pubkey_commit`), `send_counter` incremented by one, and `coin_history_root` set to the value produced by clause 8 (the recomputed per-account coin-history SMT root, [Foundations §1.7.6](#176-nullifier-accumulator-sparse-merkle-tree)). The updated `balances` **MUST** hold at most `MAX_ACCOUNT_ASSETS` distinct non-zero entries ([§2.5](#25-circuit-dimensioning-normative)); the circuit builds `serialize(new_account_state)` over its fixed `MAX_ACCOUNT_ASSETS` balance slots with the inactive-slot discipline of [§1.7.4](#174-serializeaccountstate), so the in-circuit `ash` equals the out-of-circuit variable-length `Hc("AccountState", serialize(new_account_state))` bit-for-bit. `ProofData.new_account_state_hash` **MUST** equal `ash = Hc("AccountState", serialize(new_account_state))` ([Foundations §1.4, §1.7.4](#14-identifiers-and-hashes)). `new_account_state.owner` **and** `new_account_state.nk_commit` **MUST** be unchanged.

8. **Coin-history update.** The per-account coin-history SMT is updated to mark spent inputs (`1 → 2`), admit the change/issuance coins (`0 → 1`), and admit every `received_coins[]` entry accepted by clause 10 (`0 → 1`); `ProofData.coin_history_root` **MUST** equal the resulting root. **Every `0 → 1` admission MUST be a non-membership→insert:** the circuit **MUST** prove the key's leaf state is `0` (the coin has never been received or spent by this account) against the **prior** `coin_history_root` before flipping it to `1`, exactly as the spend side proves the input's prior leaf state is `1` (clause 2(b)) before flipping it to `2`. This makes admission a proven `0 → 1` transition, **not** an idempotent set-to-`1`: the same coin cannot be admitted (hence credited by clause 7) twice along an account's lineage, because its second admission would find prior state `1` (or `2`), not `0`, and fail. The change/issuance coins admitted here carry **freshly derived** identifiers (they bind this transition's `prev_account_state_hash`, clause 5) and so are always absent at prior state `0`; the proven-`0` requirement therefore never blocks them and is load-bearing only for the `received_coins[]` entries, whose identifiers originate in **other** accounts' transitions.

9. **Public-input binding.** All six `ProofData` fields — `new_account_state_hash`, `output_coins_root`, `input_nullifiers_root`, `coin_history_root`, `anchors_commitment`, `out_intent_commit` — **MUST** be the in-circuit-computed values above and are the proof's public inputs; the spender's sign-to-contract nonce binds `H(ProofData) = SHA-256(serialize(ProofData))` over all six ([§1.4](#14-identifiers-and-hashes), [§3.3](#33-off-chain-signature-handling)). Nothing else is public: amounts, asset ids, recipients (including the whole `output_templates` list — `out_intent_commit` is **hiding**, so it exposes neither the payees nor their amounts), keys, counts, the fresh blind `npk_rand` (hidden inside `next_pubkey_commit`, clause 2), the fresh blind `oic_rand` (hidden inside `out_intent_commit`, clause 2), and the underlying `anchors_root` (hidden inside its commitment, clause 1) remain in the witness (zero-knowledge).

10. **Received-coin admission (receive path).** All `received_coins[j].identifier` within one transition **MUST** be pairwise distinct (symmetric to the pairwise-distinct `nf_j` requirement of clause 4 for the spend side), so the same coin cannot occupy two `received_coins[]` slots of one transition and be credited twice by clause 7. Together with the proven-`0 → 1` admission of clause 8 (a duplicate would fail the second admission's prior-state-`0` proof), this closes both within-transition and cross-transition double-crediting of a received coin. For every `received_coins[j]` (empty for a transition that receives nothing), the circuit **MUST** check, using `received_auth[j]`:
    ...
    c. **Cross-account anchors binding (transitivity — always).** The witnessed `creating_anchors_opening = {r_root, r_rand}` opens the creating proof's commitment: `creating_proof.ProofData.anchors_commitment == Hc("AnchorsView", r_root ‖ r_rand)`; **and** `extends(r_root, w.anchors_root)` holds via `creating_anchors_extension` — the creating account's anchors view **MUST** be a prefix of this receiver's own anchors view. This is the binding that makes anchoring **transitive**: the receiver's own `anchors_root` is checked against a real scan just once ([§2.3.3 step 2](#233-receive)), and because the creating root must be a prefix of it, any fabricated (non-admitted) anchor leaf anywhere in the creating account's history — which would make `r_root` a non-prefix of every honest root — cannot be laundered across this hop. Without this clause a colluding intermediary could strip a forged anchor by exposing its own clean root; with it, the forgery propagates into `w.anchors_root` and the first honest downstream verifier rejects it.
    d. **Admission binding (non-mint path).** If `creating_proof.ProofData.input_nullifiers_root ≠ L_⊥` (the empty-list `NullifiersRoot`, [§1.7.5](#175-poseidon-merkle-tree-used-for-ocr-and-inr)) — i.e. the creating transition spent inputs and therefore required anchoring — then: the witnessed `creating_spend_record` parses to the canonical [§1.4](#14-identifiers-and-hashes) layout, its `message` — now `inr ‖ ocr ‖ next_pubkey_commit` — has its **leading** `inr ‖ ocr` equal to `creating_proof.ProofData.input_nullifiers_root ‖ creating_proof.ProofData.output_coins_root` (binding **exactly this** creating transition against any competing spend of the same inputs; the trailing `next_pubkey_commit` is the creating transition's own rotation commitment, already fixed by the anchored batch that admitted this record — the full 96-byte `message` is hashed into `Hc("BatchMember", creating_spend_record)` below, and the receiver treats the third segment as opaque bytes and never opens it — so it needs no separate check here), `Hc("BatchMember", creating_spend_record)` hashes via `member_path` to a `member_root`, and that `member_root` is a member of the anchors MMR at **`w.anchors_root`** (this receiving transition's own root) via `anchor_path` ([§1.7.10](#1710-anchors-mmr-normative)). It **MUST** be checked against `w.anchors_root`, not `r_root`: the creating spend is appended to the MMR only once its batch reaches `completed`, which is strictly *after* the creating transition fixed `r_root`, so `member_root ∈ MMR(r_root)` is unsatisfiable for every honestly-anchored coin — whereas `w.anchors_root` is the receiver's later, scanned-valid view ([§2.3.3 step 2](#233-receive)) and does contain it. Because the anchors MMR contains only **admitted, `completed`** batches and `w.anchors_root` is scanned-valid, this proves the creating spend was actually admitted on Bitcoin — in-circuit, in constant size, without exposing which batch. (Clause (c) independently carries the creating account's *receive* history via `extends(r_root, w.anchors_root)`; (d) anchors the creating *spend* itself.)
    e. **Mint path.** If `creating_proof.ProofData.input_nullifiers_root = L_⊥` (a mint — no inputs, no nullifiers, no anchor required, [§2.3.1](#231-mint--issuance), [§3.10](#310-transaction-states) `mint-verified`), checks (a)–(c) admit the coin; `creating_spend_record`/`member_path`/`anchor_path` are absent (a mint appends nothing to the anchors MMR, and (c) still binds the mint account's anchors view as a prefix of the receiver's).
    A coin admitted here becomes spendable in a **later** transition (clause 2(b) requires membership in the *prior* coin-history root); received coins never feed clause 3's conservation sums of the same transition. The out-of-circuit receive gates — anchoring state `completed`, live nullifier non-membership, decryption — remain the receiver's node-side checks of [§2.3.3](#233-receive); clause 10 is what folds a verified receipt into the account's own recursive lineage, so that a single current proof transitively attests every coin the account holds ([§2.2](#22-proof-types)).

The spender's **SpendRecord** (`message = inr ‖ ocr ‖ next_pubkey_commit`, [Foundations §1.4](#14-identifiers-and-hashes)) — an off-chain object — binds this transition's spent nullifier set (`input_nullifiers_root`), its produced coins (`output_coins_root`), and its rotated spend authority (via the hiding commitment `next_pubkey_commit`) through a BIP-340 signature by `Pkᵢ`, and its sign-to-contract nonce binds `H(ProofData)` so the off-chain validity proof is anchored to this record — and, because `out_intent_commit` is a `ProofData` field, that same nonce also binds the transition's **validated output intent**, so a witness-building node cannot redirect the outputs away from the templates the wallet authorised (clause 2), all with the 96-byte `message` unchanged. The publisher then aggregates many `SpendRecord`s into a `BatchBundle`, builds the `AggregateBatchProof` over them, and inscribes one constant-size `BatchInscription` on Bitcoin (committing `prev_root → new_root` and binding the bundle by content-address); construction and publishing are specified in [On-chain Layer](#3--on-chain-layer).
...
1. **Per-member soundness.** For each `j ∈ [1, m]`, the spender's per-account proof verifies under `C`'s verifier data, its public `ProofData` matches the member `SpendRecord`'s `message = inr ‖ ocr ‖ next_pubkey_commit` (its leading `inr ‖ ocr` against `ProofDataⱼ.input_nullifiers_root ‖ output_coins_root`; the trailing `next_pubkey_commit` is already tied to `ProofDataⱼ.new_account_state_hash` **inside** the per-account proof verified here — [§2.1 clause 2](#21-the-compliance-predicate) signs it and checks its opening against the `w.next_pubkey` clause 7 folds into `ash` — so `C_batch` need not re-derive it; the sixth `ProofData` field `out_intent_commit` is similarly tied to `ProofDataⱼ.output_coins_root` inside that same proof ([§2.1 clause 2](#21-the-compliance-predicate) opens it against the very `output_templates` clause 5 builds `ocr` from), so `C_batch` need not re-derive it either — it enters `C_batch` only through `H(ProofDataⱼ)` below), and the spender's BIP-340 signature verifies against `(Pkⱼ, messageⱼ)` over the full 96-byte `messageⱼ` (half-aggregated in-circuit is permissible — [On-chain §3.3](#33-off-chain-signature-handling)).
2. **Per-member sign-to-contract binding.** Each member's S2C tweak `t = H(R' ‖ H(ProofDataⱼ))` checks against that member signature's `Rⱼ` (the nonce point inside the `SpendRecord`'s 64-byte BIP-340 signature — an off-chain value; no spender signature ever reaches Bitcoin, [§3.3](#33-off-chain-signature-handling)), binding the per-account proof to the SpendRecord that referenced it. Because `H(ProofDataⱼ)` is taken over the full **six-field, 192-byte** `serialize(ProofDataⱼ)` ([§1.4](#14-identifiers-and-hashes)), this same tweak binds the member's `out_intent_commit` — hence its validated output intent (clause 1) — into the signature; `oic_rand` itself never reaches the publisher, so the batch stays as opaque as before ([§7.6](#76-publisher-interface-normative)).
3. **Nullifier-set integrity — member `nullifiers` opened to `inr`.** For each member `j`, the declared `SpendRecordⱼ.nullifiers` list is **not** free: the Poseidon Merkle root `NullifiersRoot(SpendRecordⱼ.nullifiers)` — computed by the [§1.7.5](#175-poseidon-merkle-tree-used-for-ocr-and-inr) construction with tag `"NullifiersRoot"` over that declared list (leaf `Hc("NullifiersRoot/Leaf", nfᵢ)`, padding to a power of two with the empty-leaf hash `L_⊥ = Hc("NullifiersRoot/Leaf", 0₂₅₆)`, internal node `Hc("NullifiersRoot/Node", …)`, and root `L_⊥` for the empty list) — **MUST** equal `ProofDataⱼ.input_nullifiers_root` (= `messageⱼ.inr`, already opened against the per-account proof in clause 1), and `SpendRecordⱼ.k` **MUST** equal `|SpendRecordⱼ.nullifiers|` (the pre-padding leaf count the root is taken over). This **opens** each member's committed `inr` to exactly the `nf` values its per-account proof proved it spent — [§2.1 clause 4](#21-the-compliance-predicate) already fixes `inr = NullifiersRoot(nf)` for `nf` correctly derived in-circuit from the account's bound `nk` — so a member cannot ship `nullifiers = []` / `k = 0` while its proof committed a non-empty `inr`, nor substitute a different `nf` set for the one `inr` commits to. (A genuine `k = 0` member — a batched mint whose `inr = L_⊥`, [§2.3.1](#231-mint--issuance) — passes trivially: `NullifiersRoot([]) = L_⊥ = inr`, `k = 0`, and it contributes no accumulator entry.) With every member list thus pinned to its own `inr`, the multi-set union of every member `SpendRecord`'s `nullifiers` equals exactly the batch's `batch_nullifiers` list, with no duplicates introduced (every `nf` appears at most once across all members of this batch — duplicates *across* batches are caught by the global accumulator at admission); `batch_nullifiers` is therefore the concatenated opening of each member's `inr` and can neither omit nor replace a nullifier the batch is bound to insert (clause 4).
...
Wallet:
  1. derive Pk₀ = sk₀·G and the rotated next_pubkey Pk₁ (both SPEND branch); draw a fresh
     npk_rand and form next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ Pk₁ ‖ npk_rand)
     (SHA-256, wallet-native, §1.4); validate the node's single output template in the
     awaiting_signature handshake (recipient == owner, the amount == the requested supply, §7.5),
     draw a fresh oic_rand, and form out_intent_commit =
     H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand) (SHA-256, §1.4);
     reconstruct H(ProofData) over that out_intent_commit and sign the transition
     signature BIP-340(sk₀, message = inr ‖ ocr ‖ next_pubkey_commit) with the sign-to-contract
     tweak binding H(ProofData)
     (a mint spends no coin, so inr is the empty-set root; signed by Pk₀, i.e. sk₀; current_pubkey
      rotates to Pk₁, which the signed message binds via the hiding commitment next_pubkey_commit;
      out_intent_commit binds the minted coin to recipient == owner, so a node cannot divert the
      new supply to itself)
  2. derive name_hash = H(name); asset_id = Hc("AssetId", genesis_tag ‖ Pk₀ ‖ name_hash ‖ decimals ‖ issuance_version=1)   (Foundations §1.4)
  3. provide next_pubkey Pk₁ (SPEND branch) and the fresh npk_rand and oic_rand so the node can build
     the clause-2 opening witnesses (the node already holds nk — operational bundle, Foundations §1.2)
...
Node / prover:
  4. build the witness with empty inputs, asset_issuance = {asset_id, creator_pubkey = Pk₀,
     ...
     {recipient = owner, amount, asset_id}; derive anchors_rand (§1.7.10) and set anchors_root to the current tip (or
     anr_empty for a first mint on a fresh network); anchors_commitment =
     Hc("AnchorsView", anchors_root ‖ anchors_rand) (§1.7.10)
...
Produces an off-chain mint object (the canonical §1.4 SpendRecord layout with k = 0):
  { public_key: Pk₀ (x-only), signature: BIP-340(sk₀, inr ‖ ocr ‖ next_pubkey_commit),
    message: inr ‖ ocr ‖ next_pubkey_commit (next_pubkey_commit commits Pk₁ under npk_rand), k: 0, nullifiers: [] (a mint spends nothing) }
...
**Output-intent binding for mints (normative).** A mint's output cannot be redirected by a witness-building node — batched or not. Two independent in-circuit bindings cover it: (1) like a send, the mint's output template is committed by the wallet's `out_intent_commit` (steps 1–3) and opened by [§2.1 clause 2](#21-the-compliance-predicate) against the coin clause 5 builds; this binding is enforced through the sign-to-contract re-check of `H(ProofData)` whenever the mint is **batched** (the publisher's `C_batch`, [§2.2 clause 2](#22-proof-types)). (2) The **no-input rule of [§2.1 clause 5](#21-the-compliance-predicate)** requires **every** active output of any `k = 0` transition — which a mint always is — to pay `recipient == prev_account_state.owner`, a pure per-slot field equality enforced in-circuit **independent of any batching or S2C path**. Binding (2) closes the case an S2C re-check cannot reach — an **unbatched** mint: even a mint the issuer never hands to a publisher (the off-chain "self-held supply" path above) has its output forced to the issuing account's own `owner`, so a malicious or compromised witness-building node **cannot** divert the minted supply to itself. The rule constrains nothing a legitimate mint does — its issued coin is created to `recipient = owner` by construction ([§6.5](#65-issuance--versioned-schemas-v1-minimal)); distributing newly-minted value to a third party is a separate, input-spending (`k ≥ 1`) **send** that is batched and covered by binding (1).
...
Wallet:
  1. in the awaiting_signature handshake (§7.5) the node surfaces inr ‖ ocr, the five node-computed
     ProofData digests, AND the full plaintext output_templates list it will build ocr from (payees,
     per-asset change, publisher-fee coin). VALIDATE it in plaintext (no Poseidon): the payee templates
     equal the wallet's own intent, the fee template goes to the quoted fee_address / fee_asset_id and its
     amount EQUALS the quoted fee exactly (§3.8), every change template has recipient == owner, and NO
     template has a non-owner recipient other than an intended payee or that single fee coin (so the node
     can neither over-charge the fee nor add an unexpected non-owner output). Only then draw a fresh oic_rand and form
     out_intent_commit = H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)
     (SHA-256, wallet-native, §1.4), and reconstruct H(ProofData) = SHA-256(ash ‖ ocr ‖ inr ‖ chr ‖ anc ‖
     out_intent_commit) over the wallet's OWN out_intent_commit (so the wallet never blind-trusts a
     node-supplied H(ProofData) for the value-bearing field)
  2. sign the single transition signature BIP-340(skᵢ, message = inr ‖ ocr ‖ next_pubkey_commit) with the
     current per-transition signing key skᵢ (whose Pkᵢ is current_pubkey; no per-coin key), applying the
     sign-to-contract tweak binding H(ProofData) from step 1, and committing the wallet's own rotated
     next_pubkey Pkᵢ₊₁ (step 3) under a fresh npk_rand as
     next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ Pkᵢ₊₁ ‖ npk_rand), the third message segment
  3. supply the rotated next_pubkey Pkᵢ₊₁ (SPEND branch, Foundations §1.2), the fresh npk_rand, and the
     fresh oic_rand (circuit witnesses for the clause-2 openings, never published); the node
     already holds nk (operational bundle) and derives the nullifiers
...
Node / prover:
  ...
  4. assemble the witness; per asset, add a change CoinTemplate {recipient = owner,
     amount = In(a) − (payee and fee outputs for a), asset_id = a} — clause 3 now REQUIRES equality
     In(a) + Mint(a) == Out(a), so this change coin is mandatory whenever the surplus is non-zero (a
     dropped surplus would burn value and is unsatisfiable); the equality forces the excess back to owner
  ...
  6. set anchors_root to the current anchors-MMR tip (§1.7.10 mandates tip-picking, so the
     opening leaks only proving time, not receive history) and derive anchors_rand; compute
     anchors_commitment = Hc("AnchorsView", anchors_root ‖ anchors_rand) (§1.7.10)
  7. run C as an AccountUpdateProof: recursive verify of prev_proof + anchors carry-forward
...
Produces the SpendRecord (off-chain, handed to a publisher; canonical §1.4 layout):
  { public_key: Pkᵢ (x-only), signature: BIP-340(skᵢ, inr ‖ ocr ‖ next_pubkey_commit),
    message: inr ‖ ocr ‖ next_pubkey_commit (next_pubkey_commit commits Pkᵢ₊₁ under npk_rand), k: |input_coins|, nullifiers: [nf for each input coin] }
...
CoinProof produced (per recipient coin, delivered off-chain):
  { coin, proof = π, inclusion_proof (membership in ocr), creating_prev_ash (= the ash of
    this transition's prev_account_state, §1.4), anchor_hint? (this batch's bundle_locator —
    known only after publisher hand-off; MAY follow the initial delivery),
    anchors_opening = {anchors_root, anchors_rand} (lets the recipient open this proof's
    anchors_commitment and check clause 10(c), §1.7.10), epk, ciphertext, detect_tag }
...
When the publisher's `BatchInscription` is admitted ([On-chain §3.6](#36-chain-scanning)), its `AggregateBatchProof` inserts every member's nullifiers into the global accumulator and the spent coins become unspendable again ([§3.7](#37-the-nullifier-accumulator)). The rotated spend key `next_pubkey = Pkᵢ₊₁` appears in the published `SpendRecord` **only** as the hiding commitment `next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)`, never in the clear — the fresh secret `npk_rand` (a per-transition witness the wallet never publishes, [§1.4](#14-identifiers-and-hashes)) is what makes it hiding. A `BatchBundle` scanner therefore reads the next record's `public_key = Pkᵢ₊₁` in the clear but **cannot** recompute `H("zkCoins/v1/NextPubkeyCommit" ‖ Pkᵢ₊₁ ‖ npk_rand)` without the secret `npk_rand`, so the rotation edge `next_pubkey(i) → current_pubkey(i+1)` that would otherwise chain an account's consecutive `SpendRecord`s stays **hidden** (the inscription itself reveals only the publisher's identity, not the spender's). The proof's fifth public input is likewise a **hiding** `anchors_commitment` with fresh randomness per transition ([§1.7.10](#1710-anchors-mmr-normative)) rather than a decodable anchors root. The publisher does receive the fee coin's `anchors_opening` (it must, to later fold the fee coin, [§3.8](#38-fees-and-economics)) — but because the spender **MUST** set `anchors_root` to the live tip ([§1.7.10](#1710-anchors-mmr-normative)), that opening reveals only the (globally shared) proving-time tip, not the account's private receive history; distinct transitions carry distinct commitments (the `anchors_commitment`, the `next_pubkey_commit`, and the `out_intent_commit`), so **from the published `SpendRecord`/`BatchBundle` alone neither a chain-only observer nor a `BatchBundle` scanner can link this record to the account's prior records** — the rotation edge is a hiding commitment, not the raw key. The **one** residual linkage is **off-chain and publisher-only**, through the fee coin the publisher receives: a fee `CoinProof` carries the spending account's `creating_prev_ash` (plaintext, [§1.5](#15-core-data-structures)) and the recursive proof whose public input `new_account_state_hash` is the account's **un-blinded** post-transition `ash` — it is **not** a hiding commitment, because the cyclic recursion must open it (a successor proof checks `prev_proof.new_account_state_hash == ash(prev_account_state)`, [clause 1](#21-the-compliance-predicate)). For two **state-consecutive** sends the *same* publisher anchors, the earlier fee proof's `new_account_state_hash` equals the later fee coin's `creating_prev_ash` (both are that account's `ash` after the earlier transition), so a publisher holding both fee coins can join them on that 32-byte edge and order that account's consecutive anchored spends. This linkage lives **only** in the fee `CoinProof` delivered to the publisher, never in public on-chain data ([Requirement 2](/requirements)'s on-chain guarantee is unaffected — a chain-only observer still learns nothing), and an intervening **pure receive** breaks the chain (it produces no fee coin and no `SpendRecord`, so the `ash` edge has no fee-coin endpoint there). Unlinkability *toward the publisher* is therefore an account choice: a wallet that wants it **rotates publishers per transition or self-publishes** ([§3.4](#34-the-publisher)), symmetric to a publisher that rotates `Pkₚ` for its own privacy ([§3.5](#35-inscription-format)). The new **`out_intent_commit`** introduces no leak: it is a **hiding** commitment (fresh secret `oic_rand`) and, being a `ProofData` **public input of the per-account proof**, it appears **only** inside that proof — carried as a witness inside the publisher's `AggregateBatchProof` and, encrypted, inside each recipient's `CoinProof` bundle (ZBE, [§4.2.1](#421-bundle-blob-encryption-zbe-normative)) — **never** as a field of the published, plaintext `SpendRecord`/`BatchBundle` (whose wire bytes are unchanged); `oic_rand` never leaves the wallet's own node. Delivery of the per-recipient `CoinProof` over Nostr is specified in [Transport & Recovery](#4--transport--recovery); delivery of the spender's `SpendRecord` to the publisher is specified in [Transport & Recovery §4.6](#46-data-availability--replication-factor-k).
...
Receiver / node:
  ...
  2. RE-VERIFY THE FULL RECURSIVE PROOF: C.verify(proof) under the canonical verifier data.
     ...
     ADDITIONALLY: open proof.ProofData.anchors_commitment with the anchors_opening the
     sender placed in the CoinProof bundle (§1.5) — check it commits (Hc("zkCoins/v1/
     AnchorsView", anchors_root ‖ anchors_rand)) — and verify that anchors_root is a valid
     historical anchors-MMR root per the receiver's OWN scan (§1.7.10). This one check is
     what makes the provenance's receive history trustworthy: every receive folded anywhere
     into the coin's past bound its creating batch into a root that clause 10(c) forced to be
     a prefix of this top-level anchors_root, so a single lookup against the verifier's own
     chain view validates all of them at once.
  ...
  4. anchoring: verify that output_coins_root is bound by a SpendRecord whose containing
     ...
     over message = inr ‖ ocr ‖ next_pubkey_commit verifies, the spender's nullifiers appear in the BatchBundle that
  ...
  5. nullifier non-membership: against the live accumulator at NAV(tip) (Onchain §3.7), verify
     the coin's own nf = Hc("Nullifier", nk ‖ coin.identifier) — derived with the RECEIVER'S own
     nullifier key nk — is NOT in the set. Only the owner account can spend a coin (§2.1
     clause 2(a)), and the owner's nk is bound to its account by nk_commit (§2.1 clause 4), so no
     valid spend can derive a different nf — this receiver-derived nf is the coin's only valid
     nullifier; the check detects re-crediting a coin this account has already spent (a recovery/replay case).
     Sender-side equivocation is caught at batch admission and by step 4, not here.
     The receiver MAY use Path A (maintain the accumulator
...
On all of 2–6 passing:
  7. RUN THE RECEIVE TRANSITION: execute C — as an InitialProof if this is the account's first
     ...
     empty account, anchors extended from anr_empty), otherwise as an AccountUpdateProof — whose
     received_coins[] contains the verified coin (clause 10). The node fetches the witness material
     from the anchoring batch (creating SpendRecord + member path, via anchor_hint or the §5.5
     anchoring trail), the creating proof's anchors_opening from the CoinProof bundle (§1.5), and
     its own anchors MMR (§1.7.10); it extends anchors_root to a scanned-valid root that is a
     superset of every received coin's creating root (clause 1 + clause 10(c)), credits balances
     (clause 7), and admits the coin to the coin-history SMT (clause 8). Several verified receipts
     MAY be folded into one transition (bounded by MAX_RX_COINS, §2.5), and a receive MAY be
     combined with a send in a single transition. The wallet signs the transition signature exactly
     as for a send (§7.5 proving handshake) — the same handshake forms out_intent_commit over the
     transition's output_templates (the empty list for a pure receive, which opens trivially and binds
     nothing; the send's validated templates when a receive is combined with a send, §2.1 clause 2 — that
     combined transition produces a batched SpendRecord, so C_batch re-checks its S2C and the output-intent
     binding holds); no SpendRecord is handed to any publisher (nothing
     to anchor — a pure receive spends nothing and its nullifier list is empty).
...
Steps 2 (recursive re-verification of the spender's per-account proof), 4 (chain anchoring: a `completed` `BatchInscription` whose `AggregateBatchProof` admits the spend) and 5 (the receiver's own-`nf` non-membership guard) are the checks that make receipt fully trustless: the receiver depends on **Bitcoin, the spender's recursive proof, and the publisher's AggregateBatchProof, never on the courier or any node's bare claim**. A failed or malicious transport can **withhold** a bundle but can never make an invalid one verify; a dishonest node can refuse to serve a Path-B Merkle path but cannot forge one against the on-chain root.
...
|---|---|---|
| Recursive verification + input authenticity (1, 2) | **No forgery, no output redirection** — a coin exists only as the signed, proven output of a valid prior transition; no party can fabricate a coin it was not entitled to; **and** a spend's outputs pay only where the wallet authorised — the wallet-computed `out_intent_commit` (clause 2), a hiding `ProofData` field bound by the same custody signature, ties the built output coins to the wallet's SHA-256-validated template list, so a witness-building node cannot recompute the Poseidon `ocr` behind the thin wallet's back and redirect a payment to itself; a no-input (`k = 0`) mint — the one transition that can skip the publisher's S2C re-check — instead has every output forced to `owner` in-circuit (clause 5); and the **input surplus** (change) is protected the same way — clause 3 now conserves value as an **exact equality** (`In + Mint == Out`), so a node can neither burn the excess (a short output is unsatisfiable) nor divert it — the exact-fee and no-unexpected-non-`owner`-output checks of the [§7.5](#75-node-rest-api-normative) handshake force it back to `owner` as change — so **no** transition's outputs, *and no input's surplus,* can be redirected ([§6.6](#66-threat-model-and-trust-configurations) custody holds against a malicious own or foreign node, without exception) | 3 · Trustless |
...
| Nullifier derivation (4) + batch admission + receive check 4 | **No double-spend** — each coin's `nf` is admitted into the global accumulator via a publisher's `AggregateBatchProof` and can enter the set only once; the proof binds each member's inserted `nf` set to its committed `input_nullifiers_root` (it checks `NullifiersRoot(SpendRecord.nullifiers) == inr`, [§2.2 clause 3](#22-proof-types)), so a member cannot suppress its own coin's `nf` by declaring an empty `nullifiers` list; a reused `nf` is rejected at batch admission (the `AggregateBatchProof`'s SMT-update would be inconsistent), and receive check 4 (§2.3.3) confirms the creating spend was actually admitted on-chain. The **fork** case is closed by the `nk_commit` binding (clause 4): `nk` is committed to the account, so two forks of one account cannot equivocate their nullifiers — a fork's spend of a shared coin is forced to the same `nf` and collides in the accumulator. Receive check 5 — the coin's `nf` derived with the receiver's **own** `nk` — additionally prevents re-crediting a coin the receiver has already spent | 3 |
| Received-coin admission (10) + anchors carry-forward (1) | **No fabricated receipts, transitively** — a coin enters an account's provable holdings only with its creating proof verified in-circuit, its `(recipient, asset_id, amount)` bound by recomputing `coin.identifier` against the creating `output_coins_root` (clause 10(b) — so a coin can be credited **only** by its committed `recipient` and **only** for its committed `amount`, closing both cross-account duplication and receive-time inflation), **and admitted at most once** (clause 10's pairwise-distinct `received_coins[]` identifiers plus clause 8's proven `0 → 1` non-membership→insert — so a single received coin cannot be credited twice, keeping the §5.7 balance attestation honest), and (for non-mints) its creating `SpendRecord` bound, via the anchors MMR, to an admitted `completed` batch; clause 10(c) forces the creating account's anchors view to be a **prefix** of the receiver's own, and clause 1 forbids any rewind, so a colluding chain of holders cannot strip a fabricated anchor at an intermediate hop — the forgery propagates into the top-level `anchors_root`, which the single §2.3.3 step 2 scan check exposes | 3, 9 |
...
| `MAX_ANCHORS_MMR_DEPTH` | **48** | maximum anchors-MMR peak height supported by the in-circuit membership/extension gadgets ([§1.7.10](#1710-anchors-mmr-normative)); 2⁴⁸ admitted batches is far beyond any realistic lifetime |
| `MAX_BATCH_MEMBER_DEPTH` | **16** | depth of the in-circuit `member_path` gadget in clause 10(d) (`= ⌈log₂ MAX_BATCH_MEMBERS⌉`, [§2.5 `C_batch`](#25-circuit-dimensioning-normative)) |
...
`C`'s public inputs are the six `ProofData` fields (§2.1 clause 9): the five Poseidon digests (`ash`, `ocr`, `inr`, `chr`, `anc`) = **20 field elements** (four per digest, §1.7.1), plus **`out_intent_commit`**, a 32-byte SHA-256 value carried as **eight big-endian 32-bit limbs** (8 field elements, each `< 2³² < p`, so its 32 bytes reconstruct canonically — the SHA-256 output needs finer limbs than a Poseidon digest's four 8-byte limbs, whose 8-byte groups can exceed `p`; the limb width is a §1.7.8 reference-instantiation choice) = **28 field elements** total, plus the cyclic verifier-data public inputs Plonky2 appends (`add_verifier_data_public_inputs`: the circuit digest, 4 elements, and the `constants_sigmas_cap`, `num_cap_elements()` digests at `cap_height = 4`). A conforming verifier reads `ProofData` from the first 28 public-input elements and checks the appended verifier-data elements against the pinned `circuit_digest(C)` (§1.7.9). Unused `received_coins[]` slots follow the same inactive-sentinel discipline as the input/output slots above (an inactive receive slot verifies a canonical dummy proof via Plonky2's `conditionally_verify_cyclic_proof_or_dummy` and contributes nothing to balances or the coin-history update).
...
- **Leaf mode** covers **one** member. It verifies that member's per-account proof under `C`'s verifier data (non-cyclically — different verifier data), checks its `SpendRecord` binding and sign-to-contract tweak ([§2.2 clauses 1–3](#22-proof-types)) — including the opening bind `NullifiersRoot(SpendRecord.nullifiers) == ProofData.input_nullifiers_root` and `k == |nullifiers|` (clause 3, a purely per-member check the leaf is the **only** node that can make: `ProofDataⱼ`/`SpendRecordⱼ` are witness here, whereas an inner node only composes children) — then applies that member's nullifier SMT insertion(s) advancing `prev_root → new_root`, sets `m = 1`, and sets `member_root = Hc("BatchMember", serialize(SpendRecord))`.
...
**SHA-256, in-circuit.** SHA-256 (`H`, §1.1) appears in-circuit in five places: (a) **inside the BIP-340 verification itself** — BIP-340 uses tagged SHA-256 for its challenge `e = H_BIP340(R ‖ Pk ‖ message)` (§1.1), so every in-circuit signature check above already computes a tagged-SHA-256 once per signature (once per spender in `C`, once per member in `C_batch`); (b) the S2C tweak check `t = H(R' ‖ H(ProofData))` per member ([§2.2 clause 2](#22-proof-types)) — one `H(ProofData)` over the **192-byte** `serialize(ProofData)` ([§1.4](#14-identifiers-and-hashes)) plus the tweak hash, per member; (c) the mint-path binding `H(creator_pubkey ‖ nk_commit) == owner` ([§2.1 clause 3(b)](#21-the-compliance-predicate), [§6.5](#65-issuance--versioned-schemas-v1-minimal)) — one SHA-256 per mint; (d) the genesis owner-binding `H(txn_pubkey ‖ nk_commit) == owner` ([§2.1 clause 1](#21-the-compliance-predicate), InitialProof branch) — one SHA-256 per account at genesis, the check that closes genesis nullifier equivocation (§1.4); and (e) the **output-intent opening** `out_intent_commit == H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)` ([§2.1 clause 2](#21-the-compliance-predicate)) — one SHA-256 in `C` per transition, over a variable-length preimage of `26 + 80·(active output slots) + 32` bytes (at most `26 + 80·MAX_TX_OUTPUTS + 32`, §2.5), padded in-circuit by the same inactive-slot discipline as `serialize(AccountState)` ([§1.7.4](#174-serializeaccountstate)); it also feeds (b) as the sixth `serialize(ProofData)` field. For a **mint** InitialProof (d) coincides with (c) (`txn_pubkey = creator_pubkey = Pk₀`); for a **non-mint** InitialProof (an account that receives before it ever mints — the common case) it is a **standalone** check, so an implementer **MUST NOT** optimise it away with (c). The reference instantiation uses a standard Plonky2 SHA-256 gadget; (a) and (b) together mean SHA-256-in-circuit cost scales with the signature count (per member), which is the second-largest cost after the foreign-field EC arithmetic. **Every other hash in the protocol is Poseidon** (`Hc`), which is field-native and cheap; SHA-256-in-circuit is confined to these signature/identity checks where Bitcoin-key compatibility (§1.1) requires it. (ECDH, NIP-44, and `K_tx`/`detect_tag` derivation are **host-side** in the node, never in-circuit — [§1.3](#13-per-coin-keys-note-encryption--detection), §4 — so they impose no circuit cost.)

**Wide-integer balance arithmetic (multi-limb).** `amount` is a `u128` (§1.7.3), which the Goldilocks base field (`p ≈ 2^64`, §1.1) cannot hold in a single element, so amounts are carried in-circuit as fixed multi-limb integers. The per-asset conservation of [§2.1 clause 3](#21-the-compliance-predicate) accumulates `In(a)`, `Out(a)`, and `Mint(a)` over up to `MAX_TX_INPUTS`/`MAX_TX_OUTPUTS` terms (§2.5), so those sums and the `In(a) + Mint(a) == Out(a)` comparison are computed by **wide multi-limb integer gadgets** over a fixed width of at least `128 + ⌈log₂ max(MAX_TX_INPUTS + 1, MAX_TX_OUTPUTS)⌉` bits (132 bits at the v1 bounds), with each limb range-checked and carries propagated so no term or partial sum wraps. A conforming implementation MAY use any gadget that computes the identical exact-integer relation; the *relation* (exact non-negative-integer accumulation and an exact `==` equality comparison) is fixed, the *gadget* is not — the same discipline as the two operations above. This arithmetic is cheap next to the foreign-field Schnorr and SHA-256 (a handful of range-checked limb additions per transition), but it is normative: a field-native or `u128`-modular sum would let conservation be satisfied by a **wrapped** total, creating spendable value from nothing, and is non-conforming ([§2.1 clause 3](#21-the-compliance-predicate)).

**Cost, feasibility, and the review gate (normative note).** The foreign-field Schnorr verification and the in-circuit SHA-256 dominate `C` and `C_batch` proving time; the recursion overhead (§1.7.9), the Poseidon SMT updates, and the multi-limb balance arithmetic are comparatively cheap. Each active `received_coins[]` slot ([§2.1 clause 10](#21-the-compliance-predicate)) adds one cyclic proof verification (recursion-overhead class) plus cheap Poseidon paths (member tree, anchors MMR); `MAX_RX_COINS` (§2.5) bounds that cost. Concrete gate counts and proving times at the §2.5 bounds (`MAX_TX_INPUTS/OUTPUTS = 8`, `MAX_BATCH_MEMBERS = 2^16`) **MUST** be measured by the reference implementation and recorded alongside the §1.7.8 review; the in-circuit/out-of-circuit boundary fixed here is explicitly within that review's scope. Should foreign-field Schnorr prove impractical at these bounds, the resolution is a **version bump** (e.g. a Goldilocks-native signature scheme) — never a silent change; v1 fixes secp256k1/BIP-340 because address and key compatibility with Bitcoin ([Requirement 1](/requirements), §1.1) is a hard protocol requirement.
...
Each member `SpendRecord` ([Foundations §1.5](#15-core-data-structures)) inside a `BatchBundle` ([Transport & Recovery §4.6](#46-data-availability--replication-factor-k)) carries its own spender's BIP-340 signature over `message = inr ‖ ocr ‖ next_pubkey_commit`, sign-to-contract-bound to that spender's own off-chain validity proof via the S2C tweak `t = H(R' ‖ H(ProofData))` with the canonical `H(ProofData) = SHA-256(serialize(ProofData))` of [§1.4](#14-identifiers-and-hashes) and the standard `R = R' + t·G` tweak. The publisher's `AggregateBatchProof` ([Proofs §2.2](#22-proof-types)) verifies *every* member signature **in-circuit** — so the on-chain `BatchInscription` does not separately carry any spender signature.

Half-aggregation of those member signatures inside the aggregate-proof witness is an implementation-level optimisation: a publisher MAY batch `m` BIP-340 checks into one multi-scalar test, deriving aggregation coefficients `aⱼ = H( z ‖ u32-be(j) )` (big-endian, consistent with [§1.7.2](#172-field-encoding-e-of-hc-inputs)) from a per-batch transcript `z = H("zkCoins/v1/HalfAgg" ‖ R₁ ‖ Pk₁ ‖ m₁ ‖ … ‖ R_m ‖ Pk_m ‖ m_m)`, computing `s_agg = Σⱼ aⱼ·sⱼ`, and proving in-circuit that `s_agg·G == Σⱼ aⱼ·(Rⱼ + eⱼ·Pkⱼ)`. This shrinks the aggregate-proof witness without changing any on-chain field. A non-aggregating publisher (verifying each `m` signature directly in-circuit) is equally valid and produces the same `BatchInscription`.
...
------  ----  -----------------------------------------------------------
  ...
  2       1   version                   = 0x02                        (batched-inscription payload; 0x01 was
                                                                       the retired pre-batch per-spend payload
                                                                       of earlier drafts and MUST be rejected)
...
2. **Parse and bound-check.** Decode the 231-byte fixed-format payload (§3.5). Reject any payload failing the structural check, and reject any inscription violating the §3.5 `block_anchor` bound (strict ancestor of the inclusion block; gap ≤ `N = 100`).
...
5. **Fetch the bundle.** A `BatchBundle` is a plaintext blob served by Blossom **only** by its SHA-256 address `blob_id = H(serialize(BatchBundle))` ([§1.4](#14-identifiers-and-hashes), [§4.2.1](#421-bundle-blob-encryption-zbe-normative), [§7.4](#74-blossom-blob-store-normative)) — **not** by the Poseidon on-chain `bundle_locator`. The scanner therefore first resolves `bundle_locator → blob_id` from any node holding the batch — its `/v1/chain/inscriptions` index returns `blob_id` alongside the on-chain `bundle_locator` ([§7.5](#75-node-rest-api-normative)) — then fetches the `BatchBundle` blob by `blob_id` from the relay mesh ([Transport & Recovery §4.6](#46-data-availability--replication-factor-k)) and verifies `H(serialize(BatchBundle)) == blob_id` (content-address self-check, [§7.4](#74-blossom-blob-store-normative)). The `bundle_locator` is the on-chain **binding** value the next step verifies against, not a Blossom key. The node MAY retry across replicas; the `k = 3` replication target makes one reachable holder sufficient. If no holder is reached within the node's bundle-fetch deadline, the inscription is left in state `pending` (§3.10) and re-tried on a back-off schedule.
...
7. **Verify the aggregate proof.** Verify the `AggregateBatchProof` ([Proofs §2.2](#22-proof-types)) against its public inputs `(prev_root, new_root, m, member_root)` — `prev_root`/`new_root` are the inscribed roots, and `m`/`member_root` are the values recomputed from the bundle in step 6 (so `bundle_locator` re-derives to the inscribed value). Also check the proof's embedded verifier-data digest equals the pinned `circuit_digest(C_batch)` for this network (§1.7.9). The proof is a single recursive PCD attestation that every member spender's transition is valid under its own per-account recursive proof — which fixes that member's `input_nullifiers_root` as the `NullifiersRoot` of nullifiers correctly derived from the account's bound `nk` (`nf = Hc("Nullifier", nk ‖ coin.identifier)`, [§2.1 clause 4](#21-the-compliance-predicate)) — that each member's declared `SpendRecord.nullifiers` **opens** that committed root (`NullifiersRoot(SpendRecord.nullifiers) == input_nullifiers_root`, [§2.2 clause 3](#22-proof-types)), that `member_root` commits to exactly the ordered member set, and that `new_root` equals the ordered SMT-insertion of exactly those opened member nullifiers into `prev_root`. The `member_root` binding closes the gap a `(prev_root, new_root)`-only public input would leave: a publisher cannot present two distinct bundles with the same accumulator delta under the same proof. A proof-verification failure MUST cause the inscription to be discarded.
8. **Apply the transition.** Update the local accumulator from `prev_root` to `new_root`. The list of inserted nullifiers (carried in the bundle) is added to the node's per-`nf` index used to serve membership and non-membership queries (§3.7); each member's `nullifiers` are exactly the set that step 7's aggregate proof pins to that member's `input_nullifiers_root` (`NullifiersRoot(SpendRecord.nullifiers) == inr`, [§2.2 clause 3](#22-proof-types)), so the index the node builds is the same nullifier set the attested `new_root` inserts. The bundle's member `SpendRecord`s are persisted to the node's data store for receiver lookup and for serving the same `BatchBundle` to future verifiers that fetch it by its `blob_id`. Once the inscription reaches state `completed` ([§3.10](#310-transaction-states)), the batch's `member_root` is appended to the node's **anchors MMR** and the resulting root indexed ([§1.7.10](#1710-anchors-mmr-normative)).
...
- **Path B — light-client: hold only the on-chain roots, delegate the lookup.** Follow the inscribed `new_root` sequence (231 bytes per batch on-chain), then ask any Path-A node for an SMT path of `nf` against the current `new_root`. The path is 256 siblings — 8 KiB of raw sibling data (~17 KB as the hex-encoded [§7.5](#75-node-rest-api-normative) JSON); the mostly-empty-subtree siblings compress to ~512 bytes where an implementation stores or transports it compactly — and it is self-verifying against the root the light client already knows from Bitcoin. The answering node may serve either an **inclusion path** (proving `nf` is present at the queried key, leaf hash `H_leaf(1)`) or a **non-inclusion path** (proving the queried key is empty, leaf hash `H_leaf(0)`); **both** hash up to the same on-chain `new_root` under the §1.7.6 SMT construction, so the light client's verification is identical in either direction — recompute the path and check equality with `new_root`. Forging either direction against a known root would require breaking the Poseidon hash assumption (second-preimage resistance on internal nodes plus per-level domain separation). Querying multiple independent nodes hardens against denial of service; the correctness check is single-node-honest.
...
2. **Spender includes a fee coin.** When building the transition (§2.3.2) the spender adds one extra output `CoinTemplate { recipient = fee_address, amount == fee, asset_id = fee_asset_id }` — the fee coin's `amount` **equals** the quoted `fee` **exactly**, not merely at least it. (An `≥` form let a malicious witness-building node route the whole input surplus to the publisher-controlled `fee_address` as an inflated "fee", stealing the change; an over-payment / "tip" is not a specified feature and would reopen exactly that lever, so the amount is exact. The wallet enforces this exactness in the [§7.5](#75-node-rest-api-normative) handshake and the publisher re-checks it below.) This fee coin occupies one of the `MAX_TX_OUTPUTS` slots ([§2.5](#25-circuit-dimensioning-normative)) and is conserved by the balance predicate ([§2.1 clause 3](#21-the-compliance-predicate)) like any other output — it is indistinguishable on-chain from a payment, and it shares the transition's single `ocr`.
3. **Hand-off.** The spender hands the publisher its `SpendRecord` **and** the fee coin's `CoinProof` bundle (encrypted to `fee_address`'s `IVPK`, §4.2) — the publisher is just another recipient for that one coin. The publisher verifies, before batching, that the fee coin (a) is addressed to its `fee_address`, (b) is of `fee_asset_id`, (c) has `amount` **equal to** its quoted `fee` (exactly), and (d) is an output under the **same `ocr`** the `SpendRecord` signs; only then does it include the `SpendRecord` in a `BatchBundle`.
...
**Receivers SHALL act only on `completed` (or `mint-verified` where applicable).** The anchor / receive checks in [Proofs §2.3.3](#233-receive), [Transport & Recovery §4.5](#45-recovery), and [Access & Explorer §5.6 / §5.7](#56-shareable-confirmation-links) all require the relevant `BatchInscription` (and hence its member `SpendRecord`) to be in state `completed`; an inscription in any other state **MUST NOT** be treated as anchored. The sole exception is the `mint-verified` status defined below for non-batched mints, which substitutes direct re-verification of the mint's recursive proof (an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint, [§2.3.1](#231-mint--issuance)) for the chain-anchor check. The user-facing **status** rendered by an explorer (e.g. Access & Explorer §5.6 step 3) **MUST** be the §3.10 state (one of `completed`, `failed`, `pending`, or `mint-verified`), not a node-asserted classification.

**Mints not anchored on-chain.** For a mint coin whose issuer chose not to anchor it ([§2.3.1](#231-mint--issuance)), no `BatchInscription` exists and the three §3.10 states do not apply. Such a coin is classified as **`mint-verified`** instead: the receiver re-verifies the mint's recursive proof (an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint) directly and accepts on success; there is no double-spend protection to anchor (a mint introduces no nullifier) and the cryptographic safety of the credit is identical. Explorers and receivers **MUST** render the `mint-verified` status as distinct from `completed`/`pending`/`failed` so the user understands it has no on-chain anchor, and any subsequent spend of this mint coin is anchored normally — the spend's `SpendRecord` reaches the accumulator through a `BatchInscription` just like any other.
...
1. **Re-derive keys.** From the seed, re-derive the account root `A` and thereby `ivk`, `ovk`, `op`, the nullifier key `nk`, `op_secret` (the anchors-randomness key, §1.7.10), and the spend keys ([Foundations §1.2](#12-key-hierarchy)). This alone restores the address/identity (`address = H(Pk₀ ‖ nk_commit)` with `nk_commit = Hc("NkCommit", nk)`, both re-derived here — §1.4), decryption ability, the deterministic detection tags, and — via `op_secret` — the deterministic `anchors_rand` needed to rebuild prior anchors openings.
2. **Rebuild the public index from Bitcoin + bundles.** Scan Bitcoin for zkCoins `BatchInscription`s, fetch each batch's `BatchBundle` from the relay mesh ([§4.6](#46-data-availability--replication-factor-k)) by its Blossom `blob_id` (resolved from the on-chain `bundle_locator` via a holder's inscription index, then bound by recomputing `bundle_locator`, [§3.6 steps 5–6](#36-chain-scanning)), verify the publisher's `AggregateBatchProof`, and apply the inscribed `prev_root → new_root` transition. The resulting global **nullifier accumulator** ([Foundations §1.6](#16-trees-one-global-structure-one-per-account-structure)) is derived from confirmed Bitcoin data plus content-addressed, recursively verifier-checkable bundles, and requires no trust in any peer. Because each member `SpendRecord` inside a bundle carries the spender's rotating public key `Pkᵢ` and the operator's seed re-derives the spender side of all its own past transitions, the operator can privately recognise its **own** member SpendRecords (a chain-only third party cannot link them — it sees only the publisher's identity on-chain; the account's *used* publisher, which additionally holds the fee coins, can order that account's state-consecutive anchored spends via the fee coin's `ash` edge, [§2.3.2](#232-send), unless the wallet rotates publishers or self-publishes) and so reconstruct the skeleton of its own activity. Recovery is therefore conditional on bundle DA: if every replica of a past `BatchBundle` has been lost, the operator can still verify the on-chain root continuity but cannot reconstruct the member set of that batch on its own — in practice the `k = 3` replication and operator backups ([§4.6](#46-data-availability--replication-factor-k)) make this a rare degraded case.
...
3. **Node verifies (and persists).** The node `detect_tag`-matches (§4.4), fetches the blob, **persists it** (§4.8), decrypts with `K_tx` (re-derived from `ivk`, [§1.3](#13-per-coin-keys-note-encryption--detection)), and **verifies** the recursive proof (incl. `anchors_root`) and nullifier non-membership against the live accumulator (§2.3.3), then folds the coin in via the receive transition ([§2.3.3 step 7](#233-receive)). Only a verified coin is credited.
...
Recall the relevant key material from [Foundations §1.2](#12-key-hierarchy): a subject's identity is its `address = H(Pk₀ ‖ nk_commit)` ([§1.4](#14-identifiers-and-hashes)); the **operational key** `op` is the node-held Nostr/identity key that signs grants and acknowledgements but cannot spend; `ivk`/`ovk` are the viewing keys; and `K_tx` ([§1.3](#13-per-coin-keys-note-encryption--detection)) is the per-coin note key that decrypts exactly one coin. The on-chain `BatchInscription` ([§1.4](#14-identifiers-and-hashes), [§3.1](#31-the-on-chain-object)) is the only object written to Bitcoin and the integrity anchor for everything below; the per-spender `SpendRecord` lives inside the publisher's off-chain `BatchBundle` ([§4.6](#46-data-availability--replication-factor-k)).
...
3. **Verify against Bitcoin.** Check the coin's inclusion in `output_coins_root`; locate the spender's `SpendRecord` carrying that `ocr` inside the publisher `BatchBundle` it was batched into, fetch the corresponding `BatchInscription` (via the bundle's `bundle_locator`), and confirm the inscription is in state **`completed`** ([On-chain §3.10](#310-transaction-states)); verify the spender's recursive validity proof ([Foundations §1.4, §1.5](#14-identifiers-and-hashes)) — opening its `anchors_commitment` with the bundle's `anchors_opening` and checking the `anchors_root` against a node's anchors index ([§1.7.10](#1710-anchors-mmr-normative)) — and (optionally, for full chain-anchor verification) the publisher's `AggregateBatchProof` ([§2.2](#22-proof-types)). For a coin produced by a **non-batched mint** ([§2.3.1](#231-mint--issuance)), there is no `BatchInscription` to anchor against; the explorer instead verifies the mint's recursive proof (an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint) directly and renders the **`mint-verified`** status ([§3.10](#310-transaction-states)) — the user MUST be shown this status as distinct from `completed`, so they understand the coin has no on-chain anchor. The viewer trusts **Bitcoin and the proofs — never the explorer's assertion**.
...
- **Scoped.** It discloses that single transaction in full and **nothing else** — no other transactions, no balances, no counterparties beyond that payment, and no spend authority. It does reveal `coin.recipient` (B's address) for *this* payment, and — through the bundle's `anchors_opening` — the sender's proving-time tip (a global value, not its receive history, because §1.7.10 mandates tip-picking); per-relationship unlinkability is an account choice ([Foundations §1.2](#12-key-hierarchy)).
...
The narrowest *account-level* disclosure proves a balance **without exposing the account's transaction history**. The subject produces a zero-knowledge proof that its on-chain-committed account state holds a given balance of one asset, and hands over only that proof. It reveals the address, the asset, the number, and the public `anchor` below — never any coin plaintext, counterparty, or amount-flow, and **not** the account's receive-recency (the anchors view stays hidden behind its commitment; the attestation exposes only a **global** MMR ceiling, not the subject's own prefix length). The anchor is a genuine metadata disclosure and a documented limit of this design; see *Properties* below.
...
BalanceAttestation:
  public inputs (revealed):
    { subject : address,
      asset_id,
      balance : B,
      anchors_ceiling,                               // a GLOBAL anchors-MMR root (the network tip
                                                     //   at attestation time) — NOT the subject's
                                                     //   own prefix; verifier checks it against
                                                     //   its own scan (§1.7.10)
  ...
  witness (hidden):
    { AccountState S,
      ...
      anchors_opening = { anchors_root, anchors_rand },  // opens pi.ProofData.anchors_commitment
      anchors_extension,                              // extends(anchors_root, anchors_ceiling)
  ...
  statement (domain tag "zkCoins/v1/BalanceProof"):
    ...
    7. pi.ProofData.anchors_commitment == Hc("AnchorsView", anchors_root ‖ anchors_rand)
       AND extends(anchors_root, anchors_ceiling)    (the attested state's hidden anchors view is a
                                                      prefix of the disclosed global ceiling — proving
                                                      its receive history is admitted, WITHOUT revealing
                                                      the subject's own prefix length)
```

The verifier checks the proof, that `anchors_ceiling` is a valid historical anchors-MMR root per its **own** scan ([§1.7.10](#1710-anchors-mmr-normative) — since the subject's own `anchors_root` is proven a prefix of it, every receive folded into the attested state is admitted), and that the `BatchInscription` at `anchor` (`txid`) — which contains the `bundle_locator` whose `BatchBundle` carries `SpendRecordⱼ` at position `j` — is in state `completed` ([On-chain §3.10](#310-transaction-states)) at `{block_hash, height}`. No node, relay, or explorer is trusted. Because `anchors_ceiling` is a **global** MMR root shared by every account, it discloses nothing account-specific; the subject **MUST** set it to a recent global root chosen independently of its own view — RECOMMENDED the network tip at attestation time — and **MUST NOT** set it to (or derive it from) its own `anchors_root`, which would leak the subject's prefix length. A verifier that wants a freshness bound **MAY** prescribe the `anchors_ceiling` it will accept (e.g. its own current tip); the subject then proves `extends(anchors_root, that ceiling)`, and the verifier learns only that the state is no fresher than a root it already holds. Because the anchor is a `SpendRecord`, the attestation binds the balance **as of the anchored spend used as `anchor`** — at freshest, the account's most recent anchored spend; balance changes made by later receive transitions (which produce no `SpendRecord`) become attestable only once the account's next anchored spend binds the newer state.
...
- **Reveals the number, plus its anchor.** No balance-changing transaction, coin amount, counterparty, history, or receive-recency leaks — the witness never leaves the proof, and `anchors_ceiling` is a global root, not the subject's own prefix. The public `anchor = {txid, j, …}` does identify the **one** anchoring `SpendRecord` (its batch, position `j`, and on-chain time) so the verifier can check `completed`; this is inherent to standing on a Bitcoin anchor and is the same `SpendRecord` a payee of that transition already sees. This is a documented **v1 limit**: a future protocol version **MAY** replace the public `anchor` with a zero-knowledge **set-membership proof** over the inscribed batches — proving the attested state stands on *some* admitted `completed` batch without naming which — closing this disclosure; a version that does so is the planned upgrade referenced by [Requirement 9(b)](/requirements). v1 documents the disclosure instead.
...
- **Unforgeable for a third party.** Producing it requires the account's Private `AccountState` (hence its view data); no one can attest a balance for an address whose state they cannot see, and the statement can only ever prove the true committed value. The attested `balances` figure is bound to the coins the account **genuinely** received: clause 7 credits an amount only for a `received_coins[]` entry that passed clause 10's pairwise-distinctness check and clause 8's proven `0 → 1` (non-membership→insert) admission, so the number cannot be inflated by crediting a single received coin twice ([§2.1 clauses 8, 10](#21-the-compliance-predicate)).
...
**Flow.** The explorer holds `ivk` ([Foundations §1.3](#13-per-coin-keys-note-encryption--detection)), finds the account's coins by scanning the relay mesh ([Transport & Recovery](#4--transport--recovery)) and recomputing each candidate's `detect_tag` from `ss = ECDH(ivk, epk)` (a `;h=<locator>` fragment hint, if present, only speeds resolution), decrypts incoming coins with `ivk` and recovers outgoing-coin plaintext with `ovk` — opening each self-delivered record's `out_ciphertext` via `K_out = HKDF("zkCoins/v1/OutKey", ovk ‖ epk)` ([§1.3](#13-per-coin-keys-note-encryption--detection)) — and renders the full history (under the 32-byte `ivk`-only form no `ovk` is present: outgoing-coin recovery is skipped and only the incoming side of the history is rendered) — checking every transaction against Bitcoin (coin inclusion → containing `BatchBundle` → `completed` `BatchInscription` ([On-chain §3.10](#310-transaction-states)) → recursive proof, as in [§5.6](#56-shareable-confirmation-links)). For non-batched mint coins, the chain step is replaced by direct re-verification of the mint's recursive proof (an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint) and the entry is rendered with `mint-verified` status, as in §5.6. The explorer is never trusted.
...
- Sign each transition — produce `BIP-340(skᵢ, message)` over `message = inr ‖ ocr ‖ next_pubkey_commit` (Foundations §1.4; the wallet commits its own rotated `next_pubkey` under a fresh `npk_rand`) with the sign-to-contract tweak that binds the spender's off-chain `ProofData`. The node derives the nullifiers `nf = Hc("Nullifier", nk ‖ coin.identifier)` from the operational bundle's `nk` while building the witness ([§2.1 clause 4](#21-the-compliance-predicate)). The resulting `SpendRecord` is handed to a publisher off-chain ([Transport & Recovery §4.6](#46-data-availability--replication-factor-k)); the wallet does **not** itself touch Bitcoin.
...
| **relay.\*** | any ↔ node (Nostr) | NIP-44 / NIP-59 envelope; `detect_tag` for `CoinProof` discovery; Blossom `blob_id` for `BatchBundle` fetch (resolved from the on-chain `bundle_locator`, which binds the fetched bundle, [§3.6 steps 5–6](#36-chain-scanning)) | publish/fetch off-chain bundles (per-coin and per-batch), gift-wrapped delivery, note discovery, k-replication | [Transport & Recovery](#4--transport--recovery) |
...
Custody is **cryptographically safe in every configuration**: no node holds a SPEND-branch key (Foundations §1.2), value integrity is enforced by proof soundness and the nullifier accumulator, and every spend's nullifier reaches the accumulator only via a publisher `AggregateBatchProof` anchored by an immutable on-chain `BatchInscription`. This safety extends to the node that **builds the witness**: because the thin wallet cannot recompute the Poseidon `output_coins_root` it signs, it binds its **validated output intent** with a self-computed SHA-256 `out_intent_commit`, opened in-circuit by the compliance predicate ([§2.1 clause 2](#21-the-compliance-predicate), [§7.5](#75-node-rest-api-normative)) — so a malicious or compromised witness-building node (own or foreign, including a hosted "public wallet API" account, [§6.1](#61-components-and-responsibilities)) **cannot** redirect **any** transition's outputs to a recipient the wallet did not authorise, nor burn or divert the transition's **input surplus**, even though it assembles the proof: a value-spending transition's outputs are bound by `out_intent_commit` (re-verified by the publisher's `C_batch`, [§2.2 clause 2](#22-proof-types)), a no-input mint's — the one path that skips `C_batch` — by the in-circuit `k = 0 ⟹ recipient == owner` rule ([§2.1 clause 5](#21-the-compliance-predicate)), and the input surplus by clause 3's **exact-equality** conservation (`In + Mint == Out`, no burn) together with the [§7.5](#75-node-rest-api-normative) handshake's exact-fee and no-unexpected-non-`owner`-output checks, which force the excess back to the account as change to `owner` (a node can neither destroy it nor reroute it to the publisher-controlled `fee_address` as an inflated fee). The three wallet–node configurations differ only in **privacy** and in **whom you trust for correctness and availability** — never in custody:
...
- **Own wallet + a single foreign node.** Plaintext disclosed to it; you trust it for correctness and liveness (it can lie or omit), but it **cannot** steal (including by redirecting a spend's outputs — the wallet's `out_intent_commit` binds them, [§2.1 clause 2](#21-the-compliance-predicate) — or by burning/diverting the input surplus, which clause 3's exact equality plus the [§7.5](#75-node-rest-api-normative) exact-fee / no-extra-output checks force back to `owner` as change), forge, or double-spend; custody safe.
...
**Freeze-resistance ([Requirement 3](/requirements)).** No node or publisher can **freeze** — indefinitely block the spending of — coins it does not own. Publishing is **permissionless**: any participant MAY run a publisher and a wallet/node MAY act as its **own** publisher ([§3.4](#34-the-publisher)), so a publisher that censors a `SpendRecord` or sits on it collects nothing and is simply **bypassed** — the censored spender re-submits to another publisher or self-publishes ([§3.4](#34-the-publisher)). On the read side, symmetrically, **no node can lock a wallet in** ([§6.3](#63-node-portability-and-multi-node-operation)): a wallet switches nodes by configuration alone. Freeze-resistance therefore reduces to the same permissionless-publishing and node-portability properties that give custody safety; no party — node, publisher, or federation — holds the authority to withhold a holder's own coins. The one honest limit is **liveness under data availability**: if every replica of a needed `BatchBundle` is lost the affected transition cannot be reconstructed ([§4.6](#46-data-availability--replication-factor-k)), but that is a documented `k`-replication availability bound, not a freeze exercised by any party.
...
|---|---|---|---|
| **Bitcoin L1** | `BatchInscription` (231 B) | Taproot commit/reveal, witness-payload marker prefix `0x42 0x42` | [§3.5](#35-inscription-format) |
...
| **Blossom** (HTTP) | encrypted `CoinProof` blobs · plaintext `BatchBundle` blobs | content-addressed blob store (§7.4) | [§4.6](#46-data-availability--replication-factor-k) |
...
Bundle blobs — ZBE-encrypted `CoinProof` blobs and **plaintext** `BatchBundle` blobs ([§4.2.1](#421-bundle-blob-encryption-zbe-normative)) — are stored and fetched by SHA-256 content address using **Blossom** (BUD-01/02). A node MUST expose, under its base URL, the path prefix `/blossom`:

| Method | Path | Purpose | Auth |
|---|---|---|---|
| `GET` | `/blossom/<sha256>` | fetch a blob by its lowercase-hex SHA-256 (`= blob_id`) | none (a `CoinProof` blob is already encrypted; a `BatchBundle` is public) |
...
| `GET` | `/v1/info` | `{ network, bitcoin_network, protocol_version: "v1", circuit_digests: { C, C_batch }, relay_url, blossom_url, finality_confirmations: 6, max_tx_inputs: 8, max_tx_outputs: 8, max_rx_coins: 4, max_account_assets: 32 }` |
| `GET` | `/v1/chain/accumulator` | `{ root, tip_block_hash, tip_height }` — the current `NAV(tip)` ([§3.7](#37-the-nullifier-accumulator)) |
| `GET` | `/v1/chain/inscriptions?from_height=&limit=` | list of admitted `BatchInscription`s (`{ txid, height, publisher_pubkey, prev_root, new_root, bundle_locator, blob_id, state }`), where `blob_id = H(serialize(BatchBundle))` is the Blossom fetch address ([§7.4](#74-blossom-blob-store-normative)) resolving the on-chain `bundle_locator` — a scanner fetches the plaintext `BatchBundle` blob by `blob_id`, then binds it by recomputing `bundle_locator` ([§3.6 step 6](#36-chain-scanning)); `blob_id` is `null` while the bundle is still unfetched (state `pending`-DA) |
...
| `GET` | `/v1/chain/anchors` | `{ root, size }` — the node's current anchors-MMR root and leaf count ([§1.7.10](#1710-anchors-mmr-normative)) |
| `GET` | `/v1/chain/anchors/<root>` | `{ valid: bool, size? }` — whether `<root>` is a historical anchors-MMR root per this node's scan (see the §1.7.10 trust note: Path-A-trustless, not self-verifying for light clients) |
| `GET` | `/v1/chain/anchors/roots?from_index=&limit=` | ordered list of per-append anchors-MMR roots (`{ index, root }`) from `from_index` — the **bulk** sync a light client uses instead of leaking a single subject's root via `/anchors/<root>` ([§1.7.10](#1710-anchors-mmr-normative) trust note) |
| `GET` | `/v1/chain/anchors/member/<member_root>` | `{ leaf_index, path: [hex], peaks: [hex], size }` — the anchors-MMR membership path for an admitted batch's `member_root` ([§1.7.10](#1710-anchors-mmr-normative)), used by a wallet building a receive witness (clause 10(d)) |
...
| `POST` | `/v1/jobs/<job_id>/sign` | body = `{ signature, signed_message, s2c_nonce, npk_rand, oic_rand }` — the wallet returns the BIP-340 transition signature over the full `message`, the `signed_message` itself, `s2c_nonce = R'` (the pre-tweak sign-to-contract nonce point, a **non-secret** curve point the publisher needs for the `C_batch` S2C check, [§2.2 clause 2](#22-proof-types)), `npk_rand` (the fresh secret blind opening `next_pubkey_commit`), and `oic_rand` (the fresh secret blind opening `out_intent_commit`) — both blinds are circuit witnesses the node needs for the [§2.1 clause 2](#21-the-compliance-predicate) opening checks, both stay node-side and are **never** forwarded to the publisher or placed in the bundle. The wallet signs only once the node surfaces, in the `awaiting_signature` phase, `inr ‖ ocr`, the five node-computed `ProofData` digests, **and** the full plaintext `output_templates` list; the wallet **MUST** first validate that list in plaintext (payee templates == its own intent; the fee template to the quoted `fee_address`/`fee_asset_id` with `amount` == the quoted `fee` **exactly**; every change template `recipient == owner`; and **no** template with a non-`owner` recipient other than an intended payee or that single fee coin — so the node can neither over-charge the fee nor add an unexpected non-`owner` output), then form `out_intent_commit = H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)` and `message = inr ‖ ocr ‖ next_pubkey_commit` by committing its **own** `next_pubkey` under `npk_rand`, and reconstruct `H(ProofData)` over its **own** `out_intent_commit` before signing (it never blind-signs a node-supplied `message` and never blind-trusts the node's value-bearing `H(ProofData)`; the SPEND key never leaves the wallet, [§2.3](#23-state-transitions)) |
| `POST` | `/v1/jobs/<job_id>/cancel` | cancels a not-yet-published job |

The **proving handshake** keeps custody in the wallet: the wallet posts the transition intent (input coin references, output `CoinTemplate`s including the publisher-fee coin, the chosen publisher's `publisher_pubkey`, and the rotated `next_pubkey`); the node (prover) builds the witness — from which the **five node-computed** `ProofData` fields (`ash`, `ocr`, `inr`, `chr`, `anc`), hence `inr ‖ ocr`, are fully determined before any proving, while the sixth field `out_intent_commit` and thus the full `H(ProofData)` are completed by the wallet at signing — transitions the job to `awaiting_signature` and exposes `inr ‖ ocr`, those five `ProofData` digests, **and** the full plaintext `output_templates` list it will build `ocr` from ([§1.4](#14-identifiers-and-hashes), [§2.1 clause 5](#21-the-compliance-predicate)); the wallet **validates** that list in plaintext (payee templates == its own intent; the fee template to the quoted `fee_address`/`fee_asset_id` with `amount` == the quoted `fee` **exactly**; every change template `recipient == owner`; and **no** template with a non-`owner` recipient other than an intended payee or that single fee coin, §3.8 — all pure value comparisons, **no** Poseidon), draws a fresh `oic_rand` and forms `out_intent_commit = H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)`, draws a fresh `npk_rand` and forms the full `message = inr ‖ ocr ‖ next_pubkey_commit` by computing `next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)` over the same rotated key it posted in the intent (both hashes SHA-256, thin-wallet-native, with **no** proof re-derivation and **no** Poseidon in the wallet), reconstructs `H(ProofData) = SHA-256(ash ‖ ocr ‖ inr ‖ chr ‖ anc ‖ out_intent_commit)` over its **own** `out_intent_commit` (so it never blind-trusts the node's value-bearing digest), signs `message` with `skᵢ` (additionally applying the sign-to-contract tweak binding that `H(ProofData)` into the nonce, [§2.1 clause 9](#21-the-compliance-predicate), [§3.3](#33-off-chain-signature-handling)), and `POST`s `{ signature, signed_message, R', npk_rand, oic_rand }` to `/sign` (the node needs `npk_rand` and `oic_rand` to build the clause-2 opening witnesses; both are per-transition commitment secrets, not SPEND keys, and never reach the publisher or the bundle); the node then finalises the recursive proof (which verifies `txn_sig` over `inr ‖ ocr ‖ next_pubkey_commit` in-circuit, opens `next_pubkey_commit` against `w.next_pubkey`/`w.npk_rand`, **and** opens `out_intent_commit` against `serialize(w.output_templates)`/`w.oic_rand` — the same templates clause 5 builds `ocr` from, [§2.1 clause 2](#21-the-compliance-predicate) — so the custody signature authorises the rotated `next_pubkey` on **every** transition, and the exact output set on every transition whose S2C is re-verified, i.e. every anchored spend) and, for a batched send, forwards `R'` to the publisher for the `C_batch` sign-to-contract check ([§2.2 clause 2](#22-proof-types)) — the check that binds the wallet's `out_intent_commit` (via `H(ProofData)`) so the outputs cannot be redirected; the node finalises the `SpendRecord` + `CoinProof`s, delivers recipient `CoinProof`s over Nostr (§4.2), hands the `SpendRecord` + fee `CoinProof` to the chosen publisher (§7.6), and self-delivers change/state (§4.2). A pure mint ([§2.3.1](#231-mint--issuance)) and a receive transition ([§2.3.3 step 7](#233-receive)) follow the same flow with no publisher hand-off — a receive's `next_pubkey` is thus still authorised by the in-circuit signature check of [§2.1 clause 2](#21-the-compliance-predicate) even though it never reaches `C_batch` (a pure receive produces no outputs; an **unbatched** mint's output is forced to `owner` in-circuit by the no-input rule of [§2.1 clause 5](#21-the-compliance-predicate), so it cannot be redirected either).
...
| Method | Path | Body / Returns |
|---|---|---|
| `POST` | `/v1/publish/spendrecord` | body = `{ spend_record, fee_coinproof, block_anchor, s2c_nonce }` → `{ accepted: bool, reason?, batch_eta? }` — `s2c_nonce = R'` is the spender's **non-secret** pre-tweak sign-to-contract nonce point (symmetric to the `/sign` body of [§7.5](#75-node-rest-api-normative)); the publisher needs it for the `C_batch` clause-2 S2C check ([§2.2 clause 2](#22-proof-types)), since `R'` appears neither in the `SpendRecord` nor in its 64-byte signature (which carries only the tweaked nonce `R`). The publisher receives **only** the `SpendRecord` (whose rotated key is the hiding commitment `next_pubkey_commit`, [§1.4](#14-identifiers-and-hashes)), the fee `CoinProof`, `block_anchor`, and `R'` — **never** the raw `next_pubkey`, **never** the opening blind `npk_rand`, and **never** `oic_rand` or the plaintext `output_templates`, so it cannot reconstruct the account's rotation edge **or** its output intent. The member proof's public inputs do carry the hiding `out_intent_commit` (the publisher needs the six-field `serialize(ProofData)` to recompute `H(ProofData)` below), but it is a fresh-blind commitment that reveals nothing about the payees |

The publisher MUST verify, before accepting: the `SpendRecord`'s per-account proof and signature ([§2.2 clause 1](#22-proof-types)); that the supplied `s2c_nonce = R'` satisfies the sign-to-contract binding `R = R' + t·G` with `t = H(R' ‖ H(ProofData))` (the six-field, 192-byte `serialize(ProofData)`, [§1.4](#14-identifiers-and-hashes)) against the `SpendRecord` signature's nonce `R` ([§2.2 clause 2](#22-proof-types), [§3.3](#33-off-chain-signature-handling)) — this is the same relation the `AggregateBatchProof` re-proves in-circuit, so a record whose `R'` does not open the S2C tweak cannot yield a valid batch proof and MUST be rejected here; that `fee_coinproof` is a valid `CoinProof` addressed to the publisher's `fee_address`, of `fee_asset_id`, with `amount` **equal to** its quoted `fee` (exactly, [§3.8](#38-fees-and-economics)), and an output under the **same `ocr`** the `SpendRecord` signs; and that `block_anchor` is within the §3.5 bound (the per-batch on-chain `block_anchor` MUST be an ancestor of, or equal to, the oldest member's **build tip** and satisfy the §3.5 gap rule against the inclusion block, [§3.5](#35-inscription-format)). It then schedules the record (and its `R'`) into a `BatchBundle`, builds the `AggregateBatchProof` ([§2.5](#25-circuit-dimensioning-normative)) whose `C_batch` clause-2 check consumes `R'` per member, `PUT`s the bundle to ≥ `k` Blossom stores (§4.6), and inscribes the `BatchInscription` (§3.5). A publisher is permissionless: any node MAY run this endpoint; a wallet MAY point at its own node as publisher (self-publish, §3.4).
...
| `GetAnchors` | unary | current anchors-MMR root + size; validity lookup for a presented root; membership path for an admitted `member_root` ([§1.7.10](#1710-anchors-mmr-normative)) | `GET /v1/chain/anchors`, `/v1/chain/anchors/<root>`, `/v1/chain/anchors/member/<member_root>` |
| `ListAnchorsRoots` | server-stream | ordered per-append anchors-MMR roots from an index — the light-client **bulk** privacy sync ([§1.7.10](#1710-anchors-mmr-normative) trust note) | `GET /v1/chain/anchors/roots` |
...
| `Publish` | unary | hand a `SpendRecord` + fee `CoinProof` + `R'` (S2C nonce) to the publisher role, if enabled | `POST /v1/publish/spendrecord` (§7.6) |

**Proving handshake across the boundary.** Proving is kernel-side — it needs the accumulator state and the proving stack ([§6.1](#61-components-and-responsibilities)). The API layer **forwards** the wallet's transition intent to `SubmitTransition` and the wallet's signature to `SignTransition`; the witness is built and the recursive proof produced **inside the kernel**; the SPEND signature is produced **only** in the wallet and passes through the API layer and the kernel RPC verbatim (§7.5 proving handshake, [§2.3](#23-state-transitions)). The kernel RPC therefore never carries a SPEND-branch secret — only a finished BIP-340 signature over `message = inr ‖ ocr ‖ next_pubkey_commit` (the wallet commits its own `next_pubkey` under a fresh `npk_rand` and hashes it onto the witness-determined `inr ‖ ocr`), the non-secret pre-tweak S2C nonce point `R'` the kernel forwards to the publisher for the `C_batch` sign-to-contract check ([§2.2 clause 2](#22-proof-types)), and the fresh `npk_rand` **and** `oic_rand` blinds the kernel needs to build the [§2.1 clause 2](#21-the-compliance-predicate) opening witnesses. (`npk_rand` and `oic_rand`, like `nk` and `op_secret`, are **not** SPEND secrets: they are per-transition commitment blinds the wallet entrusts to its own node for witness construction — they stay kernel-side, are **never** forwarded to the publisher or placed in the bundle, and unlike `nk`/`op_secret` need not persist, since no later transition re-opens `next_pubkey_commit`/`out_intent_commit`. `nk` and `op_secret` are likewise part of the operational bundle, [§1.2](#12-key-hierarchy)/[§6.2](#62-wallet--node), and live kernel-side for witness construction — `nk` for nullifiers, `op_secret` for the `anchors_rand` derivation.) Proof construction and batch verification are **internal** kernel work driven by `SubmitTransition` and the chain scanner ([§3.6](#36-chain-scanning)) — not separately-callable procedures (the illustrative `prove`/`verifyBatch` verbs of [§6.1](#61-components-and-responsibilities) are these internal steps, not RPC entry points).
...
- **anchors MMR** — the append-only Poseidon Merkle Mountain Range over the `member_root` of every admitted `completed` batch; derived by every node from chain + bundles (never on-chain). Receive proofs bind a creating spend to an admitted batch against it (clause 10). ([§1.7.10](#1710-anchors-mmr-normative))
- **`anchors_root`** — an account's anchors-MMR root: the prefix of admitted batches its receive checks were performed against, carried forward monotonically by `extends` (clause 1) and required to be a superset of every received coin's creating root (clause 10(c)). Never exposed in the clear — see `anchors_commitment`. ([§1.7.10](#1710-anchors-mmr-normative), [§2.1](#21-the-compliance-predicate))
- **`anchors_commitment` (`anc`)** — the fifth `ProofData` field: the **hiding** commitment `Hc("AnchorsView", anchors_root ‖ anchors_rand)` a proof exposes publicly, so chain observers, publishers, and `BatchBundle` scanners learn nothing of the account's receive-recency. Opened (`anchors_opening = {anchors_root, anchors_rand}`) only to a coin's recipient (via the `CoinProof` bundle) or a disclosure verifier, who checks `anchors_root` against their own scan. ([§1.7.10](#1710-anchors-mmr-normative), [§2.1](#21-the-compliance-predicate), [§2.3.3](#233-receive))
...
- **BatchBundle** — `{prev_root, new_root, spend_records, aggregate_proof, nullifiers (derived view)}`; off-chain, `k = 3` replicated; content-addressed (fetched) by `blob_id = H(serialize(BatchBundle))` and on-chain bound by `bundle_locator = Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root)`, where `member_root` is the `BatchMember` hash tree over the ordered members ([§1.4](#14-identifiers-and-hashes), [§2.5](#25-circuit-dimensioning-normative)); carries every member `SpendRecord` plus the `AggregateBatchProof`. ([§1.4](#14-identifiers-and-hashes), [§3.1](#31-the-on-chain-object), [§4.6](#46-data-availability--replication-factor-k))
...
- **Blossom** — content-addressed HTTP blob store (one per node) holding ZBE-encrypted `CoinProof` blobs (`blob_id = H(ciphertext)`) and plaintext `BatchBundle` blobs (`blob_id = H(serialize(BatchBundle))`). ([§7.4](#74-blossom-blob-store-normative))
...
- **Bundle (CoinProof)** — `{coin, proof, inclusion_proof, creating_prev_ash, anchor_hint?, anchors_opening, epk, ciphertext, detect_tag}`; the off-chain object that is the recipient's receipt and — once folded into the recipient's own lineage by a receive transition ([§2.3.3 step 7](#233-receive)) — the basis of its spend credential. ([§1.5](#15-core-data-structures))
- **`bundle_locator`** — `Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root)`; the 32-byte on-chain **binding** value of a `BatchBundle`, inscribed in every `BatchInscription`. A scanner resolves it to the bundle's Blossom fetch key `blob_id = H(serialize(BatchBundle))` (returned by `/v1/chain/inscriptions`, [§7.5](#75-node-rest-api-normative)), fetches the blob by `blob_id`, then binds it by recomputing `bundle_locator` ([§3.6 steps 5–6](#36-chain-scanning)). ([§1.4](#14-identifiers-and-hashes), [§3.1](#31-the-on-chain-object), [§3.5](#35-inscription-format))
...
- **Invoice** — `{amount, recipient, asset_id, memo?, pk0, nk_commit, ivpk, op_pubkey, relays, addr_sig, sig}`; the off-chain payer-facing addressing object. `nk_commit` is a mandatory field: with `pk0` it forms the `recipient` preimage `H(pk0 ‖ nk_commit) == recipient` the sender checks first ([§4.3](#43-addressing-for-delivery)), and it is part of the signed `invoice_message`. `addr_sig` is a BIP-340 signature by `sk₀` that chains the address-holder to every field, including the choice of `ivpk` and `op_pubkey`; `sig` is the per-issuance BIP-340 signature by `op` that the recipient's online relay applies. Both are required. ([§1.5](#15-core-data-structures), [§4.3](#43-addressing-for-delivery))
...
- **`message`** — `inr ‖ ocr ‖ next_pubkey_commit`; the BIP-340-signed payload of a `SpendRecord` (96 bytes) — the spent-nullifier root, the output-coins root, and a hiding commitment to the account's rotated spend authority. The transition's validated **output intent** is bound **separately**, via the `out_intent_commit` field of `H(ProofData)` carried in the sign-to-contract nonce, so `message` stays 96 bytes and the `SpendRecord` wire is unchanged. ([§1.4](#14-identifiers-and-hashes))
- **Mint** — the issuance transition; produces a creator-owned coin under the asset's `IssuanceTerms_v1` (the creator of the asset is its sole minter; anyone can create their own asset, no one can mint someone else's); spends no coin, has an empty `nullifiers` list, and does not require a `BatchInscription` — its receiver verifies it directly against the mint's recursive proof (an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint). A creator MAY optionally batch a mint into a `BatchInscription` for a publicly anchored "first appearance"; this is publisher policy, not protocol-required. ([§2.3.1](#231-mint--issuance), [§6.5](#65-issuance--versioned-schemas-v1-minimal))
- **`mint-verified` (transaction state)** — a coin produced by a non-batched mint ([§2.3.1](#231-mint--issuance)) whose recursive mint proof (an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint) has been re-verified by the receiver. No `BatchInscription` exists; cryptographic safety is identical to `completed` but there is no on-chain anchor. Explorers MUST render `mint-verified` distinct from `completed`/`pending`/`failed` so the user understands the absence of a chain anchor. Any subsequent spend of the mint coin is anchored normally. ([§3.10](#310-transaction-states))
- **MMR (anchors MMR)** — the append-only Merkle Mountain Range over the `member_root` of every admitted `completed` batch; derived deterministically by every node from chain + bundles (never on-chain itself); the receive path proves against it that a creating spend was admitted, and its root `anchors_root` is exposed only through a hiding `anchors_commitment` (the fifth `ProofData` field), carried forward monotonically. (Earlier drafts' global commitment-MMR over account states is a *different, removed* structure — [§1.6](#16-trees-one-global-structure-one-per-account-structure) explains why no account-keyed global tree exists.) ([§1.7.10](#1710-anchors-mmr-normative), [§2.1 clauses 1/10](#21-the-compliance-predicate))
...
- **`next_pubkey_commit`** — `H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)`; the SHA-256 hiding commitment to the account's rotated spend key `next_pubkey = Pkᵢ₊₁` that forms the third `message` segment of a `SpendRecord`. Signed and opened in-circuit ([§2.1 clause 2](#21-the-compliance-predicate)), so custody binds the rotation exactly as a raw-key signature would, while the published value stays hiding — closing the rotation-edge linkage a raw `next_pubkey` would expose to a `BatchBundle` scanner. ([§1.4](#14-identifiers-and-hashes), [§2.3.2](#232-send))
...
- **`npk_rand`** — the fresh secret 32-byte blind the wallet draws per transition to form `next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)`; a circuit witness only (opened by [§2.1 clause 2](#21-the-compliance-predicate)), carried wallet→node in the `/sign` body ([§7.5](#75-node-rest-api-normative)), **never** in a `SpendRecord`, `BatchBundle`, `CoinProof`, or to a publisher; load-bearing for the rotation edge's hiding and, unlike `anchors_rand`, need not be reconstructible. ([§1.4](#14-identifiers-and-hashes))
- **`op_secret`** — hardened `A/4'` secret in the operational bundle; keys the deterministic `anchors_rand = HKDF("zkCoins/v1/AnchorsRand", op_secret ‖ u64-be(send_counter))` derivation ([§1.7.10](#1710-anchors-mmr-normative)); separate from `op` so anchors randomness never shares key material with the Nostr signature; cannot spend. ([§1.2](#12-key-hierarchy))
...
- **`oic_rand`** — the fresh secret 32-byte blind the wallet draws per transition to form `out_intent_commit = H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)`; a circuit witness only (opened by [§2.1 clause 2](#21-the-compliance-predicate)), carried wallet→node in the `/sign` body ([§7.5](#75-node-rest-api-normative)), **never** in a `SpendRecord`, `BatchBundle`, `CoinProof`, or to a publisher; load-bearing for the output-intent commitment's hiding and, like `npk_rand`, need not be reconstructible. ([§1.4](#14-identifiers-and-hashes))
...
- **`out_intent_commit`** (`oic`) — `H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates) ‖ oic_rand)`; the SHA-256 hiding commitment to a transition's **validated output intent** — the exact output-`CoinTemplate` list clause 5 builds `ocr` from — and the **sixth `ProofData` field**. The wallet computes it itself (**no** Poseidon) over the plaintext templates it validated in the proving handshake; [§2.1 clause 2](#21-the-compliance-predicate) opens it in-circuit against those templates, so the account's single custody signature (which binds `H(ProofData)`) authorises **which coins the outputs pay**, and a witness-building node cannot redirect them. Being a hiding public input — **not** a `message`/`SpendRecord` field — it adds no published bytes (the 96-byte `message` and the `SpendRecord` wire are unchanged) and no linkage. ([§1.4](#14-identifiers-and-hashes), [§2.1 clause 2](#21-the-compliance-predicate), [§7.5](#75-node-rest-api-normative))
...
- **`ProofData`** — `{new_account_state_hash, output_coins_root, input_nullifiers_root, coin_history_root, anchors_commitment, out_intent_commit}`; the proof's public inputs (six fields, `serialize(ProofData)` = 192 bytes). The sixth field `out_intent_commit` is the hiding commitment to the transition's validated output intent, bound into the custody signature via `H(ProofData)`. ([§1.4](#14-identifiers-and-hashes), [§2.1 clause 9](#21-the-compliance-predicate))
...
- **Receive (transition)** — the `C` execution that folds verified incoming coins into the account's own lineage: in-circuit verification of each creating proof, admission binding via the anchors MMR (non-mints), balance credit, and coin-history admission; produces no `SpendRecord` for any publisher. ([§2.1 clause 10](#21-the-compliance-predicate), [§2.3.3 step 7](#233-receive))
...
- **`serialize(ProofData)` / `H(ProofData)`** — canonical `new_account_state_hash ‖ output_coins_root ‖ input_nullifiers_root ‖ coin_history_root ‖ anchors_commitment ‖ out_intent_commit` (192 bytes); `H(ProofData) = SHA-256(serialize(ProofData))` is the spender's sign-to-contract tweak digest — so the custody signature binds the output intent (`out_intent_commit`) as well as `ocr`. ([§1.4](#14-identifiers-and-hashes), [§3.3](#33-off-chain-signature-handling))
...
- **ZBE (zkCoins Bundle Encryption)** — chunked ChaCha20-Poly1305 AEAD framing for `CoinProof` bundle blobs, which exceed NIP-44 v2's 65 535-byte limit; key `HKDF("zkCoins/v1/BlobKey", K_tx)`, 64 KiB chunks, per-chunk counter nonce + index-binding AAD. `BatchBundle`s are public and stored in plaintext — never ZBE-encrypted. ([§4.2.1](#421-bundle-blob-encryption-zbe-normative))
...
| `anr_empty` | `Hc("AnchorsMmr/Root", 0)` — the empty anchors-MMR root ([§1.7.10](#1710-anchors-mmr-normative)); the base of every account's anchors lineage and the view a first-transition mint on a fresh network commits (§2.3.1) | `<REGEN>` |
| `npk_rand@0` | `H("zkCoins/v1/test-vector/npk_rand")` = `a04b10a7ac57db9e12b2cac644653f97ffdfc4911935f21f027936f60c543b98` — the fixed illustrative blind, exactly as `anchors_rand@0` below | fixed |
| `oic_rand@0` | `H("zkCoins/v1/test-vector/oic_rand")` = `62140ab01622b2d5b7289fc33e6087677aa95d9ebd36785054587dfbff6f2800` — the fixed illustrative output-intent blind, exactly as `npk_rand@0` above | fixed |
| `next_pubkey_commit@0` | `H("zkCoins/v1/NextPubkeyCommit" ‖ Pk₁_sample ‖ npk_rand@0)` — the 91-byte SHA-256 preimage is the 27-byte ASCII tag `zkCoins/v1/NextPubkeyCommit` ‖ `Pk₁_sample` (V.1, 32B) ‖ `npk_rand@0` (32B), byte-concatenated per [§1.1](#11-cryptographic-primitives); opens to `Pk₁_sample` = V.3 `current_pubkey`, so the clause-2/clause-7 invariant holds | `<REGEN>` (pure SHA-256 of the pinned preimage above — left `<REGEN>` because no other vector yet pins a tagged multi-field SHA-256 output to cross-check the tag serialisation) |
| `message@0` | `inr@0 ‖ ocr@0 ‖ next_pubkey_commit@0` (three 32-byte values — the two roots above and the hiding commitment `next_pubkey_commit@0` to the genesis mint's rotated spend key `Pk₁_sample` (V.1), the same illustrative value pinned as `current_pubkey` in V.3 so the clause-2/clause-7 invariant that the commitment opens to `current_pubkey` holds; **96 bytes**) | `<REGEN>` (the two roots are Poseidon-dependent; the third segment is the SHA-256 `next_pubkey_commit@0` above) |
| `anchors_rand@0` | `H("zkCoins/v1/test-vector/anchors_rand")` = `b8e4bdbca08db9b0e3311030ff3b56db90c9e0701d9bfe1093c7187b924e4724` | fixed |
| `anchors_commitment@0` | `Hc("AnchorsView", anr_empty ‖ anchors_rand@0)` (a first-transition mint commits the empty anchors view) | `<REGEN>` |
| `out_intent_commit@0` | `H("zkCoins/v1/OutIntentCommit" ‖ serialize(output_templates@0) ‖ oic_rand@0)`, where the genesis mint's single output template is `{recipient = address, asset_id, amount = 1000000000}`, so `serialize(output_templates@0) = address (32B) ‖ asset_id (32B) ‖ amount (16B big-endian = 0000000000000000000000003b9aca00)` (§1.4, the clause-5 output order — one 80-byte record). The preimage is the 26-byte ASCII tag `zkCoins/v1/OutIntentCommit` ‖ that 80-byte record ‖ `oic_rand@0` (32B), byte-concatenated per [§1.1](#11-cryptographic-primitives) — 138 bytes total; opens to `recipient == owner`, so [§2.1 clause 2](#21-the-compliance-predicate) binds the minted coin to the issuing account | `<REGEN>` (SHA-256, but `address` and `asset_id` in the preimage are Poseidon-dependent — V.2/V.4 — so the preimage is not yet pinnable) |
| `H(ProofData@0)` | `SHA-256(serialize(ProofData@0))` = `SHA-256(ash@0 ‖ ocr@0 ‖ inr@0 ‖ coin_history_root@0 ‖ anchors_commitment@0 ‖ out_intent_commit@0)` (canonical 192-byte `serialize(ProofData)`, six 32-byte fields, [§1.4](#14-identifiers-and-hashes)) | derived from the six above |
...
message      (96B): <message = inr ‖ ocr ‖ next_pubkey_commit>   ← inr 32B ‖ ocr 32B ‖ next_pubkey_commit 32B
k            ( 1B): 01                                                                ← one input coin
nullifiers   (32B): <nf = Hc("Nullifier", nk ‖ coin.identifier)>

Record size: 225 bytes for a typical single-input spend (32 + 64 + 96 + 1 + 32·k = 32 + 64 + 96 + 1 + 32).
```

The mint case has `k = 0` (an empty nullifier list, 193 bytes total) and need not be batched at all: a creator who chooses not to anchor a mint hands the receiver only the `CoinProof` bundle, and the mint's validity is checked by direct recursive verification of the mint's proof — an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint ([§2.3.1](#231-mint--issuance)).
...
Payload (231 bytes):
   marker                    ( 2B): 4242
   version                   ( 1B): 02                                ← batched-inscription payload version
   ...
   bundle_locator            (32B): <Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root) — on-chain binding value>
...
5. Compute the BIP-340 signature with sign-to-contract per [§3.3](#33-off-chain-signature-handling) (the spender's S2C tweak over `H(ProofData)` — now the six-field, 192-byte `serialize(ProofData@0)` including `out_intent_commit@0`, V.4 — over the full 96-byte `message@0 = inr@0 ‖ ocr@0 ‖ next_pubkey_commit@0`) and fill in V.5's `signature`. The signing key `sk₀` is a real secp256k1 key derived from a real BIP-32 path; a separate signing fixture is needed because the V.1 illustrative `Pk₀_sample` is a raw 32-byte string, not a curve point. The third `message@0` segment is the **hiding commitment** `next_pubkey_commit@0 = H("zkCoins/v1/NextPubkeyCommit" ‖ Pk₁_sample ‖ npk_rand@0)` (V.4), **not** the raw rotated key; its opening reveals `next_pubkey@0 = Pk₁_sample` (V.1) — the same illustrative value pinned as `current_pubkey` in V.3, so the clause-2 opening yields exactly the key clause 7 folds into `ash@0` (clause-2/clause-7 invariant: `next_pubkey_commit` opens to `current_pubkey`). The 32-byte commitment is signed as raw message bytes and need not be a curve point.
6. Submit the completed vectors back to the spec as a PR; the reference is locked once the SDK's independent primitive-level re-implementation reproduces the hash- and derivation-level values bit-for-bit — the two `circuit_digest`s are locked by the node's deterministic §1.7.9 build alone, and the `signature` values (V.5, V.6) are locked by BIP-340 verification including the sign-to-contract tweak check (V.5's spender S2C per [§3.3](#33-off-chain-signature-handling), V.6's publisher S2C per [§3.2](#32-batchinscription-signing-bip-340--sign-to-contract)), not byte equality.
