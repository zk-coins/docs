...
**Names (normative).** The API and app layers **MUST** provide every account they serve with an email-style **name** `<user>@<domain>`, resolve it as NIP-05, and publish the [§7.3](#73-nostr-event-kinds-normative) kind-0 `zkcoins` object under it. The kernel works from `op_pubkey`, `nprofile`, and `addr_sig`-carrying objects ([§6.1](#61-components-and-responsibilities) *Kernel and API — two boundaries*). A name is a label over those keys: it stays outside the seed derivation, every signature preimage, and every value-bearing structure, and recipient resolution verifies the `addr_sig`-carrying object — an `Invoice` or the kind-0 profile below.
...
**Names are plural and replaceable (normative).** Any number of names, across any number of domains, **MAY** resolve to the same `op_pubkey`. A client treats each of them as equally valid and accepts any of them for discovery, whichever one the account's kind-0 `nip05` field happens to carry. Adding or replacing a name leaves keys, `address`, and pinned contacts as they are.
...
**Name changes (normative).** Pinning covers `op_pubkey` and the payment identity above. A verified kind-0 event under the pinned `op_pubkey` whose `nip05` value differs from a stored name **MAY** be adopted silently: the client adds the new name to the record and marks the previous one no longer self-attested, leaving the pinned keys, the payment identity, the conversation, and retained transport as they are. A name that now maps to a **different** public key belongs to a different contact and is attached to that one, per the reverse-check outcome above.

An account changes its names while leaving the `zkcoins` object's payment fields in place; `addr_sig` covers no name.
...
| Standard NIP-17 messaging on kind-10050 DM relays, addressed by key, and known-contact pinning ([§4.3](#43-addressing-for-delivery), [§7.3](#73-nostr-event-kinds-normative)) | **Core** | always on |
...
- **Names live in the API layer.** The **API layer** resolves an account's email-style name ([§4.3](#43-addressing-for-delivery)) and, under the `aliasing` role, issues it; where no API container runs, the end-user app resolves. The kernel takes `op_pubkey`, `nprofile`, or an `addr_sig`-carrying object for everything it is asked to do, so a publisher — a pure kernel deployment — runs on a key and a relay ([§3.8](#38-fees-and-economics)).
