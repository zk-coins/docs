...
| **11 · Standard identity and messaging** | §4.1/§4.3 (NIP-05 identity, kind-0 payment metadata, retained `nprofile` plus TLS-and-WebSocket-validated relay endpoints for DNS-free known contacts), §7.3/V.12 (mandatory NIP-17/kind-10050 and external-client interoperability), Lightning/mail bridge pages (independently optional) |
...
The client forms one discovery set as the **union** of (a) every non-empty `relays[op_pubkey]` hint and (b) its already configured Nostr profile/discovery relays, including the Bootstrap Manifest's seed relays. It searches that whole set for the latest kind-0 and kind-10050 events by using ordinary NIP-01 filters for that author and kind; the two valid events may come from different relays. It introduces no custom endpoint, kind, tag, NIP-65 requirement, or zkCoins capability metadata. The client verifies the kind-0 signature under `op_pubkey` and requires its `nip05` field to equal the normalized identifier (the reverse NIP-05 check). It verifies kind 10050 under the same author and requires at least one valid `relay` tag. If either the reverse kind-0 check or kind-10050 discovery fails across the union, the identity is **not ready**: messaging fails closed, with no SMTP or NIP-04 fallback.
...
if an nprofile for the identifier is known locally:
    use its stored op_pubkey, relay hints, DM relays, and TLS-and-WebSocket-validated IP endpoints directly
    ...
    retain op_pubkey, nprofile, relay hints, DM relays, successful endpoints,
...
- **The `zkcoins` profile is public.** The kind-0 event carrying `nip05` and the `zkcoins` object ([§7.3](#73-nostr-event-kinds-normative)) is not gift-wrapped: it is public and unencrypted, and — like any Nostr kind-0 — is indexed and archived by ordinary Nostr clients and aggregators. Publishing it therefore publicly binds the NIP-05 identifier to `address`, `pk0`, `nk_commit`, and `ivpk`. A user who does not want their payment identity publicly tied to their messaging identity needs separate accounts.
...
- **Known-contact continuity uses local state.** The retained §4.3 contact record — normalized identifier, pinned `op_pubkey`, standard `nprofile`, last valid kind-10050 DM relays, optional pinned payment identity, and original-hostname-authenticated TLS/WebSocket endpoints — is not seed- or chain-derived. Switching nodes or recovering from the seed alone **without transferring that record** therefore resets trust-on-first-use and loses DNS-free cold-start reachability for that contact; the contact must be rediscovered through NIP-05 when DNS returns. A deployment that claims known-contact or DNS-free continuity across a node switch **MUST** transfer or synchronise the complete retained record atomically through an authenticated operator-controlled facility. A partial record **MUST NOT** be treated as complete or as providing DNS-free continuity, and an existing pin **MUST NOT** be silently discarded or replaced. This portability facility is local deployment behavior, not a zkCoins or Nostr wire protocol, and its absence never affects custody or spendability. A client that has lost its pin set **MUST NOT** silently re-pin a previously known contact as if it were a first contact; the full obligation is stated under [§4.3](#43-addressing-for-delivery) *Payment-identity pinning*.
...
**Core surface vs optional roles.** The families above are the node **core** surface — every node serves them, for the accounts it is responsible for. This includes NIP-05 resolution/contact pinning and standard NIP-17 messaging; only hosting users' NIP-05 names is optional. The optional node roles ([§6.1](#61-components-and-responsibilities)) layer **on top** of the same surface rather than adding new wire protocols: the **public wallet API** is `read.account` + `submit.tx` (with proving) offered for **hosted** accounts; the **publisher** consumes already-submitted transitions to half-aggregate and inscribe their nullifiers ([§3.4](#34-the-publisher)); operator-hosted NIP-05 names use `aliasing`. These roles are advertised through the closed `roles` set on `GET /v1/info` / kernel `Info` (`{core, local_prover, hosted_wallet_api, publisher, aliasing}`, [§7.5](#75-node-rest-api-normative)). The [Lightning bridge](/lightning-bridge) and [mail bridge](/mail-bridge) remain independent services outside both the core and that advertisement: Lightning receive discovery uses standard kind-0 `lud16` plus LUD-16/LNURL-pay, Lightning send requires explicit local bridge configuration and a quote, and SMTP use requires explicit operator/user configuration. No bridge role, endpoint, or event is added to the node protocol here. A client that has not confirmed a bridge through these path-specific signals — a resolved `lud16`/LNURL-pay endpoint for receive, or explicit local configuration and a successful quote for send — **MUST** treat that bridge as absent, the same fail-closed rule applied to the closed `roles` set.
...
| **11 · Standard identity and messaging** | Mandatory NIP-05 `user@domain` identity, signed kind-0 `zkcoins` object, standard NIP-17/kind-10050 messaging, and retained TLS-and-WebSocket-validated relay endpoints for DNS-free known contacts (§4.1, §4.3, §7.3); Lightning and SMTP bridges remain independent and optional. |
...
3. The sender wraps the seal in a kind-1059 gift wrap under a fresh random one-time wrapper key, with **exactly one** outer recipient `p` tag naming that copy's receiver, as NIP-59 requires.
4. The sender creates separate receiver and sender copies: one gift wrap whose sole outer recipient `p` names the recipient and one whose sole outer recipient `p` names the sender. Each copy is encrypted separately for that receiver and **MUST** use its own fresh, distinct one-time wrapper key; the two outer kind-1059 `pubkey` values **MUST** differ.
...
- **`address`** — `H(Pk₀ ‖ nk_commit)`; the protocol's only value-bearing cryptographic payment identifier, fixed at account creation and encoded as Bech32m `zk`; commits to both the initial spend key and the account's nullifier-key commitment. NIP-05 is the public human/Nostr identity. ([§1.4](#14-identifiers-and-hashes), [§4.3](#43-addressing-for-delivery))
...
- **NIP-05 identifier** — the mandatory normalized public human/Nostr account identity (`user@domain`) that maps through `/.well-known/nostr.json?name=user` to the account's `op_pubkey`; DNS is used only for first discovery or explicit revalidation, never normal use of a contact with retained relay endpoints that passed original-hostname TLS authentication and the WebSocket relay upgrade on the original path. ([§4.3](#43-addressing-for-delivery))
...
| M-14 | parsed rumor kind is not 14, `created_at` is missing, or `content` is not a plaintext JSON string | discard before display/storage; no side effect or fallback |
...
5. Captured events show a kind-14 rumor with exactly one recipient `p`, NIP-44 v2, kind-13 seal, kind-1059 gift wrap, kind-10050 relay selection, and separate sender/recipient copies whose fresh one-time outer `pubkey` values differ.
...
8. Exercise a true cold start after the external contact and successful relay endpoints are retained: close every WebSocket, terminate and restart the process, empty or block resolver and application DNS caches, and load only the persistent §4.3 contact/transport state. Send successfully by dialing an endpoint retained only after original-hostname-authenticated TLS **and** a successful relay WebSocket upgrade, while preserving the original scheme, hostname, port, and path for TLS SNI, certificate verification, WebSocket `Host`, and the upgrade request; assert zero DNS and zero NIP-05 requests and assert that TLS verification is neither disabled nor replaced by IP-certificate validation.

**Acceptance:** accept all three positive NIP-05 variants; reject P-01–P-07 with the named scope; block the silent change or use of a pinned payment identity in P-08–P-09; treat P-10 as first use with a warning; reject M-01–M-14 with the named scope; pass the distinct-wrapper-key and cold-start assertions and every live test in both directions with Primal, Amethyst, Damus, and 0xchat. A release fails V.12 if baseline one-to-one messaging fails with any client.
