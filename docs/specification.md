...
| `message@0` | `inr@0 ‖ ocr@0 ‖ next_pubkey@0` (three 32-byte values — the two roots above and the genesis mint's rotated x-only spend key `next_pubkey@0 = Pk₁`, from the V.5 signing test-key fixture; **96 bytes**) | `<REGEN>` (depends on the fixture `Pk₁`) |
...
5. Compute the BIP-340 signature with sign-to-contract per [§3.3](#33-off-chain-signature-handling) (the spender's S2C tweak over `H(ProofData)`, over the full 96-byte `message@0 = inr@0 ‖ ocr@0 ‖ next_pubkey@0`) and fill in V.5's `signature`. The signing key `sk₀` is a real secp256k1 key derived from a real BIP-32 path, and the fixture also provides the rotated `next_pubkey@0 = Pk₁` (the x-only public key of the next signing key `sk₁`) that the third `message@0` segment binds; a separate test-key fixture is needed because the V.1 illustrative `Pk₀_sample` is a raw 32-byte string, not a curve point.
