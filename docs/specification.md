...
| **SpendRecord** | `{ public_key: Pkᵢ (32B x-only), nullifiers: [nf]ⱼ (32B each — the coins spent in this transition), signature: BIP-340(skᵢ, message) (64B), message (64B) }` — an **off-chain** object: a spender produces one per transition and hands it to a publisher. The publisher aggregates many `SpendRecord`s into one `BatchBundle` ([§4.6](#46-data-availability--replication-factor-k)), builds the `AggregateBatchProof` over them, and anchors the batch with one constant-size `BatchInscription` on Bitcoin ([§3.1](#31-the-on-chain-object)). A mint, which spends no coin, has an empty `nullifiers` list and need not be batched at all — its receiver verifies it directly against its `CoinProof` bundle | ~160 + 32·\|nf\| bytes off-chain |
...
| **BatchBundle** | `{ prev_root, new_root, nullifiers: [nf], spend_records: [SpendRecord], aggregate_proof: AggregateBatchProof }` — off-chain, content-addressed, `k = 3` replicated by the same DA discipline as `CoinProof` bundles ([§4.6](#46-data-availability--replication-factor-k)). Carries every member `SpendRecord` plus the recursive proof that attests the whole batch | grows with member count + recursive-proof size (typically ~100 KB) |
...
- **Public inputs:** `prev_root` (the accumulator root before this batch), `new_root` (the resulting accumulator root), and the batch-nullifier set commitment `Hc("BatchNfs", nf₁ ‖ … ‖ nf_n)` (a single field-encoded digest binding the off-chain ordered nullifier list to the proof, so the bundle binding of [On-chain §3.6](#36-chain-scanning) covers exactly what the proof attests).
- **Private witness:** the `m` member `SpendRecord`s; the `m` corresponding per-account proofs (`InitialProof` / `AccountUpdateProof` from `C`); the SMT insertion paths from `prev_root` to `new_root` for every member nullifier.
...
4. **Accumulator transition correctness.** Starting from `prev_root` and applying SMT-insertion of each `nf ∈ batch_nullifiers` in the canonical insertion order produces exactly `new_root`. The order is defined by the publisher in the bundle and bound by the public-input nullifier commitment.

Verifier data for `C_batch` is also fixed, so the `AggregateBatchProof` is **constant-size in `m`** (asymptotic to the recursion-overhead floor of ~100 KB; the marginal per-member contribution is in proving *time*, not in proof *size*). A scanner verifies one `AggregateBatchProof` per `BatchInscription` — never per member — and accepts the entire batch's accumulator transition on a single check.
...
6. **Verify bundle binding.** Recompute `Hc("BatchBundle", serialize(BatchBundle))` and check it equals the inscribed `bundle_locator`. Recompute `H(serialize(AggregateBatchProof))` and check it equals the sign-to-contract tweak input (§3.2). A binding failure MUST cause the inscription to be discarded.
7. **Verify the aggregate proof.** Verify the `AggregateBatchProof` ([Proofs §2.2](#22-proof-types)) against its public inputs `(prev_root, new_root, batch_nullifiers)` derived from the bundle. The proof is a single recursive PCD attestation that every member spender's transition is valid under its own per-account recursive proof, every member nullifier is correctly derived (`nf = Hc("Nullifier", nk ‖ coin.identifier)`), and `new_root` equals the deterministic SMT-insertion of all member nullifiers into `prev_root`. A proof-verification failure MUST cause the inscription to be discarded.
...
- **Path B — light-client: hold only the on-chain roots, delegate the lookup.** Follow the inscribed `new_root` sequence (231 bytes per batch on-chain), then ask any Path-A node for an SMT inclusion-or-exclusion path of `nf` against the current `new_root`. The path is ~512 bytes (compressed empty subtrees) and self-verifying against the root the light client already knows from Bitcoin. The answering node cannot lie: a forged "absent" response would require fabricating a Merkle path against the on-chain root, which the proof system's hash assumption makes infeasible. Querying multiple independent nodes hardens against denial of service; the correctness check is single-node-honest.
...
**Reorg handling.** If Bitcoin reorganises, every admitted `BatchInscription` published only in orphaned blocks MUST be reverted — its `prev_root → new_root` transition is undone — and re-applied (if its bundle is still available and its `prev_root` still matches in the new canonical chain) under the new canonical tip. The accumulator state at the new tip is the deterministic replay of admissions in the new canonical order. Because `NAV` is explicitly tied to a tip, a stale one is self-identifying: a verifier MUST recompute or re-fetch `NAV` for the current canonical tip before acting on a non-membership result, and SHOULD wait for finality (§3.9) so that the anchoring tip is reorg-stable.
...
**Receivers SHALL act only on `completed`.** The anchor / receive checks in [Proofs §2.3.3](#233-receive), [Transport & Recovery §4.5](#45-recovery), and [Access & Explorer §5.6 / §5.7](#5--access--explorer) all require the relevant `BatchInscription` (and hence its member `SpendRecord`) to be in state `completed`; an inscription in any other state **MUST NOT** be treated as anchored. The user-facing **status** rendered by an explorer (e.g. Access & Explorer §5.6 step 3) **MUST** be the §3.10 state, not a node-asserted classification.
...
- **BatchBundle** — `{prev_root, new_root, nullifiers, spend_records, aggregate_proof}`; off-chain, content-addressed (`bundle_locator = Hc("BatchBundle", serialize(BatchBundle))`), `k = 3` replicated; carries every member `SpendRecord` plus the `AggregateBatchProof`. ([§3.1](#31-the-on-chain-object), [§4.6](#46-data-availability--replication-factor-k))
