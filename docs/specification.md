...
9. **Public-input binding.** All five `ProofData` fields — `new_account_state_hash`, `output_coins_root`, `input_nullifiers_root`, `coin_history_root`, `anchors_commitment` — **MUST** be the in-circuit-computed values above and are the proof's public inputs. Nothing else is public: amounts, asset ids, recipients, keys, counts, the fresh blind `npk_rand` (hidden inside `next_pubkey_commit`, clause 2), and the underlying `anchors_root` (hidden inside its commitment, clause 1) remain in the witness (zero-knowledge).
...
Wallet:
  1. derive Pk₀ = sk₀·G and the rotated next_pubkey Pk₁ (both SPEND branch); draw a fresh
     ...
     (SHA-256, wallet-native, §1.4); sign the transition
     signature BIP-340(sk₀, message = inr ‖ ocr ‖ next_pubkey_commit)
  ...
  3. provide next_pubkey Pk₁ (SPEND branch) and the fresh npk_rand so the node can build the
     clause-2 opening witness (the node already holds nk — operational bundle, Foundations §1.2)
...
Wallet:
  1. sign the single transition signature BIP-340(skᵢ, message = inr ‖ ocr ‖ next_pubkey_commit) with the
     current per-transition signing key skᵢ (whose Pkᵢ is current_pubkey; no per-coin key), committing
     the wallet's own rotated next_pubkey Pkᵢ₊₁ (step 2) under a fresh npk_rand as
     next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ Pkᵢ₊₁ ‖ npk_rand), the third message segment
  2. supply the rotated next_pubkey Pkᵢ₊₁ (SPEND branch, Foundations §1.2) and the fresh npk_rand
     (circuit witness for the clause-2 opening, never published); the node
...
On all of 2–6 passing:
  7. RUN THE RECEIVE TRANSITION: execute C — as an InitialProof if this is the account's first
     ...
     as for a send (§7.5 proving handshake); no SpendRecord is handed to any publisher (nothing to
     anchor — a pure receive spends nothing and its nullifier list is empty).
...
**Handle syntax.** The local part `<user>` is lowercase `a-z0-9-_.`; `<domain>` is a DNS hostname. Both parts are compared **after lowercasing**, so `Alice@Example.com` and `alice@example.com` denote the same handle. The syntax is deliberately LUD-16-compatible (see *one handle for Lightning and zkCoins* below).

**Resolution.** `<user>@<domain>` resolves to `https://<domain>/.well-known/zkcoins/<user>` by an HTTPS `GET`. The response body is the recipient's `addr_sig`-signed `Invoice` ([Foundations §1.5](#15-core-data-structures)), or the **profile** tuple carrying the profile-fixed values (`amount = 0`, the all-zero `asset_id`, empty `memo`) exactly as defined above in §4.3. The sender then **MUST** run the same three-check verification of §4.3 — `H(pk0 ‖ nk_commit) == recipient`, `addr_sig` under `pk0`, `sig` under `op_pubkey` — before encrypting anything, and proceeds with delivery (§4.2) and real-time push (§4.9). Registering a handle is the existing **optional aliasing role** of the API layer ([§6.1](#61-components-and-responsibilities)); a sovereign node without that role hands out the raw `Invoice`/profile directly, or the user fronts their **own** domain — the handle is an opt-in convenience, not part of the trustless core.

**Trust is unchanged.** The handle is a resolution/UX layer only; the trust anchor remains the `addr_sig` binding (§4.3). A malicious or lying resolver can at most refuse, or return an `Invoice` whose `addr_sig` the sender **rejects** — it can **never** redirect funds, because the sender encrypts only to an `addr_sig`-verified `ivpk`. Resolving a handle discloses the handle → `Invoice` mapping to the serving domain, the same disclosure as publishing a profile.

**Handle pinning.** On the **first** successful resolution the client **MUST** pin the mapping `<user>@<domain>` → `address` (trust-on-first-use). If a later resolution of the same handle yields a **different** `address`, the client **MUST** warn the user and **MUST NOT** proceed silently. The client **SHOULD** cross-check the resolved tuple against the recipient's kind-30420 profile on the recipient's relays (`#d = <address>`, [§7.3](#73-nostr-event-kinds-normative)), so that the HTTPS domain and the Nostr relay set **must** agree — neither channel alone can silently re-map an established handle.
...
**One handle for Lightning and zkCoins (forward-looking).** The handle syntax is LUD-16-compatible precisely so a **future** app version can serve **both rails from the same handle and the same QR code**: a Lightning wallet resolves `https://<domain>/.well-known/lnurlp/<user>` per LNURL-pay (LUD-16), a zkCoins wallet resolves `https://<domain>/.well-known/zkcoins/<user>` as above. One handle, one QR, two rails; the wallet selects the rail — a standard LNURL-pay response for Lightning, the `addr_sig`-signed `Invoice` for zkCoins. QR codes encode the **handle**, never a raw `zk1…` string, so a user shares **one** receive identity and can be paid on either rail. This dual-rail capability is a **planned future version**; the zkCoins resolution defined here stands on its own today.
...
| `POST` | `/v1/jobs/<job_id>/sign` | body = `{ signature, signed_message, s2c_nonce, npk_rand }` — the wallet returns the BIP-340 transition signature over the full `message`, the `signed_message` itself, `s2c_nonce = R'` (the pre-tweak sign-to-contract nonce point, a **non-secret** curve point the publisher needs for the `C_batch` S2C check, [§2.2 clause 2](#22-proof-types)), and `npk_rand` (the fresh secret blind opening `next_pubkey_commit`, a circuit witness the node needs for the [§2.1 clause 2](#21-the-compliance-predicate) opening check — it stays node-side and is **never** forwarded to the publisher or placed in the bundle), once the node surfaces `inr ‖ ocr` in the `awaiting_signature` phase; the wallet forms `message = inr ‖ ocr ‖ next_pubkey_commit` by committing its **own** `next_pubkey` under `npk_rand` (it never blind-signs a node-supplied `message`; the SPEND key never leaves the wallet, [§2.3](#23-state-transitions)) |
| `POST` | `/v1/jobs/<job_id>/cancel` | cancels a not-yet-published job |

The **proving handshake** keeps custody in the wallet: the wallet posts the transition intent (input coin references, output `CoinTemplate`s including the publisher-fee coin, the chosen publisher's `publisher_pubkey`, and the rotated `next_pubkey`); the node (prover) builds the witness — from which the five `ProofData` fields, hence `inr ‖ ocr` and `H(ProofData)`, are fully determined before any proving — transitions the job to `awaiting_signature` and exposes `inr ‖ ocr` ([§1.4](#14-identifiers-and-hashes)); the wallet forms the full `message = inr ‖ ocr ‖ next_pubkey_commit` by drawing a fresh `npk_rand` and computing `next_pubkey_commit = H("zkCoins/v1/NextPubkeyCommit" ‖ next_pubkey ‖ npk_rand)` over the same rotated key it posted in the intent (SHA-256, a thin-wallet-native 32-byte hash, with **no** proof re-derivation and **no** Poseidon in the wallet, so the wallet never has to trust a node-supplied `message`), signs it with `skᵢ` (additionally applying the sign-to-contract tweak binding the witness-determined `H(ProofData)` into the nonce, [§2.1 clause 9](#21-the-compliance-predicate), [§3.3](#33-off-chain-signature-handling)), and `POST`s `{ signature, signed_message, R', npk_rand }` to `/sign` (the node needs `npk_rand` to build the clause-2 opening witness; it is a per-transition commitment secret, not a SPEND key, and never reaches the publisher or the bundle); the node then finalises the recursive proof (which verifies `txn_sig` over `inr ‖ ocr ‖ next_pubkey_commit` in-circuit **and** opens `next_pubkey_commit` against `w.next_pubkey`/`w.npk_rand`, [§2.1 clause 2](#21-the-compliance-predicate) — so the custody signature authorises the rotated `next_pubkey` on **every** transition) and, for a batched send, forwards `R'` to the publisher for the `C_batch` sign-to-contract check ([§2.2 clause 2](#22-proof-types)); the node finalises the `SpendRecord` + `CoinProof`s, delivers recipient `CoinProof`s over Nostr (§4.2), hands the `SpendRecord` + fee `CoinProof` to the chosen publisher (§7.6), and self-delivers change/state (§4.2). A pure mint ([§2.3.1](#231-mint--issuance)) and a receive transition ([§2.3.3 step 7](#233-receive)) follow the same flow with no publisher hand-off — a receive's `next_pubkey` is thus authorised by the in-circuit signature check of [§2.1 clause 2](#21-the-compliance-predicate) even though it never reaches `C_batch`.
...
- **`message`** — `inr ‖ ocr ‖ next_pubkey_commit`; the BIP-340-signed payload of a `SpendRecord` (96 bytes) — the spent-nullifier root, the output-coins root, and a hiding commitment to the account's rotated spend authority. ([§1.4](#14-identifiers-and-hashes))
...
- **`ProofData`** — `{new_account_state_hash, output_coins_root, input_nullifiers_root, coin_history_root, anchors_commitment}`; the proof's public inputs. ([§1.4](#14-identifiers-and-hashes), [§2.1 clause 9](#21-the-compliance-predicate))
...
- **`serialize(ProofData)` / `H(ProofData)`** — canonical `new_account_state_hash ‖ output_coins_root ‖ input_nullifiers_root ‖ coin_history_root ‖ anchors_commitment` (160 bytes); `H(ProofData) = SHA-256(serialize(ProofData))` is the spender's sign-to-contract tweak digest. ([§1.4](#14-identifiers-and-hashes), [§3.3](#33-off-chain-signature-handling))
...
| `H(ProofData@0)` | `SHA-256(serialize(ProofData@0))` = `SHA-256(ash@0 ‖ ocr@0 ‖ inr@0 ‖ coin_history_root@0 ‖ anchors_commitment@0)` (canonical 160-byte `serialize(ProofData)`, [§1.4](#14-identifiers-and-hashes)) | derived from the five above |
