...
- **DeliveryEvent** — Nostr delivery: rumor payload `{blob_id, blob_locators, ack_nonce}` (optional `record_kind` for self-delivery), NIP-44 encrypted to `IVPK` and NIP-59 gift-wrapped under an ephemeral key; the **outer** kind-1059 event carries **only** the per-coin scan tags `zkdt`/`zkepk` in cleartext. `blob_locators` is the encrypted interior `BlobLocatorSet`. The `ack_nonce` is a fresh sender-chosen 32-byte value the recipient echoes in the ACK signature, binding the ACK to this delivery attempt. ([§4.2](#42-bundle-delivery), [§7.3](#73-nostr-event-kinds-normative))
...
- **`out_ciphertext`** — per-outgoing-coin NIP-44 v2 encryption of `K_tx` under `K_out = HKDF("zkCoins/v1/OutKey", ovk ‖ epk)`; carried in each `SelfDeliveryRecordV1.output_ref` so an `ovk` holder can recover outgoing plaintext. ([§1.3](#13-per-coin-keys-note-encryption--detection), [§4.2](#42-bundle-delivery))
