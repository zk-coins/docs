...
| `UNAVAILABLE` | not ready / circuit-digest mismatch | `circuit_digest_mismatch` (and not-ready answers that surface as HTTP `503` on `/health/ready`) |
...
| `ListInscriptions` | `malformed_request`/`400` (bad `from_height`/`limit`) | — | — | — | — | `rate_limited`/`429` | not-ready → `503` | `internal_error`/`500` |
