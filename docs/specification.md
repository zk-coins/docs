...
| **11 · Standard identity and messaging** | §1.2 (identity keys seed-derived), §4.1/§4.3 (every account has a name; the name is operational, not cryptographic — node-served, in no preimage, plural, replaceable, survivable on loss; contacts keyed by `op_pubkey`), §7.3/V.12 (mandatory NIP-17/kind-10050 and external-client interoperability), Lightning/mail bridge pages (independently optional) |
...
**Two layers, kept apart (normative).** Every account **MUST** have an email-style **name** (`user@domain`, resolved as NIP-05) and is reachable and payable by it. But the name is **operational, not cryptographic**: it is served by a node or any name provider rather than by the wallet, is never derived from the seed, and is present in no signature preimage and no value-bearing structure. The **protocol layer** underneath therefore resolves a recipient from an `addr_sig`-carrying object — an `Invoice`, or the recipient's signed profile below — and from that object alone; the name is what leads a sender *to* that object, never what authorises it.
...
When no `Invoice` is supplied, a zkCoins recipient **MAY** additionally publish its delivery data in an additive `zkcoins` object inside the standard replaceable Nostr **kind-0 user-metadata event** authored by its `op` key ([§7.3](#73-nostr-event-kinds-normative)). This is a convenience that lets any sender resolve the recipient without asking for an `Invoice` first; it is **not** required for the account to operate:
...
`op_pubkey` is the kind-0 event author and **MUST NOT** be duplicated inside `zkcoins`. The `lud16` field is present only when the same name supports the optional Lightning bridge; when it is present it **MUST** equal the normalized `nip05` value of the same profile, so one profile can never resolve to two different Lightning endpoints ([Lightning bridge](/lightning-bridge)). The `zkcoins` object is **optional for every account**: its absence or invalidity makes payment *through that profile* unavailable — the account remains payable by `Invoice` — and **MUST NOT** disable NIP-17 messaging.

**Publishing it is a privacy decision (normative).** A kind-0 event is public, unencrypted, and — like any Nostr profile — indexed and archived by ordinary clients and aggregators. Publishing a `zkcoins` object therefore ties the profile's name and public key to `address`, `pk0`, `nk_commit`, and `ivpk` **permanently and publicly**. A client **MUST NOT** publish the object without the account holder's explicit choice, **MUST** state that consequence when offering it, and **MUST** keep the account fully operable — receiving, paying, and messaging — when the holder declines ([§4.7](#47-metadata-and-privacy-tradeoffs)).
...
- **A published `zkcoins` profile is public and permanent.** The kind-0 event carrying `nip05` and the `zkcoins` object ([§7.3](#73-nostr-event-kinds-normative)) is not gift-wrapped: it is public and unencrypted, and — like any Nostr kind-0 — is indexed and archived by ordinary Nostr clients and aggregators, so it cannot be retracted once seen. Publishing it therefore binds that name and public key to `address`, `pk0`, `nk_commit`, and `ivpk` in a searchable public record. This is why the object is **opt-in** ([§4.3](#43-addressing-for-delivery)): an account that does not publish it stays fully payable by `Invoice`, which discloses the same fields only to the payer it is handed to. A user who wants a public payment profile *and* an unlinked messaging identity needs separate accounts.
...
| **11 · Standard identity and messaging** | Identity keys are seed-derived (§1.2); standard NIP-17/kind-10050 messaging on that key, interoperable with ordinary Nostr clients (§4.1, §7.3). Every account has an email-style name, but the name is operational rather than cryptographic — node-served, absent from every signature preimage, plural, replaceable, and survivable on loss (§4.3). Lightning and SMTP bridges remain independent and optional. |
...
|---|---|---|---|
| `0` | Nostr user metadata | replaceable | standard profile; all zkCoins-relevant content is optional — `nip05` when the account carries a name, `lud16` when the Lightning bridge is enabled, and the additive `zkcoins` object when the holder opts into a public payment profile |
