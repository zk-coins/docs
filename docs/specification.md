...
- **A publisher therefore needs a key and a relay, and nothing else.** With no fee address to bind, it needs no zkCoins account, no payment address, no SPEND key, and no wallet — only the `op` key that signs its profile and the relays that profile lists ([§7.3](#73-nostr-event-kinds-normative) *Publisher profile*, [Requirement 7](/requirements)).
...
**Name changes (normative).** Pinning covers `op_pubkey` and the payment identity above. A verified kind-0 event under the pinned `op_pubkey` whose `nip05` differs from the stored name **MAY** be adopted once its `name_sig` verifies under the pinned `pk0` **and** a forward NIP-05 resolution of the new name returns that same pinned `op_pubkey`; the client then replaces the stored name, leaving the pinned keys, the payment identity, the conversation, and retained transport as they are. Until that resolution succeeds the client **MUST NOT** present the new name, and **MUST** continue messaging and paying by the pinned key — a name awaiting confirmation costs a label, never reachability. A profile whose `name_sig` fails keeps the stored name in force.

**What `name_sig` proves, and what it does not (normative).** It proves that the seed holder consented to this name for this `op_pubkey`. That consent is **permanent and cannot be withdrawn** ([§5](#5--access--explorer) *Consent is one-way*), so a valid `name_sig` is **not** evidence that its name is the account's current one: an old attestation stays verifiable forever, and a node holding `op` may republish it. Currency comes from resolution, not from signatures — which is why adopting a changed name requires the forward check above, and why this specification defines no name-consent revocation, sequence number, or expiry. A consumer that cannot resolve simply keeps the last name it confirmed. A host that withholds newer profiles is an availability limit no signature scheme closes, and it is not treated as one here. A name that now maps to a **different** public key belongs to a different contact, per the reverse-check outcome above.
...
**Name-consent framing vector (pinned bytes).** The semantic fixture above passes even when signer and verifier share the *same wrong* encoding, so the byte layout is pinned separately. Using the V.2-ext chain, `network = "regtest"`, and the normalized name `alice@example.com`, a harness **MUST** reproduce byte-for-byte: the `name_message` preimage (`"zkCoins/v1/NameConsent"` ‖ `network` ‖ `u32-be name_len` ‖ UTF-8 name ‖ `op_pubkey`, all as raw bytes and in that order), its SHA-256 digest, and a BIP-340 verification of `name_sig` under `pk0`. `name_len` counts the **canonical UTF-8 byte length** of the normalized name, not its character count. Five mutations **MUST** each fail verification and **MUST** be tested individually: a different `network` string; `name_len` as little-endian; `name_len` as a character count where it differs from the byte count (a non-ASCII name is included for exactly this); the name lowercased differently from [§4.3](#43-addressing-for-delivery) normalization; and `op_pubkey` replaced by another valid key.
...
**Publisher profile (accept + reject).** A sixth fixture is a kind-30421 event whose `content` is exactly `{version: 1, relays: […]}` with a non-empty relay array, signed by an `op` key equal to the event `pubkey`. A spender **MUST** accept it and hand off fee-lessly to those relays **without** fetching any kind-0 event, resolving any name, or making any DNS request — the control that publishing needs a key and a relay and nothing more. Three variants **MUST** be rejected: an invalid event signature; a `pubkey` differing from the signing key; and an empty or absent `relays` array. A fourth variant carries any of the deferred fee fields (`fee_address`, `fee_asset_id`, `fee`, `pk0`, `nk_commit`, `ivpk`, `addr_sig`) and **MUST** be rejected as not-v1 under the closed field list of [§7.3](#73-nostr-event-kinds-normative), never silently ignored.
