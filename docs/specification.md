...
3. The sender constructs a **delivery event**, an application-specific Nostr event whose plaintext payload is:
   ...
   DeliveryEvent.payload = {
     ...
     blob_locators,              // serialize(BlobLocatorSet) holders for blob_id (§7.1);
                                 //   blob_id is context (above); lives ONLY in this encrypted interior (§4.3)
...
- **`block_anchor`** = `{ block_hash (32B), height (u32) }` as in [§3.5](#35-inscription-format). **`occurred_at`** is a `u64` Unix-seconds timestamp that is **NOT sender-asserted**: a conforming node **MUST** set it to the **Median-Time-Past (MTP, BIP-113)** of the Bitcoin block that carries the **verified first-occurrence inclusion** of this transition's own `(Pk, R)` in the nullifier-accumulator log ([§1.7.6](#176-nullifier-accumulator-append-only-merkle-log), [§3.6](#36-chain-scanning)) — **exactly** that inclusion block's MTP, no other chain-derived clock — and **MUST** reject a recovered record whose claimed `occurred_at` is inconsistent with that first-occurrence inclusion's MTP.
...
1. **Phase A — build + publish nullifier.** Prove the transition, assemble the nullifier hand-off (`(Pk, R, s, R')` and any fee coin), and publish / self-publish the nullifier ([§3.4](#34-the-publisher), [§7.6](#76-publisher-interface-normative)). Outgoing coin blobs **MAY** already be replicated; the SDR is **not** yet final-serialised.
2. **Phase B — observe first-occurrence, then finalise SDR.** After the node's own scan confirms the first-occurrence inclusion of `(Pk, R)` ([§3.6](#36-chain-scanning)), set `occurred_at` to that inclusion block's **BIP-113 MTP**, set `block_anchor` to the corresponding `{block_hash, height}`, complete every `output_ref` / `self_blob_locators` holder set, **then** compute `serialize(SelfDeliveryRecordV1)`, ZBE-encrypt, content-address, replicate to `k` holders, and self-deliver under [§4.2](#42-bundle-delivery) / [§4.6](#46-data-availability--replication-factor-k).
...
‖ occurred_at (8B u64-be)                                     // MTP of first-occurrence inclusion, §4.2
