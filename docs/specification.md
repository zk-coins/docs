...
```
anchors_commitment = Hc("zkCoins/v1/AnchorsView", anchors_root ‖ anchors_rand)
```

with `anchors_rand` a fresh 256-bit random value per transition. The pair `{anchors_root, anchors_rand}` is the **anchors opening**; it is secret by default and is disclosed only to parties who must check the root against their own scan — carried in the `CoinProof` bundle to a coin's recipient (§1.5) and used in the [§2.1](#21-the-compliance-predicate) witness. A chain-only observer, a `BatchBundle` scanner, and a publisher (who receives a proof but no opening) learn **nothing** from the commitment.
...
- **Carry-forward (clause 1).** In-circuit, `extends(prev_proof.anchors_root, anchors_root)` — the account can only extend its own anchors view, never rewind or replace it. The prover holds its own past openings (seed-/state-derived), so it witnesses `prev_proof.anchors_root` and re-derives `prev_proof.anchors_commitment`.
...
Node / prover:
  4. build the witness with empty inputs, asset_issuance = {asset_id, creator_pubkey = Pk₀,
     ...
     {recipient = owner, amount, asset_id}; pick fresh anchors_rand and set
     anchors_commitment = Hc("zkCoins/v1/AnchorsView", anr_empty ‖ anchors_rand) (a first-
     transition mint has the empty anchors view; §1.7.10)
...
Node / prover:
  ...
  6. pick anchors_root (a scanned-valid MMR root ≥ the account's prior view; the current tip
     is the natural choice) and fresh anchors_rand; compute
     anchors_commitment = Hc("zkCoins/v1/AnchorsView", anchors_root ‖ anchors_rand) (§1.7.10)
...
| Account view key | Bech32m `zkavk` | read full history (incoming side only under the 32-byte `ivk`-only form, §5.8) | whole account | **yes** — `ivk‖ovk` is the secret (or `ivk` alone, incoming-only, §5.8) | no (forward-only by nature) |
...
BalanceAttestation:
  ...
  statement (domain tag "zkCoins/v1/BalanceProof"):
    ...
    7. pi.ProofData.anchors_commitment == Hc("zkCoins/v1/AnchorsView", anchors_root ‖ anchors_rand)
...
The verifier checks the proof, that `anchors_ceiling` is a valid historical anchors-MMR root per its **own** scan ([§1.7.10](#1710-anchors-mmr-normative) — since the subject's own `anchors_root` is proven a prefix of it, every receive folded into the attested state is admitted), and that the `BatchInscription` at `anchor` (`txid`) — which contains the `bundle_locator` whose `BatchBundle` carries `SpendRecordⱼ` at position `j` — is in state `completed` ([On-chain §3.10](#310-transaction-states)) at `{block_hash, height}`. No node, relay, or explorer is trusted. Because `anchors_ceiling` is a **global** MMR root shared by every account, it discloses nothing account-specific; the subject picks any admitted root at or after its own view (the network tip is the natural choice). Because the anchor is a `SpendRecord`, the attestation binds the balance **as of the anchored spend used as `anchor`** — at freshest, the account's most recent anchored spend; balance changes made by later receive transitions (which produce no `SpendRecord`) become attestable only once the account's next anchored spend binds the newer state.
...
- **Reveals the number — and one anchored transition (documented limit).** Amounts, counterparties, coin plaintexts, and every other transition stay hidden: the witness never leaves the proof; the account's receive-recency also stays hidden (`anchors_ceiling` is a global root, not the subject's own prefix). The public `anchor`, however, is disclosed in the clear, and the `BatchBundle` it points to is public and content-addressed ([§4.6](#46-data-availability--replication-factor-k)): a verifier can fetch it and read `SpendRecordⱼ` — its rotating `Pkᵢ`, input count `k`, and nullifier list — and so learns that the subject executed the transition at position `j` of the batch anchored by `txid`, and when (`block_hash`, `height`). Sharing an attestation therefore links the subject's address to exactly one batch member — an exception to the member-unlinkability a `BatchBundle` otherwise provides. A leak-free anchor (e.g. ZK set-membership over the inscribed batches) would be a separate design change; v1 documents the disclosure instead.
...
| `anr_empty` | `Hc("AnchorsMmr/Root", 0)` — the empty anchors-MMR root ([§1.7.10](#1710-anchors-mmr-normative)); the `anchors_root` of an account that has never received | `<REGEN>` |
