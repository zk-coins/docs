...
- **Path B (verifier path)** — a light-client verifier that holds no accumulator and asks any Path-A node for a self-verifying SMT path of `Pkᵢ` (present with leaf `Rᵢ`, or absent). display/delegation only — crediting is Path-A-only ([§2.3.3 step 4](#233-receive)); a Path-B answer, single or combined, MUST NOT back a credit. ([§3.7](#37-the-nullifier-accumulator))
