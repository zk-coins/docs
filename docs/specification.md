...
CoinProof    = {                            // the value-bearing off-chain bundle (bearer)
  ...
  creating_nullifier = { Pk_create,          // the creating transition's on-chain nullifier (§3.1);
                         ...
                         R_prime_create },    // (§2.1 clause 10(d) / clause 9) and R_create S2C-opens to
                                             ...
                                             // the accumulator (§2.3.3 step 4, §5.6), including for a mint
  ...
  asset_terms? = { creator_pubkey, name,     // OPTIONAL plaintext IssuanceTerms of coin.asset_id
                   decimals,                 // (§6.5, §2.3.2) — determines exactly the non-constant
                   issuance_version },       // asset_id preimage inputs of §1.4 (name enters as
                                             // name_hash = H(name)); self-authenticating: the
                                             // receiver recomputes asset_id and rejects the bundle
                                             // on mismatch (§2.3.3 step 6); absent ⇒ coin valid,
                                             // asset carried as opaque asset_id only
...
Receiver / node:
  ...
  6. amount/asset sanity & issuance terms: confirm coin.recipient = receiver's address and
     ...
     recompute asset_id == Hc("AssetId", genesis_tag ‖ creator_pubkey ‖ H(name) ‖ decimals
     ‖ issuance_version) (§1.4) from the supplied terms and compare against coin.asset_id;
     a mismatch MUST reject the whole bundle — never credit the coin while silently
     discarding the terms. If asset_terms is absent the coin remains valid and creditable,
