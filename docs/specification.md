...
Produces an off-chain mint object (the canonical §1.4 SpendRecord layout with k = 0):
  { public_key: Pk₀ (x-only), signature: BIP-340(sk₀, inr ‖ ocr), message: inr ‖ ocr,
    k: 0, nullifiers: [] (a mint spends nothing) }
...
Produces the SpendRecord (off-chain, handed to a publisher; canonical §1.4 layout):
  { public_key: Pkᵢ (x-only), signature: BIP-340(skᵢ, inr ‖ ocr), message: inr ‖ ocr,
    k: |input_coins|, nullifiers: [nf for each input coin] }
...
The mint case has `k = 0` (an empty nullifier list, 161 bytes total) and need not be batched at all: a creator who chooses not to anchor a mint hands the receiver only the `CoinProof` bundle, and the mint's validity is checked by direct recursive verification of the mint's proof — an `InitialProof`, or an `AccountUpdateProof` carrying `asset_issuance` for a follow-up mint ([§2.3.1](#231-mint--issuance)).
