...
**`‖` inside `Hc` call sites (normative).** Wherever this document writes `Hc(tag, x₁ ‖ x₂ ‖ … ‖ xₙ)`, the `‖` separates the **input list** of the §1.7.1 signature: each `‖`-separated argument is **one** input, individually encoded by the rules above and the widths of [§1.7.3](#173-fixed-widths) — the arguments **MUST NOT** be byte-concatenated into a single byte-string input first. `Hc(tag, a ‖ b)` and `Hc(tag, a, b)` denote the same invocation. For example, `Hc("Nullifier", nk ‖ coin.identifier)` absorbs `nk` as a byte-string input (length prefix + 5 chunks) followed by `coin.identifier` as a 4-limb digest input. Byte concatenation applies only outside `Hc` input lists — e.g. in `serialize(…)` layouts, `H(…)` and HKDF preimages (`ss ‖ epk`), Bech32m payloads (`ivk ‖ ovk`), and `message = inr ‖ ocr ‖ next_pubkey`. In the `bundle_locator` preimage `Hc("BatchBundle", prev_root ‖ new_root ‖ u32-be(m) ‖ member_root)` ([§1.4](#14-identifiers-and-hashes)), `u32-be(m)` pins the member count `m` as a **4-byte big-endian byte string**, absorbed via the byte-string rule (one length element + one chunk element) — **not** as a small-numeric element.
...
7. **New account state.** `new_account_state` is `prev_account_state` with: `balances` updated per clause 3 (debit spent inputs, credit change and any issuance) **and credited with every `received_coins[]` amount admitted by clause 10**, `current_pubkey = next_pubkey = Pkᵢ₊₁`, `send_counter` incremented by one, and `coin_history_root` set to the value produced by clause 8 (the recomputed per-account coin-history SMT root, [Foundations §1.7.6](#176-nullifier-accumulator-sparse-merkle-tree)). The updated `balances` **MUST** hold at most `MAX_ACCOUNT_ASSETS` distinct non-zero entries ([§2.5](#25-circuit-dimensioning-normative)); the circuit builds `serialize(new_account_state)` over its fixed `MAX_ACCOUNT_ASSETS` balance slots with the inactive-slot discipline of [§1.7.4](#174-serializeaccountstate), so the in-circuit `ash` equals the out-of-circuit variable-length `Hc("AccountState", serialize(new_account_state))` bit-for-bit. `ProofData.new_account_state_hash` **MUST** equal `ash = Hc("AccountState", serialize(new_account_state))` ([Foundations §1.4, §1.7.4](#14-identifiers-and-hashes)). `new_account_state.owner` **and** `new_account_state.nk_commit` **MUST** be unchanged.
...
1. **Per-member soundness.** For each `j ∈ [1, m]`, the spender's per-account proof verifies under `C`'s verifier data, its public `ProofData` matches the member `SpendRecord`'s `message = inr ‖ ocr ‖ next_pubkey` (its leading `inr ‖ ocr` against `ProofDataⱼ.input_nullifiers_root ‖ output_coins_root`; the trailing `next_pubkey` is already tied to `ProofDataⱼ.new_account_state_hash` **inside** the per-account proof verified here — [§2.1 clause 2](#21-the-compliance-predicate) signs it and clause 7 folds the same witness value into `ash` — so `C_batch` need not re-derive it), and the spender's BIP-340 signature verifies against `(Pkⱼ, messageⱼ)` over the full 96-byte `messageⱼ` (half-aggregated in-circuit is permissible — [On-chain §3.3](#33-off-chain-signature-handling)).
...
Wallet:
  1. derive Pk₀ = sk₀·G and the rotated next_pubkey Pk₁ (both SPEND branch); sign the transition
     signature BIP-340(sk₀, message = inr ‖ ocr ‖ next_pubkey) with next_pubkey = Pk₁
     (a mint spends no coin, so inr is the empty-set root; signed by Pk₀, i.e. sk₀; current_pubkey
      rotates to Pk₁, which the signed message now binds directly)
  2. derive name_hash = H(name); asset_id = Hc("AssetId", genesis_tag ‖ Pk₀ ‖ name_hash ‖ decimals ‖ issuance_version=1)   (Foundations §1.4)
  3. provide next_pubkey Pk₁ from the SPEND branch (the node already holds nk — operational
     bundle, Foundations §1.2)
...
Produces an off-chain mint object (the canonical §1.4 SpendRecord layout with k = 0):
  { public_key: Pk₀ (x-only), signature: BIP-340(sk₀, inr ‖ ocr ‖ next_pubkey),
    message: inr ‖ ocr ‖ next_pubkey (next_pubkey = Pk₁), k: 0, nullifiers: [] (a mint spends nothing) }
...
Wallet:
  1. sign the single transition signature BIP-340(skᵢ, message = inr ‖ ocr ‖ next_pubkey) with the
     current per-transition signing key skᵢ (whose Pkᵢ is current_pubkey; no per-coin key), appending
     the wallet's own rotated next_pubkey Pkᵢ₊₁ (step 2) as the third message segment
  2. supply the rotated next_pubkey Pkᵢ₊₁ (SPEND branch, Foundations §1.2); the node
...
Produces the SpendRecord (off-chain, handed to a publisher; canonical §1.4 layout):
  { public_key: Pkᵢ (x-only), signature: BIP-340(skᵢ, inr ‖ ocr ‖ next_pubkey),
    message: inr ‖ ocr ‖ next_pubkey (next_pubkey = Pkᵢ₊₁), k: |input_coins|, nullifiers: [nf for each input coin] }
...
When the publisher's `BatchInscription` is admitted ([On-chain §3.6](#36-chain-scanning)), its `AggregateBatchProof` inserts every member's nullifiers into the global accumulator and the spent coins become unspendable again ([§3.7](#37-the-nullifier-accumulator)). The rotated `current_pubkey` is bundle-only (the inscription itself reveals only the publisher's identity, not the spender's), and the proof's fifth public input is a **hiding** `anchors_commitment` with fresh randomness per transition ([§1.7.10](#1710-anchors-mmr-normative)) rather than a decodable anchors root. The publisher does receive the fee coin's `anchors_opening` (it must, to later fold the fee coin, [§3.8](#38-fees-and-economics)) — but because the spender **MUST** set `anchors_root` to the live tip ([§1.7.10](#1710-anchors-mmr-normative)), that opening reveals only the (globally shared) proving-time tip, not the account's private receive history; distinct transitions carry distinct commitments, so the publisher still cannot link this `SpendRecord` to the account's prior records, and a chain-only observer learns nothing about the spender. Delivery of the per-recipient `CoinProof` over Nostr is specified in [Transport & Recovery](#4--transport--recovery); delivery of the spender's `SpendRecord` to the publisher is specified in [Transport & Recovery §4.6](#46-data-availability--replication-factor-k).
...
Receiver / node:
  ...
  4. anchoring: verify that output_coins_root is bound by a SpendRecord whose containing
     ...
     over message = inr ‖ ocr ‖ next_pubkey verifies, the spender's nullifiers appear in the BatchBundle that
...
Each member `SpendRecord` ([Foundations §1.5](#15-core-data-structures)) inside a `BatchBundle` ([Transport & Recovery §4.6](#46-data-availability--replication-factor-k)) carries its own spender's BIP-340 signature over `message = inr ‖ ocr ‖ next_pubkey`, sign-to-contract-bound to that spender's own off-chain validity proof via the S2C tweak `t = H(R' ‖ H(ProofData))` with the canonical `H(ProofData) = SHA-256(serialize(ProofData))` of [§1.4](#14-identifiers-and-hashes) and the standard `R = R' + t·G` tweak. The publisher's `AggregateBatchProof` ([Proofs §2.2](#22-proof-types)) verifies *every* member signature **in-circuit** — so the on-chain `BatchInscription` does not separately carry any spender signature.
...
- Sign each transition — produce `BIP-340(skᵢ, message)` over `message = inr ‖ ocr ‖ next_pubkey` (Foundations §1.4; the wallet appends its own rotated `next_pubkey`) with the sign-to-contract tweak that binds the spender's off-chain `ProofData`. The node derives the nullifiers `nf = Hc("Nullifier", nk ‖ coin.identifier)` from the operational bundle's `nk` while building the witness ([§2.1 clause 4](#21-the-compliance-predicate)). The resulting `SpendRecord` is handed to a publisher off-chain ([Transport & Recovery §4.6](#46-data-availability--replication-factor-k)); the wallet does **not** itself touch Bitcoin.
...
- **`message`** — `inr ‖ ocr ‖ next_pubkey`; the BIP-340-signed payload of a `SpendRecord` (96 bytes) — the spent-nullifier root, the output-coins root, and the account's rotated spend authority. ([§1.4](#14-identifiers-and-hashes))
...
| `message@0` | `inr@0 ‖ ocr@0 ‖ next_pubkey@0` (three 32-byte values — the two roots above and the genesis mint's rotated spend key `next_pubkey@0 = Pk₁_sample` (V.1), the same illustrative value pinned as `current_pubkey` in V.3 so the clause-2/clause-7 rotation invariant `next_pubkey == current_pubkey` holds; **96 bytes**) | `<REGEN>` (the two roots are Poseidon-dependent; the third segment is the fixed `Pk₁_sample`) |
...
message      (96B): <message = inr ‖ ocr ‖ next_pubkey>   ← inr 32B ‖ ocr 32B ‖ next_pubkey 32B (x-only)
...
5. Compute the BIP-340 signature with sign-to-contract per [§3.3](#33-off-chain-signature-handling) (the spender's S2C tweak over `H(ProofData)`, over the full 96-byte `message@0 = inr@0 ‖ ocr@0 ‖ next_pubkey@0`) and fill in V.5's `signature`. The signing key `sk₀` is a real secp256k1 key derived from a real BIP-32 path; a separate signing fixture is needed because the V.1 illustrative `Pk₀_sample` is a raw 32-byte string, not a curve point. The third `message@0` segment binds `next_pubkey@0 = Pk₁_sample` (V.1) — the same illustrative value pinned as `current_pubkey` in V.3, so the signed message carries exactly the key clause 7 folds into `ash@0` (clause-2/clause-7 invariant `next_pubkey == current_pubkey`); it is signed as raw message bytes and need not be a curve point.
