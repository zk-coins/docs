...
- **Identifiers and per-coin derivations** — `AssetId`, `Coin`, `AccountState`, `Nullifier`, `NoteKey`, `DetectTag`, `OutKey` (outgoing-recovery key, §1.3).
...
7. **New account state.** `new_account_state` is `prev_account_state` with: `balances` updated per clause 3 (debit spent inputs, credit change and any issuance) **and credited with every `received_coins[]` amount admitted by clause 10**, `current_pubkey = next_pubkey = Pkᵢ₊₁`, `send_counter` incremented by one, and `coin_history_root` set to the value produced by clause 8 (the recomputed per-account coin-history SMT root, [Foundations §1.7.6](#176-nullifier-accumulator-sparse-merkle-tree)). `ProofData.new_account_state_hash` **MUST** equal `ash = Hc("AccountState", serialize(new_account_state))` ([Foundations §1.4, §1.7.4](#14-identifiers-and-hashes)). `new_account_state.owner` **MUST** be unchanged.
...
Node / prover:
  ...
  5. run C as an InitialProof (clause 1, InitialProof path) when this is the account's first
     ...
     checks the four §6.5 mint clauses — issuance_version == 1, H(creator_pubkey) == owner,
...
Receiver / node:
  ...
  5. nullifier non-membership: against the live accumulator at NAV(tip) (Onchain §3.7), verify
     ...
     clause 2(a)), so this receiver-derived nf is the coin's only valid nullifier; the check
     detects re-crediting a coin this account has already spent (a recovery/replay case).
...
1. **Re-derive keys.** From the seed, re-derive the account root `A` and thereby `ivk`, `ovk`, `op`, the nullifier key `nk`, `op_secret` (the anchors-randomness key, §1.7.10), and the spend keys ([Foundations §1.2](#12-key-hierarchy)). This alone restores the address/identity, decryption ability, the deterministic detection tags, and — via `op_secret` — the deterministic `anchors_rand` needed to rebuild prior anchors openings.
...
| `GET` | `/v1/info` | `{ network, bitcoin_network, protocol_version: "v1", circuit_digests: { C, C_batch }, relay_url, blossom_url, finality_confirmations: 6, max_tx_inputs: 8, max_tx_outputs: 8, max_rx_coins: 4 }` |
