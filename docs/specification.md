...
- **Leaf hash** (the RFC 6962 `0x00` leaf domain, here the tag `NfLog/Leaf`; **binds the position**): the hash of entry `p = (Pk, R)` is `MTH([eₚ]) = Hc("NfLog/Leaf", p ‖ Pk ‖ R)`, with `p` a `u64` big-endian numeric input and `Pk, R` the two 32-byte inputs ([§1.7.2](#172-field-encoding-e-of-hc-inputs)).
...
- **Empty log:** `MTH({}) = Hc("NfLog/Empty", 0)`, a protocol constant.
- **Accumulator value.** The accumulator at `size = n` is the pair `(n, mth)`, `mth = MTH(D[0:n])`; its committed 32-byte form is `nav_root = Hc("NfLog/Root", n ‖ mth)` (`n` a `u64` input). Binding `size` inside the preimage forecloses length-extension. This is the value opened by `nav_commitment = Hc("NavCommit", nav_root ‖ nav_rand)` ([§1.4](#14-identifiers-and-hashes)).
...
| coin-history / nullifier SMT depth | **256** | [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log) |
...
| `GET` | `/v1/chain/nullifier/<pubkey>` | a self-verifying SMT path for the account-state key `<pubkey> = Pkᵢ` against the current accumulator root (the Path-B service of [§3.7](#37-the-nullifier-accumulator)): `{ root, tip_block_hash, tip_height, present: bool, leaf?: <Rᵢ hex, when present>, siblings: [hex; 256] }`; `siblings` are ordered **bottom-to-top** (element `i` is the level-`i` sibling, [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log)), left/right per bit `i` of the key under the §1.7.6 bit rule (`tip_block_hash` 32B, internal order) |
...
- **Nullifier accumulator** — global, 256-bit-depth SMT keyed by every account-state nullifier public key `Pkᵢ` ever published on-chain, leaf value the transition's S2C commitment `Rᵢ`, folded by **first-occurrence** (first-spend-wins) from the on-chain nullifiers; a **pure function of Bitcoin**, so two honest nodes never diverge; the only global structure. ([§1.6](#16-trees-one-global-structure-one-per-account-structure), [§3.7](#37-the-nullifier-accumulator), [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log))
...
- **SMT (Sparse Merkle Tree)** — 256-bit-depth Merkle tree with default-hashed empty subtrees; used for the coin-history root and the global nullifier accumulator. ([§1.6](#16-trees-one-global-structure-one-per-account-structure), [§1.7.6](#176-nullifier-accumulator-append-only-merkle-log))
