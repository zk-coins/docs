...
| `GET` | `/v1/chain/inscriptions?from_height=&from_tx_index=&limit=` | paginated list of zkCoins nullifier inscriptions. **Query (normative):** `from_height` optional, default `0`; `from_tx_index` optional, default `0` — together the **inclusive** lexicographic lower bound on the reveal transaction's `(height, tx_index)` pair; `limit` optional, default `100`, valid range `1..1000` — `limit = 0` or `limit > 1000` → `400 bounds_exceeded`. **Response:** `{ inscriptions: [ { txid, height, tx_index, count, format, nullifiers: [{ pubkey, r, state }], confirmation_state } ], next_height?: <u64>, next_tx_index?: <u64> }` — the half-aggregated `(Pkⱼ, Rⱼ)` set of each inscription ([§3.5](#35-inscription-format)), whose signatures the node has verified against Bitcoin; each `nullifiers[i].state` is that member's §3.10 state (`completed` \| `pending` \| `failed`) — members of one aggregate MAY differ by first-occurrence (a later `Pk` collision is `failed` while earlier members stay `pending`/`completed`); `confirmation_state ∈ {pending, completed}` is **only** the reveal transaction's confirmation depth relative to the §3.9 6-confirmation floor (not a per-nullifier aggregate of §3.10) and **MUST NOT** carry `failed`. **Stable sort (normative):** ascending `(height, tx_index)` of the reveal transaction (then the §3.6 payload-member order inside one inscription). **Pagination cursor (normative):** when more results remain, **both** `next_height` and `next_tx_index` are present and form the **exclusive** pair-cursor after the last returned inscription — the `(height, tx_index)` of the first not-yet-returned inscription in sort order; a client requests the next page with `from_height = next_height` and `from_tx_index = next_tx_index` (so a page that ends mid-block does **not** skip the remaining inscriptions of that height). When no further results remain, **both** cursor fields are **absent**. A response **MUST NOT** carry only one of the two cursor fields |
...
| `ListInscriptions` | server-stream | zkCoins nullifier inscriptions from a `(height, tx_index)` cursor (paginated; defaults `from_height=0`, `from_tx_index=0`, `limit=100`) | `GET /v1/chain/inscriptions` |
...
message ListInscriptionsRequest {
  ...
  //   from_tx_index = 0, limit = 100. Valid limit ∈ 1..1000; 0 or >1000 → INVALID_ARGUMENT / HTTP 400
  //   bounds_exceeded.
  // Proto3: optional so absence is distinguishable from zero; a caller that sets limit = 0 is rejected.
  // Inclusive lexicographic lower bound on (height, tx_index); REST response carries
  //   next_height + next_tx_index as the exclusive pair-cursor (§7.5). The kernel stream itself yields
  //   Inscription messages in stable (height, tx_index) sort order.
...
message Inscription {
  ...
  uint64 tx_index = 7;                     // reveal-tx index within the block; pairs with height for sort/cursor
}
