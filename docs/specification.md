...
| **11 · Standard identity and messaging** | §1.2 (identity keys seed-derived), §4.1/§4.3 (the app and API layers give every account they serve a name and a signed kind-0 payment object; the name is operational, not cryptographic — node-served, in no preimage, plural, replaceable, survivable on loss; contacts keyed by `op_pubkey`), §7.3/V.12 (mandatory NIP-17/kind-10050 and external-client interoperability), Lightning/mail bridge pages (independently optional) |
...
**Names (normative).** The API and app layers **MUST** provide every account they serve with an email-style **name** `<user>@<domain>`, resolved as NIP-05, and **MUST** publish the [§7.3](#73-nostr-event-kinds-normative) kind-0 `zkcoins` object under it. The kernel requires neither: it is addressed by `op_pubkey`, `nprofile`, or an `addr_sig`-carrying object, and **MUST NOT** perform NIP-05, DNS, or `/.well-known` resolution ([§6.1](#61-components-and-responsibilities) *Kernel and API — two boundaries*). A name is not seed-derived and appears in no signature preimage and no value-bearing structure. Recipient resolution verifies an `addr_sig`-carrying object — an `Invoice` or the kind-0 profile below — never the name.

A name **MUST** be replaceable and an account **MAY** carry several at once; neither affects a key, a balance, or an established contact. The name **MAY** come from the operator's `aliasing` role ([§6.1](#61-components-and-responsibilities)), a self-hosted domain, or any third-party NIP-05 provider.
...
The kernel addresses a counterparty by key and **MUST NOT** require a name to message or pay it. An application resolves a name before presenting a contact ([Implementation Mandate](/implementation-mandate#app-layer-identity-and-contacts-normative)). A name verified later **MUST** be attached to the existing record, not used to create a second one.
...
| **11 · Standard identity and messaging** | Identity keys are seed-derived (§1.2); standard NIP-17/kind-10050 messaging on that key, interoperable with ordinary Nostr clients (§4.1, §7.3). The app and API layers give every account they serve an email-style name and the signed payment object bound to it; the name is operational rather than cryptographic — node-served, absent from every signature preimage, plural, replaceable, and survivable on loss (§4.3). Lightning and SMTP bridges remain independent and optional. |
