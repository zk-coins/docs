...
A name is therefore only ever an **entry point** to a record, never the record's identity. A contact reached by `nprofile`, by an `Invoice`, or by an earlier conversation is complete without any name at all.
