...
The per-coin `CoinProof` bundle — the recipient's custody of a coin's value and history — still travels off-chain, replicated to `k = 3` independent holders ([spec §4.6](/specification#46-data-availability)); that is a different object from the on-chain nullifier and its data-availability story is unchanged.
