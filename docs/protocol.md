...
The per-coin `CoinProof` bundle — the recipient's custody of a coin's value and history — still travels off-chain, retained indefinitely by every holder and published to the network's seed-discoverable relays and blob stores ([spec §4.3](/specification#43-addressing-for-delivery), [§4.6](/specification#46-data-availability)); that is a different object from the on-chain nullifier and its data-availability story is unchanged.
