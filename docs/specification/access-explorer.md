...
4. The explorer surfaces **verifiable evidence**: it checks the coin's inclusion in `output_coins_root`, that root's `SpendRecord` is admitted under [On-chain §3.5+§3.6](onchain) — admission, not mere inscription, and the recursive validity proof ([Foundations §1.4, §1.5](foundations)). The viewer therefore trusts **Bitcoin and the proof — never the explorer's assertion**.
...
The verifier checks the proof and that the `SpendRecord` at `anchor` (`txid:j`) is **admitted** at `{block_hash, height}` under [On-chain §3.5+§3.6](onchain) — admission, not mere inscription; a record the verifier's own scanner has rejected MUST be treated as not anchored. No node, relay, or explorer is trusted.
