...
- **`asset_id`** — `Hc("AssetId", genesis_tag ‖ Pk₀ ‖ H(name) ‖ decimals ‖ issuance_version)`; globally unique per asset, binds the creator's `Pk₀` and the issuance-schema version, never carries the human-readable name on-chain. ([§1.4](foundations#14-identifiers-and-hashes), [§6.5](architecture#65-issuance--versioned-schemas-v1-minimal))
...
- **`completed` (transaction state)** — `SpendRecord` is admitted under §3.5+§3.6 AND its inclusion block has ≥ 6 confirmations; the only state in which a receiver MAY credit; absolute under the "no reorgs deeper than 5 blocks" assumption. ([§3.10](onchain#310-transaction-states))
...
- **`failed` (transaction state)** — `SpendRecord` rejected by the scan (parser, `block_anchor` bounds, signature, `inr` binding, canonical order, or first-spend-wins violated); receiver MUST NOT credit; forward-sticky in time, can only flip via reorg. ([§3.10](onchain#310-transaction-states))
...
- **`IssuanceTerms`** — the versioned record bound to an `asset_id` that fixes its mint rules. v1 is creator-only with no protocol-enforced cap, quantum, or time window — `{asset_id, issuance_version=1, name_hash, decimals, terms_hash}`. Later versions MAY add protocol-enforced supply rules. ([§6.5](architecture#65-issuance--versioned-schemas-v1-minimal))
- **`issuance_version`** — `u8` schema version under which an asset is created (`1` in this spec); bound into `asset_id` so coins minted under different versions are distinct. ([§1.4](foundations#14-identifiers-and-hashes), [§6.5](architecture#65-issuance--versioned-schemas-v1-minimal))
...
- **`pending` (transaction state)** — `SpendRecord` classified as neither `completed` nor `failed`: either still off-chain or inclusion block has < 6 confirmations; receiver MUST NOT credit, but its `nf`s are already in the nullifier accumulator (double-spend protection applies). ([§3.10](onchain#310-transaction-states))
...
- **Transaction state** — see `completed`, `failed`, `pending` ([§3.10](onchain#310-transaction-states)).
