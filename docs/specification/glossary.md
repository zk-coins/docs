...
- **`completed`** — the `SpendRecord` lifecycle state in which the record has been admitted under §3.5+§3.6 by the verifier's own scan **and** its inclusion block has at least 6 confirmations; the only state in which a receiver MAY credit. Absolute under zkCoins' assumption that Bitcoin has no reorgs deeper than 5 blocks. ([§3.10](onchain#310-transaction-states))
...
- **`failed`** — the `SpendRecord` lifecycle state in which the verifier's §3.5/§3.6 scan has rejected the record (any single admission rule violated). Forward-sticky: a rejection cannot become an admission by waiting. Never crediteable. ([§3.10](onchain#310-transaction-states))
...
- **`pending`** — the `SpendRecord` lifecycle state of a record that is neither `completed` nor `failed` — either still off-chain or inscribed with fewer than 6 confirmations. A receiver MUST NOT credit it; but its `nf`s are already in the nullifier accumulator at admission (asymmetry: admission gates the accumulator, finality gates credit). ([§3.10](onchain#310-transaction-states))
