...
- A publisher SHOULD batch over a bounded interval (e.g. **once per hour**, well within §3.5's `N = 100`-block anchor bound) and SHOULD half-aggregate (§3.3) to minimise per-record cost. The half-aggregated marginal cost per record is fixed (§3.5 size note), so per-record cost flattens beyond roughly 100 records per batch — batching longer than ~1 hour adds latency without meaningfully saving cost. Records inscribed redundantly by two publishers are idempotent: a scanner inserts each unique `nf` into the accumulator once, and a second inscription of an already-seen `nf` is a no-op (§3.6).
