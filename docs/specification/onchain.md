...
**The publisher carries the failed-record risk.** A `SpendRecord` whose admission fails (rejected by §3.5 or §3.6, state `failed` per §3.10) was still inscribed — the publisher's BTC fee is spent regardless of the record's fate. Publishers therefore **SHOULD** admission-verify each record they accept before broadcasting (proof verifies, every `nf` non-member of their accumulator at the proving tip, `block_anchor` within bound, message and nullifier-binding intact). This filter eliminates **most but not all** failure paths: the residual covers (a) state drift between proving tip and Bitcoin inclusion height, and (b) **simultaneous-inscription races** where two publishers inscribe records sharing one `nf` and the §3.6 canonical order picks the other one as the winner. The expected residual loss across both enters the fee they quote; there is **no** in-band "refund on failure" mechanism, and one would not be sound — the Bitcoin fee is already burned.

**Reimbursement is a zkCoins-native transfer, never a Bitcoin UTXO.** A spender **MAY** compensate a publisher by including a fee-allowance `CoinTemplate` (§1.5) in the same transition's off-chain bundle ([Transport & Recovery](transport-recovery)): `recipient = publisher.address`, `asset_id` one of the publisher's advertised accepted fee assets. The fee coin is created by the same `SpendRecord` as the rest of the spend, so on-chain it becomes `completed` precisely when the rest does — and never if the spend is `failed`. A spender **MUST NOT** sign or expose a Bitcoin UTXO; the spender's on-chain footprint stays limited to the opaque `SpendRecord`.

**Reimbursement delivery is part of the same hand-off.** On-chain atomicity of the fee coin is only half the story: the publisher also needs the off-chain `CoinProof` bundle for the fee coin in order to actually spend it later (§1.5, §4.2). The spender's wallet therefore **MUST** deliver the publisher's fee-coin bundle to the publisher's advertised relay set per [Transport & Recovery §4.2](transport-recovery) — encrypted to the publisher's `IVPK`, addressed via the publisher's `op_pubkey`, replicated to `k` independent holders (§4.6) — **before or concurrently with** submitting the spend for inscription. A publisher **MAY** refuse to broadcast a spend until it has received and verified that bundle. This closes the delivery-trust asymmetry: a publisher that has not received the fee-coin bundle simply does not broadcast, and the spender has no way to extract the inscription without paying.
...
**Publisher profile (what a publisher MUST advertise).** A publisher that accepts zkCoins-native reimbursement is, by construction, also a **zkCoins account holder** (it is the recipient of a fee `CoinTemplate`, so it must possess an `address`, an `IVPK` to be a delivery target, and the SPEND-branch keys to ever spend the fee coin onward). A reimbursable publisher **MUST** publicly advertise:

- its **`address`** (`= H(Pk₀)`, [Foundations §1.4](foundations)) — the `recipient` of the fee `CoinTemplate`;
- its **`IVPK`** ([Foundations §1.3](foundations)) — the delivery target for the fee-coin `CoinProof` bundle;
- its **`op_pubkey`** ([Foundations §1.2](foundations)) — the Nostr identity used to ACK delivery (§4.2);
- its **advertised relay set** — where the spender's wallet publishes the fee-coin bundle;
- its **accepted fee assets** and per-asset rates (e.g. a BTC-pegged asset, a stablecoin asset, or the asset being moved itself).

A wallet picks a publisher whose accepted set intersects what the wallet holds; if no intersection exists, the wallet **MUST** swap into an accepted asset first or self-publish. The advertisement *format* and discovery mechanism are **out of scope of v1** — implementations advertise out-of-band (a publisher's HTTPS profile, a Nostr replaceable event under `op_pubkey`, a directory) — but the rule that the publisher names every field above publicly is normative, so wallets can shop and so no publisher can demand undisclosed terms after the fact. A **Bitcoin-only broadcaster** that declines zkCoins-native reimbursement entirely (and is paid by some other means, or self-funded) is still legal under §3.4 and need not run a zkCoins account.
